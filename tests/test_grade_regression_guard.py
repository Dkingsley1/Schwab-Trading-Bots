...
def test_grade_regression_guard_reports_degraded_when_surfaces_are_recovering(
    ...
    _write_json(
        ...
        {"overall_status": "needs_attention", "training_quality_score": 82.62},
    )
    _write_json(
    ...
    _write_json(
        health / "security_audit_latest.json",
        {"overall_status": "ready", "passed_checks": 11, "failed_checks": 0},
    )
    _write_json(
        health / "incident_closeout_autopilot_latest.json",
        {
            ...
            "bounded_data_plane_recovery": True,
        },
    )
    _write_json(
    ...
    _write_json(
        health / "autonomy_control_plane_latest.json",
        {"overall_status": "degraded", "autonomy_score": 50.0},
    )
    _write_json(
        champion / "promotion_autopilot_packet_latest.json",
        {
            "overall_status": "degraded",
            "packet_completeness_score": 45.0,
    ...
    assert any(
        row["surface"] == "training_quality" and row["state"] == "degraded"
        for row in payload["surfaces"]
    )
    assert payload["regression_guardrail_contract"]["per_surface_retry_budgets"] is True
    assert all(
        row["retry_budget"]["max_attempts_per_run"] >= 1 for row in payload["surfaces"]
    )
    assert all(
        row["notification_contract"]["dedupe_key"].startswith("grade_regression:")
        for row in payload["surfaces"]
    )
...
def test_grade_regression_guard_blocks_on_hard_regression(tmp_path: Path) -> None:
    ...
    _write_json(
        health / "training_quality_control_latest.json",
        {"overall_status": "blocked", "training_quality_score": 40.0},
    )
    _write_json(
        health / "training_lineage_manifest_latest.json",
        {"overall_status": "blocked", "lineage_score": 20.0},
    )
    _write_json(
        health / "ingestion_storage_control_latest.json",
        {
    ...
    _write_json(
        health / "security_audit_latest.json",
        {"overall_status": "needs_work", "passed_checks": 6, "failed_checks": 12},
    )
    _write_json(
        health / "incident_closeout_autopilot_latest.json",
        {
            ...
            "bounded_data_plane_recovery": False,
        },
    )
    _write_json(
        health / "live_canary_control_latest.json",
        {"overall_status": "blocked", "recommended_mode": "hold"},
    )
    _write_json(
        health / "autonomy_control_plane_latest.json",
        {"overall_status": "blocked", "autonomy_score": 20.0},
    )
    _write_json(
        champion / "promotion_autopilot_packet_latest.json",
        {
...
def test_grade_regression_guard_softens_training_quality_for_guarded_paper_soak(
    tmp_path: Path,
) -> None:
    health = tmp_path / "governance" / "health"
    champion = tmp_path / "governance" / "champion_challenger"
    _write_json(
        health / "training_quality_control_latest.json",
        {"overall_status": "blocked", "training_quality_score": 56.0},
    )
    _write_json(
        health / "training_lineage_manifest_latest.json",
        {
    ...
    _write_json(
        health / "security_audit_latest.json",
        {"overall_status": "ready", "passed_checks": 17, "failed_checks": 0},
    )
    _write_json(
        health / "incident_closeout_autopilot_latest.json",
        {"overall_status": "ready", "open_incident_count": 0},
    )
    _write_json(
        health / "live_canary_control_latest.json",
        {
    ...
    _write_json(
        health / "autonomy_control_plane_latest.json",
        {"overall_status": "ready", "autonomy_score": 96.0},
    )
    _write_json(
    ...
    _write_json(
        ...
        {
            "overall_status": "ready",
            "packet_completeness_score": 100.0,
    ...
    payload = src.build_payload(tmp_path)
    training_row = next(
        row for row in payload["surfaces"] if row["surface"] == "training_quality"
    )
...
def test_grade_regression_guard_treats_current_guarded_paper_debt_as_advisory(
    tmp_path: Path,
) -> None:
    health = tmp_path / "governance" / "health"
    champion = tmp_path / "governance" / "champion_challenger"
    _write_json(
        health / "training_quality_control_latest.json",
        {"overall_status": "blocked", "training_quality_score": 38.88},
    )
    _write_json(
        health / "training_lineage_manifest_latest.json",
        {
    ...
    _write_json(
        health / "incident_closeout_autopilot_latest.json",
        {"overall_status": "blocked", "open_incident_count": 2},
    )
    _write_json(
        health / "live_canary_control_latest.json",
        {"overall_status": "blocked", "recommended_mode": "validate_only"},
    )
    _write_json(
        health / "autonomy_control_plane_latest.json",
        {"overall_status": "ready", "autonomy_score": 96.0},
...
def test_grade_regression_guard_marks_guarded_paper_promotion_and_autonomy_debt_ready_advisory(
    ...
    _write_json(
        health / "training_quality_control_latest.json",
        {"overall_status": "ready", "training_quality_score": 92.0},
    )
    _write_json(
        health / "training_lineage_manifest_latest.json",
        {
    ...
    _write_json(
        health / "live_canary_control_latest.json",
        {"overall_status": "degraded", "recommended_mode": "validate_only"},
    )
    _write_json(
        health / "autonomy_control_plane_latest.json",
        {"overall_status": "degraded", "autonomy_score": 81.96},
    )
    _write_json(
        health / "health_fast_latest.json",
        {
    ...
    _write_json(
        ...
        {
            "overall_status": "degraded",
            "packet_completeness_score": 75.0,
            "promotion_ready": False,
        },
    )
    ...
    assert (
        rows["promotion_autopilot"]["metrics"]["live_promotion_gate_deferred"] is True
    )


def test_grade_regression_guard_accepts_paper_soak_lineage_without_signed_promotion_packet(
    tmp_path: Path,
) -> None:
    health = tmp_path / "governance" / "health"
    champion = tmp_path / "governance" / "champion_challenger"
    _write_json(
        health / "training_quality_control_latest.json",
        {"overall_status": "blocked", "training_quality_score": 63.0},
    )
    _write_json(
        health / "training_lineage_manifest_latest.json",
        {
    ...
    _write_json(
        ...
        {
            "overall_status": "degraded",
            "packet_completeness_score": 40.0,
            "promotion_ready": False,
        },
    )
    ...
    payload = src.build_payload(tmp_path)
    lineage = next(
        row for row in payload["surfaces"] if row["surface"] == "training_lineage"
    )
...
def test_grade_regression_guard_keeps_snapshot_and_training_debt_advisory_for_guarded_paper(
    tmp_path: Path,
) -> None:
    health = tmp_path / "governance" / "health"
    champion = tmp_path / "governance" / "champion_challenger"
    _write_json(
        health / "training_quality_control_latest.json",
        {"overall_status": "ready", "training_quality_score": 90.0},
    )
    _write_json(
        health / "training_lineage_manifest_latest.json",
        {
            ...
            "hash_bundle_complete": True,
            "promotion_packet_seed_ready": True,
            "feature_store_lineage_ok": False,
    ...
    assert payload["overall_status"] == "ready"
    assert lineage["state"] == "ready"
    assert lineage["severity"] == "advisory"
    assert lineage["metrics"]["paper_soak_lineage_advisory"] is True
    assert lineage["metrics"]["live_promotion_gate_deferred"] is True
    assert lineage["metrics"]["snapshot_coverage_ok"] is False
    assert lineage["metrics"]["feature_store_lineage_ok"] is False
    assert lineage["metrics"]["training_confirmed"] is False
    assert "snapshot_coverage" in lineage["metrics"]["missing_contracts"]


def test_grade_regression_guard_accepts_zero_open_incidents_with_stale_blocked_status(
    tmp_path: Path,
) -> None:
    health = tmp_path / "governance" / "health"
    champion = tmp_path / "governance" / "champion_challenger"
    _write_json(
        health / "training_quality_control_latest.json",
        {"overall_status": "ready", "training_quality_score": 93.0},
    )
    _write_json(
        health / "training_lineage_manifest_latest.json",
        {
    ...
    _write_json(
        health / "incident_closeout_autopilot_latest.json",
        {"overall_status": "blocked", "open_incident_count": 0},
    )
    _write_json(
        health / "live_canary_control_latest.json",
        {
    ...
    payload = src.build_payload(tmp_path)
    incident = next(
        row for row in payload["surfaces"] if row["surface"] == "incident_closeout"
    )
...
def test_grade_regression_guard_respects_live_bounded_storage_recovery_even_with_high_pressure(
    tmp_path: Path,
) -> None:
    health = tmp_path / "governance" / "health"
    champion = tmp_path / "governance" / "champion_challenger"
    _write_json(
        health / "training_quality_control_latest.json",
        {"overall_status": "needs_attention", "training_quality_score": 80.62},
    )
    _write_json(
        health / "training_lineage_manifest_latest.json",
        {
    ...
    _write_json(
        health / "autonomy_control_plane_latest.json",
        {"overall_status": "degraded", "autonomy_score": 59.01},
    )
    _write_json(
        champion / "promotion_autopilot_packet_latest.json",
        {
    ...
    payload = src.build_payload(tmp_path)
    storage_row = next(
        row for row in payload["surfaces"] if row["surface"] == "storage_control"
    )
