...
def _active_pressure_fixture(project_root: Path) -> None:
    ...
    _write_json(
        health / "ingestion_storage_control_latest.json",
        {
            ...
            "storage": {
                "sqlite_wal_size_gb": 154.849,
                "backlog_drain_status": "blocked",
            },
    ...
    _write_json(
        health / "health_gates_latest.json",
        {
            ...
            "thresholds": {
                "sql_wal_size_gb_limit": 24.0,
                "ingestion_pending_lines_limit": 20000,
            },
            "inputs": {
...
def test_storage_pressure_clearance_refuses_to_fake_clear_active_pressure(
...
def test_storage_pressure_clearance_clears_only_stale_storage_gate(
    tmp_path: Path, monkeypatch
) -> None:
    project_root = tmp_path / "project"
    health = project_root / "governance" / "health"
    ...
    payload = clearance_src.build_payload(
        project_root, apply=True, force_clear_stale_gate=True
    )
...
def test_storage_pressure_clearance_accepts_blocked_preflight_after_verified_recovery(
    tmp_path: Path, monkeypatch
) -> None:
    project_root = tmp_path / "project"
    health = project_root / "governance" / "health"
    ...
    ingestion_refresh_count = 0

    def _write_recovered_storage() -> None:
        _write_json(
            health / "ingestion_storage_control_latest.json",
            {
                "overall_status": "ready",
                ...
                "backpressure": {
                    ...
                    "total_pending_lines": 1204,
                    "oldest_pending_age_seconds": 0.0,
                    "estimated_total_drain_minutes": 0.1,
                },
                "storage": {
                    "sqlite_wal_size_gb": 1.0,
    ...
    def _fake_run_json(cmd: list[str], *, cwd: Path, timeout_sec: int) -> dict:
        nonlocal ingestion_refresh_count
        joined = " ".join(cmd)
        rc = 0
        child_payload: dict = {"overall_status": "ready", "ok": True}
        if "ingestion_storage_control.py" in joined:
            ingestion_refresh_count += 1
            if ingestion_refresh_count == 1:
                rc = 2
                child_payload = {"overall_status": "blocked", "ok": False}
        elif "storage_backpressure_autopilot.py" in joined:
            _write_recovered_storage()
        elif "health_gates.py" in joined:
            _write_json(
                health / "health_gates_latest.json",
                {
                    "hard_gate_triggered": False,
                    ...
                    "inputs": {
                        "backpressure_overload_severe": False,
                        "backpressure_pending_lines": 1204,
                        "sql_wal_size_gb_live": 1.0,
                    },
                },
            )
    ...
    assert payload["overall_status"] == "ready"
    assert payload["metrics"]["attempt_error_count"] == 0
    assert payload["attempts"][0]["name"] == "refresh_storage_control"
    assert payload["attempts"][0]["rc"] == 2
    assert payload["attempts"][0]["status"] == "ok"


def test_storage_pressure_clearance_preflight_rc_two_requires_non_ready_health_payload() -> (
    None
):
    assert clearance_src._preflight_refresh_accepted_rcs(
        {
            "rc": 2,
            "timed_out": False,
    ...
    assert clearance_src._preflight_refresh_accepted_rcs(
        {"rc": 2, "timed_out": False, "payload": {}}
    ) == {0}
    assert clearance_src._preflight_refresh_accepted_rcs(
        {
            "rc": 2,
            "timed_out": False,
            "payload": {"overall_status": "ready", "ok": True},
        }
    ) == {0}
    assert clearance_src._preflight_refresh_accepted_rcs(
        {
...
def test_storage_pressure_clearance_keeps_bounded_draining_soft_target_paper_safe(
    tmp_path: Path,
) -> None:
    project_root = tmp_path / "project"
    health = project_root / "governance" / "health"
...
def test_storage_pressure_clearance_accepts_small_verified_queue_waiting_for_off_hours(
    tmp_path: Path,
) -> None:
    project_root = tmp_path / "project"
    health = project_root / "governance" / "health"
...
def test_storage_pressure_clearance_accepts_verified_intentional_local_hot_route(
    tmp_path: Path,
) -> None:
    project_root = tmp_path / "project"
    health = project_root / "governance" / "health"
    ...
    _write_json(
        health / "health_gates_latest.json", {"hard_gates": {}, "thresholds": {}}
    )
    _write_json(
...
def test_storage_pressure_clearance_still_blocks_at_hard_pressure_envelope(
    tmp_path: Path,
) -> None:
    project_root = tmp_path / "project"
    health = project_root / "governance" / "health"
