...
def test_decision_provenance_cards_falls_back_to_local_fallback_storage(tmp_path: Path) -> None:
    project_root = tmp_path / "project"
    first = project_root / "local_fallback_storage" / "decision_explanations" / "shadow_aggressive_equities" / "latest_decisions.log"
    first.parent.mkdir(parents=True, exist_ok=True)
    first.write_text(
        "[Decision] mode=shadow_aggressive_equities status=SHADOW_ONLY symbol=QQQ action=BUY score=0.7120 threshold=0.5200 reasons=score_above_threshold | trend_support | bot_id=brain_refinery_v55 bot_role=signal_sub_bot gates=market_data_ok=PASS safety=market_data_only=1,execution_enabled=0\n",
        encoding="utf-8",
    )
    ...
    assert payload["recent_cards"][0]["symbol"] == "QQQ"
