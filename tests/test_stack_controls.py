...
def test_opsctl_exposes_the_launcher_execution_breaker_recovery_command() -> None:
    opsctl = _read(OPSCTL_PATH)

    assert "one-numbers-refresh)" in opsctl
    assert (
        'exec "$PROJECT_ROOT/scripts/ops/run_one_numbers_refresh_launchd.sh" "$@"'
        in opsctl
    )
    assert "  one-numbers-refresh\n" in opsctl
...
LIVE_FEED_LAUNCHD_PATH = (
    PROJECT_ROOT / "scripts" / "ops" / "run_livefeed_local_launchd.sh"
)
LIVE_FEED_HEAVY_GUARDED_PATH = (
    PROJECT_ROOT / "scripts" / "ops" / "live_feed_heavy_guarded.sh"
)
...
OPS_AUTOMATION_INSTALL_PATH = (
    PROJECT_ROOT / "scripts" / "ops" / "install_ops_automation_launchd.sh"
)
STARTUP_PROMPT_INSTALL_PATH = (
    PROJECT_ROOT / "scripts" / "install_startup_start_prompt_launchd.sh"
)
STARTUP_PROMPT_RUN_PATH = (
    PROJECT_ROOT / "scripts" / "ops" / "run_startup_start_prompt_launchd.sh"
)
STARTUP_PROMPT_NOTIFIER_PATH = (
    PROJECT_ROOT / "scripts" / "ops" / "startup_prompt_notifier.swift"
)
PRODUCTION_HARDENING_WATCH_INSTALL_PATH = (
    PROJECT_ROOT / "scripts" / "install_production_hardening_watch_launchd.sh"
)
SOAK_SELF_HEAL_INSTALL_PATH = (
    PROJECT_ROOT / "scripts" / "install_soak_self_healing_launchd.sh"
)
SOAK_SELF_HEAL_RUN_PATH = (
    PROJECT_ROOT / "scripts" / "ops" / "run_soak_self_healing_launchd.sh"
)
STORAGE_BACKPRESSURE_AUTOPILOT_RUN_PATH = (
    PROJECT_ROOT / "scripts" / "ops" / "run_storage_backpressure_autopilot_launchd.sh"
)
RUNTIME_SMOOTH_MODE_RUN_PATH = (
    PROJECT_ROOT / "scripts" / "ops" / "run_runtime_smooth_mode_launchd.sh"
)
PRODUCTION_HARDENING_WATCH_RUN_PATH = (
    PROJECT_ROOT / "scripts" / "ops" / "run_production_hardening_watch_launchd.sh"
)
...
DAILY_AUTO_VERIFY_RUN_PATH = (
    PROJECT_ROOT / "scripts" / "ops" / "run_daily_auto_verify_launchd.sh"
)
...
def test_livefeed_labels_expired_empty_alert_history_without_stale_runtime_wording() -> (
    None
):
    text = _read(LIVE_FEED_TAIL_PATH)

    assert 'f"expired_empty_snapshot={as_bool(stale_idle_snapshot)} "' in text
    assert 'f"managed_stale={as_bool(stale_idle_snapshot)} "' not in text
...
def test_production_resilience_control_launchd_is_periodic_and_live_locked() -> None:
    text = _read(
        PROJECT_ROOT / "scripts" / "install_production_resilience_control_launchd.sh"
    )
...
def test_uniform_backlog_drain_process_is_late_loaded_and_exposed() -> None:
    ...
    assert loader.index(".env.backlog_pcore_accelerator_override") < loader.index(
        ".env.backlog_drain_uniform_override"
    )
    assert loader.index(".env.load_shape_smooth_override") < loader.index(
        ".env.backlog_drain_uniform_override"
    )
...
def test_run_all_sleeves_child_nice_targets_are_parent_relative() -> None:
    ...
    assert run_all_sleeves._nice_prefix_for_target(8, parent_nice=5) == [
        "nice",
        "-n",
        "3",
    ]
...
def test_run_all_sleeves_child_nice_does_not_try_to_lift_above_parent() -> None:
    ...
    assert run_all_sleeves._nice_prefix_for_target(4, parent_nice=5) == [
        "nice",
        "-n",
        "0",
    ]
...
def test_run_all_sleeves_paper_executor_nice_falls_back_to_baseline(
    monkeypatch,
) -> None:
...
def test_paper_trade_lock_disables_live_executor(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        run_all_sleeves, "_emit_incident_snapshot", lambda *_args, **_kwargs: None
    )
...
def test_run_all_sleeves_breaker_respects_data_quality_warmup() -> None:
    ...
    reasons, domain = run_all_sleeves._breaker_reasons(
        metrics, args, runtime_seconds=120.0
    )
...
def test_run_all_sleeves_breaker_enforces_data_quality_after_warmup() -> None:
    ...
    reasons, domain = run_all_sleeves._breaker_reasons(
        metrics, args, runtime_seconds=901.0
    )
...
def test_startup_breaker_releases_on_fresh_post_start_clear_evidence() -> None:
    now_epoch = datetime.fromisoformat("2026-09-03T17:20:00+00:00").timestamp()
    args = argparse.Namespace(
        broker="schwab",
        breaker_min_data_quality=75.0,
        breaker_max_blocked_rate=0.35,
    ...
    metrics = {
        ...
        "data_quality_session_local_timestamp": "2026-09-03T13:19:55-04:00",
        "data_quality_session_aware": "true",
        "data_quality_session_open": "true",
        "data_quality_score": "100.00",
        "combined_blocked_rate": "0.10",
        "stocks_pnl_proxy": "0.00",
    }

    evidence = run_all_sleeves._startup_breaker_evidence(
        metrics,
        args,
        launcher_started_at=now_epoch - 30.0,
        now_epoch=now_epoch,
    )
...
def test_startup_breaker_requires_post_start_measurement() -> None:
    now_epoch = datetime.fromisoformat("2026-09-03T17:20:00+00:00").timestamp()
    args = argparse.Namespace(
        broker="schwab",
        breaker_min_data_quality=75.0,
    ...
    metrics = {
        "_breaker_source_present": True,
        "_breaker_source_age_seconds": 90.0,
        "data_quality_session_local_timestamp": "2026-09-03T13:18:30-04:00",
        "data_quality_session_aware": "true",
        "data_quality_session_open": "true",
        "data_quality_score": "100.00",
        "combined_blocked_rate": "0.10",
    ...
    assert evidence["status"] == "startup_evidence_pending"
    assert evidence["reasons"] == ["launcher_startup_evidence_pending"]


def test_startup_breaker_enforces_quality_before_early_release() -> None:
    now_epoch = datetime.fromisoformat("2026-09-03T17:20:00+00:00").timestamp()
    args = argparse.Namespace(
        broker="schwab",
        breaker_min_data_quality=75.0,
    ...
    assert evidence["ready"] is False
    assert evidence["source_actionable"] is True
    assert evidence["status"] == "startup_evidence_blocked"
    assert evidence["reasons"] == ["data_quality_low:60.00"]
...
def test_run_all_sleeves_breaker_ignores_closed_schwab_session() -> None:
    ...
    actionable, reason = run_all_sleeves._breaker_metrics_actionable(metrics, args)
    reasons, _domain = run_all_sleeves._breaker_reasons(
        metrics, args, runtime_seconds=901.0
    )
...
def test_run_all_sleeves_breaker_parks_execution_without_parking_collectors() -> None:
    specs = {
        "baseline_parallel": run_all_sleeves.JobSpec(
            ...
            [],
            {},
            breaker_group=run_all_sleeves.COLLECTION_BREAKER_GROUP,
        ),
        ...
        "paper_executor": run_all_sleeves.JobSpec(
            ...
            [],
            {},
            breaker_group=run_all_sleeves.EXECUTION_BREAKER_GROUP,
        ),
    }
...
def test_run_all_sleeves_keeps_paper_lane_alive_for_paused_heartbeat(
    monkeypatch,
) -> None:
...
def test_run_all_sleeves_recycles_execution_lane_on_code_change(tmp_path) -> None:
    ...
    recycle, reason = run_all_sleeves._job_recycle_due(
        spec, started_at=100.0, now_ts=120.0
    )
...
def test_run_all_sleeves_debounces_execution_code_reload_until_files_settle(
    tmp_path,
) -> None:
    watched = tmp_path / "execution_contract.py"
    watched.write_text("# changed\n", encoding="utf-8")
    os.utime(watched, (110.0, 110.0))
    spec = run_all_sleeves.JobSpec(
        "paper_executor",
        [],
        {},
        ...
        code_watch_paths=(watched,),
        code_watch_settle_seconds=30,
    )

    settling, settling_reason = run_all_sleeves._job_recycle_due(
        spec, started_at=100.0, now_ts=120.0
    )
    settled, settled_reason = run_all_sleeves._job_recycle_due(
        spec, started_at=100.0, now_ts=141.0
    )

    assert settling is False
    assert settling_reason == ""
    assert settled is True
    assert settled_reason == "code_changed:execution_contract.py"


def test_execution_breaker_keeps_paper_consumer_resident() -> None:
    specs = {
        "paper_executor": run_all_sleeves.JobSpec(
            "paper_executor", [], {}, breaker_group="execution"
        ),
        "live_executor": run_all_sleeves.JobSpec(
            "live_executor", [], {}, breaker_group="execution"
        ),
    }
    ...
    assert parked == {"live_executor"}


def test_run_all_sleeves_keeps_workers_running_during_routine_auth_rotation(
    tmp_path,
) -> None:
    ...
    recycle, reason = run_all_sleeves._job_recycle_due(
        spec, started_at=1.0, now_ts=120.0
    )
...
def test_run_all_sleeves_recycles_execution_lane_on_max_runtime() -> None:
    ...
    recycle, reason = run_all_sleeves._job_recycle_due(
        spec, started_at=100.0, now_ts=161.0
    )
...
def test_run_all_sleeves_launcher_health_marks_degraded_children() -> None:
    ...
    specs = {
        "baseline_parallel": run_all_sleeves.JobSpec(
            "baseline_parallel", [], {}, breaker_group="core"
        ),
    ...
    assert "sleeve_launcher_parent_watchdog" in [
        row["name"] for row in payload["repair_infrabots"]
    ]
    states = {row["name"]: row["state"] for row in payload["jobs"]}
    assert states == {"baseline_parallel": "running", "bond": "exited", "fx": "missing"}


def test_run_all_sleeves_launcher_health_does_not_repair_not_yet_spawned_starting_jobs() -> (
    None
):
    class FakeProc:
        def __init__(self, pid: int, exit_code):
...
def test_run_all_sleeves_launcher_health_treats_fanout_parked_jobs_as_policy() -> None:
    ...
    specs = {
        ...
        "volatility": run_all_sleeves.JobSpec(
            "volatility", [], {}, breaker_group="core"
        ),
        "aggressive_modes": run_all_sleeves.JobSpec(
            "aggressive_modes", [], {}, breaker_group="core"
        ),
    }
    procs = {
        "baseline_parallel": FakeProc(101, None),
    ...
    payload = run_all_sleeves._launcher_health_payload(
        ...
        procs=procs,
        proc_started_at={
            "baseline_parallel": 50.0,
            "volatility": 60.0,
            "aggressive_modes": 70.0,
        },
        restart_history={},
        quarantined_jobs={},
        launcher_started_at=40.0,
    ...
    assert payload["repair_packet"]["problem_job_count"] == 0
    assert (
        payload["launcher_readiness_contract"]["readiness_status"]
        == "stable_with_parked_lanes"
    )
...
def test_run_all_sleeves_process_fanout_policy_parks_optional_sleeves(
    ...
    assert (
        run_all_sleeves._job_parked_by_fanout_policy("aggressive_modes", policy) is True
    )
    assert (
        run_all_sleeves._job_parked_by_fanout_policy("dividend_capture", policy) is True
    )
    assert (
        run_all_sleeves._job_parked_by_fanout_policy("baseline_parallel", policy)
        is False
    )
...
def test_run_all_sleeves_releases_requested_jobs_after_fanout_hold() -> None:
    specs = {
        "baseline_parallel": run_all_sleeves.JobSpec(
            "baseline_parallel", [], {}, breaker_group="collection"
        ),
        "dividend_capture": run_all_sleeves.JobSpec(
            "dividend_capture", [], {}, breaker_group="collection"
        ),
        "aggressive_modes": run_all_sleeves.JobSpec(
            "aggressive_modes", [], {}, breaker_group="collection"
        ),
    }
    active_policy = {
        "active": True,
...
def test_run_all_sleeves_cpu_pressure_guard_narrows_specialized_to_paper_allowlist(
    monkeypatch, tmp_path: Path
) -> None:
    monkeypatch.setenv(
        "RUN_ALL_SLEEVES_SPECIALIZED_PROFILE_ALLOWLIST",
        "volatility,pairs_correlation,unknown_profile",
    )
    ...
    assert policy["specialized_allowlist_profiles"] == [
        "volatility",
        "pairs_correlation",
    ]
    assert run_all_sleeves._specialized_profiles_for_launch() == (
        "volatility",
        "pairs_correlation",
    )
    assert run_all_sleeves._job_parked_by_fanout_policy("volatility", policy) is False
    assert (
        run_all_sleeves._job_parked_by_fanout_policy("earnings_event", policy) is True
    )
    assert changes == [
        "specialized_sleeves_allowlist_only",
        "aggressive_modes",
        "dividend_capture",
    ]
    assert args.with_specialized_sleeves is True
...
def test_run_all_sleeves_memory_pressure_guard_overrides_paper_allowlist(
    monkeypatch, tmp_path: Path
) -> None:
    monkeypatch.setenv(
        "RUN_ALL_SLEEVES_SPECIALIZED_PROFILE_ALLOWLIST", "volatility,pairs_correlation"
    )
    monkeypatch.setenv("PAPER_SOAK_SPECIALIZED_ALLOWLIST_BYPASS_FANOUT", "1")
    override = tmp_path / "override.env"
    override.write_text(
...
def test_run_all_sleeves_applies_process_fanout_policy_before_building_specs(
    monkeypatch,
) -> None:
    monkeypatch.delenv("RUN_ALL_SLEEVES_SPECIALIZED_PROFILE_ALLOWLIST", raising=False)
    monkeypatch.delenv("PAPER_SOAK_SPECIALIZED_ALLOWLIST_BYPASS_FANOUT", raising=False)
...
def test_run_all_sleeves_disk_gate_uses_external_runtime_storage(
...
def test_browser_quiet_override_keeps_soak_reports_from_spawning_headless_chrome() -> (
    None
):
...
def test_stop_start_lifecycle_restores_unattended_supervisors() -> None:
    ...
    assert (
        'recover_launchd_label "com.dankingsley.ops.sql_link_writer" 1' in start_stack
    )
    assert (
        'recover_launchd_label "com.dankingsley.reboot_resilience_guard" 1'
        in start_stack
    )
...
def test_soak_self_healing_always_renews_walkaway_certificate_before_heavy_gate() -> (
    None
):
    runner = _read(SOAK_SELF_HEAL_RUN_PATH)
    sentinel = 'soak_reliability_sentinel.py" \\'
    readiness = 'unattended_soak_readiness.py" \\'
    maintenance = 'run_guarded_maintenance.sh" soak_self_healing'

    assert sentinel in runner
    assert readiness in runner
    assert "--max-actions 3" in runner
    assert "sentinel=attention continuing_to_readiness_and_guarded_repair=1" in runner
    assert '--target-days "${SOAK_SELF_HEAL_TARGET_DAYS:-30}"' in runner
    assert runner.index(sentinel) < runner.index(readiness) < runner.index(maintenance)
    assert "lightweight_readiness_refresh=failed" in runner
...
def test_archive_automation_has_no_protected_volume_escape_hatch_or_default() -> None:
    ...
    assert "BOT_ALLOW_VIDEO_COLD_ARCHIVE=0" in _read(
...
def test_start_stack_does_not_treat_supervisor_launcher_arguments_as_live_workers() -> (
    None
):
    text = _read(PROJECT_ROOT / "scripts" / "ops" / "start_stack.sh")
...
def test_start_stack_pauses_watchdog_before_force_restart_drain() -> None:
    ...
    assert (
        text.index(force_restart)
        < text.index(pause_call)
        < text.index(orchestrator_branch)
    )
...
def test_force_restart_fence_precedes_supervisor_pause_and_releases_after_settlement() -> (
    None
):
    text = _read(PROJECT_ROOT / "scripts" / "ops" / "start_stack.sh")
...
def test_critical_watchdog_launchagents_pin_default_nice() -> None:
    ...
    watchdog_block = ops[
        ops.index('cat > "$WATCHDOG_PLIST"') : ops.index('cat > "$REPORT_PLIST"')
    ]
...
def test_start_stack_resumes_shadow_watchdog_after_all_sleeves_are_stable() -> None:
    ...
    assert (
        text.index(stable_marker)
        < text.rindex(resume_call)
        < text.rindex(support_restore)
    )
...
def test_force_restart_serializes_hot_standby_with_primary_supervisors() -> None:
    ...
    assert (
        text.index(pause_failover)
        < text.index(pause_process)
        < text.index(pause_shadow)
    )
...
def test_paper_trade_lock_is_present_on_stack_entrypoints() -> None:
    start_stack = _read(PROJECT_ROOT / "scripts" / "ops" / "start_stack.sh")
    all_sleeves_launchd = _read(
        PROJECT_ROOT / "scripts" / "ops" / "run_all_sleeves_launchd.sh"
    )
    shadow_watchdog_launchd = _read(
        PROJECT_ROOT / "scripts" / "ops" / "run_shadow_watchdog_launchd.sh"
    )
...
def test_livefeed_refresh_starts_fx_when_all_source_requested() -> None:
    ...
    assert (
        "livefeed-refresh|live-feed-refresh [paper default] [--dry-run] [--force-restart]"
        in text
    )
...
def test_opsctl_exposes_commands_hygiene() -> None:
    ...
    assert ".env.paper_400_ramp_override" in _read(
        PROJECT_ROOT / "scripts" / "ops" / "load_runtime_env.sh"
    )
    assert (
        "platform-intelligence|platform-intelligence-expansion|bot-admission|bot-lifecycle-manager"
        in text
    )
    assert "platform_intelligence_expansion.py" in text
    assert ".env.platform_intelligence_override" in _read(
        PROJECT_ROOT / "scripts" / "ops" / "load_runtime_env.sh"
    )
    assert "platform-brain-v4|grande-brain" in text
    assert "platform_brain_v4.py" in text
    assert ".env.platform_brain_v4_override" in _read(
        PROJECT_ROOT / "scripts" / "ops" / "load_runtime_env.sh"
    )
    assert "platform-brain-v5|reflex-brain" in text
    assert "platform_brain_v5.py" in text
    assert ".env.platform_brain_v5_override" in _read(
        PROJECT_ROOT / "scripts" / "ops" / "load_runtime_env.sh"
    )
    assert "platform-stabilization|quality-stabilizer" in text
    assert "platform_stabilization_quality.py" in text
    assert ".env.platform_stabilization_quality_override" in _read(
        PROJECT_ROOT / "scripts" / "ops" / "load_runtime_env.sh"
    )
    ...
    assert ".env.platform_settlement_stabilization_override" in _read(
        PROJECT_ROOT / "scripts" / "ops" / "load_runtime_env.sh"
    )
...
def test_opsctl_exposes_bot_fleet_production_posture() -> None:
    ...
    assert env_text.index(".env.sleeve_ingestion_production_override") < env_text.index(
        ".env.bot_fleet_production_posture_override"
    )
...
def test_opsctl_exposes_income_operating_platform() -> None:
    ...
    assert (
        "income-operating-platform|income-platform-control [--apply] [--json]" in text
    )
...
def test_official_macro_launchd_refreshes_underlying_fred_context() -> None:
    text = _read(
        PROJECT_ROOT / "scripts" / "ops" / "run_official_macro_context_launchd.sh"
    )
    installer = _read(
        PROJECT_ROOT / "scripts" / "ops" / "install_ops_automation_launchd.sh"
    )
...
def test_refresh_finder_logs_publishes_bot_logs_shortcut() -> None:
    ...
    assert (
        'mount_candidates="${BOT_LOGS_EXTERNAL_MOUNT_CANDIDATES:-$mount_root}"' in text
    )
    ...
    assert (
        '"bot_logs_alias_ok":$([[ "$BOT_LOGS_ALIAS_OK" == "1" ]] && echo true || echo false)'
        in text
    )
    assert (
        '"publish_bot_logs_desktop_shortcut":$([[ "$BOT_LOGS_FINDER_DESKTOP_SHORTCUTS" == "1" ]] && echo true || echo false)'
        in text
    )
...
def test_storage_failback_sync_republishes_finder_shortcuts() -> None:
    ...
    def has_dict_entry(key_name: str, expression: str) -> bool:
        expected = ast.dump(
        ...
        return any(
            ...
            and any(
                ...
                and ast.dump(value, include_attributes=False) == expected
    ...
    assert any(
        ...
        and node.name == "_sync_bot_logs_finder_shortcuts"
        for node in ast.walk(tree)
    )
    assert any(
        isinstance(node, ast.Constant) and node.value == "bot_logs_finder_sync.py"
        for node in ast.walk(tree)
    )
    assert has_dict_entry(
        "finder_sync",
        "_sync_bot_logs_finder_shortcuts(PROJECT_ROOT)",
    )
    assert has_dict_entry(
    ...
    assert has_dict_entry("match_reason", "str(resolution.match_reason)")
...
def test_runtime_env_and_storage_guard_support_mount_candidates() -> None:
    ...
    guard_runner = _read(
        PROJECT_ROOT / "scripts" / "ops" / "run_storage_eject_guard_launchd.sh"
    )
    guard_installer = _read(
        PROJECT_ROOT / "scripts" / "install_storage_eject_guard_launchd.sh"
    )
    ...
    assert (
        'ProcessInfo.processInfo.environment["BOT_LOGS_EXTERNAL_MOUNT_CANDIDATES"]'
        in guard_text
    )
    assert (
        'ProcessInfo.processInfo.environment["BOT_LOGS_EXTERNAL_VOLUME_NAME"]'
        in guard_text
    )
    assert (
    ...
    assert (
        "storage-transition-coordinator --transition-mode external --apply --json"
        in guard_text
    )
    ...
    assert "BOT_LOGS_DENY_UNSAFE_EJECT" in guard_text
    assert "DADissenterCreate" in guard_text
    assert "BOT_LOGS_FLAP_COOLDOWN_SECONDS" in guard_text
    assert "BOT_LOGS_SPOTLIGHT_DISABLE_TIMEOUT_SECONDS" in guard_text
    assert "external_available_flap_cooldown" in guard_text
    assert "currentMountBackoffSeconds" in guard_text
    assert "storage_eject_guard_events.jsonl" in guard_text
    assert ".metadata_never_index" in guard_text
    assert "/usr/bin/mdutil" in guard_text
    ...
    assert '"$GUARD_BINARY" -nt "$GUARD_SOURCE"' in guard_runner
    assert 'SWIFT_MODULE_CACHE_PATH="$SWIFT_CACHE_DIR"' in guard_runner
    assert '/usr/bin/swiftc -O "$GUARD_SOURCE" -o "$GUARD_BINARY_TMP"' in guard_runner
    ...
    assert "BOT_LOGS_FLAP_COOLDOWN_SECONDS" in guard_installer
    assert "BOT_LOGS_DENY_UNSAFE_EJECT" in guard_installer
    assert "BOT_LOGS_SPOTLIGHT_DISABLE_TIMEOUT_SECONDS" in guard_installer
...
def test_storage_disaster_recovery_command_and_launchd_are_wired() -> None:
    ...
    assert "storage-disaster-recovery --apply --json" in _read(
...
def test_paper_mirror_all_active_defaults_to_bounded_authority_cohorts() -> None:
    ...
    watchdog_nodes = list(ast.walk(ast.parse(process_watchdog)))
    for expression in (
        'env.setdefault("PAPER_MIRROR_ALL_ACTIVE_SUB_BOTS", "0")',
        'os.getenv("OPS_WATCHDOG_REQUIRE_COINBASE_FUTURES", "1")',
    ):
        expected = ast.dump(ast.parse(expression, mode="eval").body)
        assert any(
            isinstance(node, ast.Call) and ast.dump(node) == expected
            for node in watchdog_nodes
        )
    ...
    expected_live = ast.dump(ast.parse("bool(process_live)", mode="eval").body)
    assert any(
        isinstance(key, ast.Constant)
        and key.value == "process_live"
        and ast.dump(value) == expected_live
        for node in watchdog_nodes
...
def test_runtime_env_has_keychain_handoff_and_calm_support_defaults() -> None:
    ...
    assert (
        'OPS_SUPPORT_JOBS_BACKGROUND_POLICY="${OPS_SUPPORT_JOBS_BACKGROUND_POLICY:-1}"'
        in runtime_env
    )
    assert (
        'SUPPORT_MAINTENANCE_CONCURRENCY="${SUPPORT_MAINTENANCE_CONCURRENCY:-2}"'
        in runtime_env
    )
    assert runtime_env.index(".env.guard_intelligence_override") < runtime_env.index(
        ".env.process_fanout_guard_override"
    )
    assert ".env.load_shape_smooth_override" in runtime_env
    assert runtime_env.index(".env.accelerator_always_on_override") < runtime_env.index(
        ".env.load_shape_smooth_override"
    )


def test_live_runtime_resource_controller_overrides_stale_pressure_relief_state() -> (
    None
):
    runtime_env = _read(PROJECT_ROOT / "scripts" / "ops" / "load_runtime_env.sh")

    assert runtime_env.index(".env.pressure_relief_override") < runtime_env.index(
        ".env.runtime_resource_guard_override"
    )


def test_one_numbers_refreshes_paper_reconciliation_before_risk_boundary() -> None:
    script = _read(
        PROJECT_ROOT / "scripts" / "ops" / "run_one_numbers_refresh_launchd.sh"
    )

    assert script.index("paper_reconciliation_slo_guard.py") < script.index(
        "risk_service_boundary.py"
    )
    assert "auth_epoch_refresh_required" in script
    assert "ONE_NUMBERS_BREAKER_MAX_AGE_SECONDS" in script
    assert "MAINTENANCE_SLOT_ONE_NUMBERS_REFRESH_MIN_INTERVAL_SECONDS" in script


def test_load_shape_smooth_override_caps_backlog_pressure_without_disabling_drain() -> (
    None
):
    ...
    assert (
        "MAINTENANCE_SLOT_RUNTIME_ROOT=$PROJECT_ROOT/runtime/maintenance_slots"
        in override
    )
...
def test_shadow_watchdog_defaults_cover_fx_and_dividend_capture() -> None:
    ...
    assert (
        "default,conservative,aggressive,intraday_aggressive,swing_aggressive,dividend,bond,fx"
        in watchdog
    )
    assert (
        "default,conservative,aggressive,intraday_aggressive,swing_aggressive,dividend,bond,fx"
...
def test_livefeed_refresh_market_correlation_is_async_by_default() -> None:
    ...
    assert (
        ...
        in _read(
            ...
            / "scripts"
            / "ops"
            / "run_market_crypto_correlation_launchd.sh"
        )
    )
    assert "bounded_market_crypto_correlation_sync.py" in _read(
        PROJECT_ROOT / "scripts" / "run_shadow_training_loop.py"
    )
    assert '"--timeout-seconds",' in _read(
        PROJECT_ROOT / "scripts" / "run_shadow_training_loop.py"
    )
    assert (
...
def test_live_feed_tail_has_memory_aware_heavy_defaults() -> None:
    ...
    assert "LIVE_FEED_HEAVY_ASYNC_BOOTSTRAP" in text
    ...
    assert "LIVE_FEED_STATUS_ARTIFACT_MAX_AGE_SECONDS" in text
    ...
    assert '"$keepalive_count" -gt 0' in text
    assert 'detail_snapshot_state="loading"' in text
    assert "LIVE_FEED_BOOTSTRAP_PID" in text
    ...
    assert "[paper-plumbing]" in text
    assert "execution_plumbing_status" in text
    assert "contract_failures=" in text
    assert "consumer_failures=" in text
    ...
    assert (
        "weak_zero_entry={as_bool(runtime_enforcement.get('block_new_entries_on_weak_profiles'))}"
        in text
    )
    assert (
        "reduce_only_open={as_bool(runtime_enforcement.get('keep_sells_and_reduce_only_paths_open'))}"
        in text
    )
    ...
    assert (
        "(status-contract|system|collection|fx-provider|auth|schwab-auth|storage|throttle|soak|dashboard|paper|paper-plumbing|paper-data|paper-profit|profit-hardening|paper-truth|decision-latest|decision-route)"
        in text
    )
    ...
    assert "mark_historical_bootstrap_lines" in text
    assert "history non_authoritative=true" in text
    assert "live_feed_detail_snapshot=begin" in text
    assert "live_feed_detail_snapshot=ready" in text
    assert "detail_preserved=true" in text
    assert "artifact_freshness" in text
    assert "dashboard_source_status" in text
...
def test_retrain_entrypoints_stamp_trigger_source_and_logs() -> None:
    ...
    assert (
        'OVERNIGHT_TRAINING_WINDOW_TARGET="${OVERNIGHT_TRAINING_WINDOW_TARGET:-100}"'
        in weekly
    )
    assert "--window-target" in weekly


def test_retrain_entrypoints_refresh_generation_fill_learning() -> None:
    daily = _read(RETRAIN_DAILY_PATH)
    weekly = _read(RETRAIN_WEEKLY_PATH)

    for script in (daily, weekly):
        assert "GENERATION_FILL_LEARNING_ENABLED:-1" in script
        assert "generation-fill-learning --apply" in script
        assert "generation_fill_learning_rc=$?" in script
        assert "owner=current_candidate" in script
        assert script.index("generation-fill-learning --apply") < script.index(
            "export RETRAIN_ACTIVE_ONLY=1"
        )


def test_paper_standard_routes_master_through_consensus_only() -> None:
    shadow_loop = _read(PROJECT_ROOT / "scripts" / "run_shadow_training_loop.py")

    assert "def _master_execution_lane_allowed" in shadow_loop
    assert 'os.getenv("PAPER_LIVE_DATA_STANDARD_ENABLED", "0")' in shadow_loop
    assert (
        '"[ExecutionLanePolicy] hierarchical_master=orchestration_only "' in shadow_loop
    )
    assert '"executable_routes=registry_bound_bot,portfolio_consensus"' in shadow_loop
