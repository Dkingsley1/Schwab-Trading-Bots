...
def test_build_sqlite_skip_report_accepts_drained_larger_queue_standby(
    ...
    monkeypatch.setenv(
        "BOT_CHANNEL_QUEUE_DB", str(repo_data / "bot_channel_queue.sqlite3")
    )
    ...
    assert (
        queue_row["route_verification"]["state"]
...
def test_maybe_autoprune_external_low_space(monkeypatch, tmp_path):
    ...
    (external_data / "jsonl_link.sqlite3-wal.local_fallback").write_text(
        "x", encoding="utf-8"
    )
    (external_shards / "jsonl_link_trading.sqlite3").write_text("x", encoding="utf-8")
    (external_shards / "jsonl_link_trading.sqlite3.local_fallback").write_text(
        "x", encoding="utf-8"
    )
    ...
    payload = storage_failback_sync._maybe_autoprune_external_low_space(
        tmp_path, external_root
    )
...
def test_preserve_verified_local_route_intent_repairs_missing_override(
    monkeypatch, tmp_path
):
    project_root = tmp_path / "project"
    local_root = project_root / "local_fallback_storage"
...
def test_preserve_verified_local_route_intent_yields_to_explicit_switch(
    monkeypatch, tmp_path
):
    project_root = tmp_path / "project"
    local_root = project_root / "local_fallback_storage"
...
def test_matching_maintenance_token_authorizes_storage_failback(monkeypatch):
    ...
    assert (
        storage_failback_sync._maintenance_hold_blocks_route_mutation(snapshot) is False
    )
...
def test_wrong_maintenance_token_keeps_storage_failback_blocked(monkeypatch):
    ...
    assert (
        storage_failback_sync._maintenance_hold_blocks_route_mutation(snapshot) is True
    )
...
def test_lock_busy_payload_preserves_last_completed_route(tmp_path):
    ...
    payload = storage_failback_sync._lock_busy_payload(
        lock_path, "pid=123 cmd=storage_failback_sync", out
    )
    ...
    assert (
        payload["refresh_deferred"]["skipped_reason"]
        == "lock_busy_preserved_last_completed_route"
    )


def test_build_sqlite_skip_report_classifies_active_queue_and_warm_standby(
    monkeypatch, tmp_path
):
    project_root = tmp_path / "project"
    local_root = project_root / "local_fallback_storage"
    ...
    (local_root / "data" / "jsonl_link.sqlite3").write_text(
        "local-primary", encoding="utf-8"
    )
    (external_root / "data" / "jsonl_link.sqlite3").write_text(
        "external-primary", encoding="utf-8"
    )
    (local_root / "data" / "bot_channel_queue.sqlite3").write_text(
        "queue-local", encoding="utf-8"
    )
    (external_root / "data" / "bot_channel_queue.sqlite3").write_text(
        "queue-external", encoding="utf-8"
    )
    (local_root / "data" / "snapshot_context.sqlite3").write_text(
        "snapshot-local", encoding="utf-8"
    )
    (external_root / "data" / "snapshot_context.sqlite3").write_text(
        "snapshot-external", encoding="utf-8"
    )

    monkeypatch.setenv("BOT_LOGS_LOCAL_FALLBACK_ROOT", str(local_root))
    monkeypatch.setenv(
        "BOT_CHANNEL_QUEUE_DB", str(local_root / "data" / "bot_channel_queue.sqlite3")
    )
    ...
    assert payload["queue_db_path"] == str(
        local_root / "data" / "bot_channel_queue.sqlite3"
    )
    by_rel = {row["relative_path"]: row for row in payload["entries"]}
    assert (
        by_rel["data/bot_channel_queue.sqlite3"]["classification"]
        == "active_local_queue"
    )
    assert (
        by_rel["data/jsonl_link.sqlite3"]["classification"] == "warm_standby_retained"
    )
    assert (
        by_rel["data/snapshot_context.sqlite3"]["classification"]
        == "warm_standby_retained"
    )
    assert (
        by_rel["data/jsonl_link.sqlite3"]["route_verification"]["state"] == "verified"
    )
...
def test_build_sqlite_skip_report_certifies_active_local_nested_links(
    monkeypatch, tmp_path
):
    project_root = tmp_path / "project"
    local_root = project_root / "local_fallback_storage"
    ...
    (local_data / "snapshot_context.sqlite3").write_text(
        "snapshot-local", encoding="utf-8"
    )
    (external_data / "bot_channel_queue.sqlite3").write_text(
        "queue-external", encoding="utf-8"
    )
    (repo_data / "jsonl_link.sqlite3").symlink_to(local_data / "jsonl_link.sqlite3")
    (repo_data / "snapshot_context.sqlite3").symlink_to(
        local_data / "snapshot_context.sqlite3"
    )
    (repo_data / "bot_channel_queue.sqlite3").symlink_to(
        external_data / "bot_channel_queue.sqlite3"
    )
    ...
    assert (
        by_rel["data/jsonl_link.sqlite3"]["route_verification"]["state"]
        == "active_local_ready"
    )
    assert (
        by_rel["data/snapshot_context.sqlite3"]["route_verification"]["state"]
        == "active_local_ready"
    )
    assert (
        by_rel["data/bot_channel_queue.sqlite3"]["route_verification"]["state"]
        == "verified"
    )
...
def test_build_sqlite_skip_report_names_verified_mixed_route_external_curated(
    ...
    (local_data / "jsonl_link.sqlite3").write_text("jsonl", encoding="utf-8")
    (external_data / "bot_channel_queue.sqlite3").write_text("queue", encoding="utf-8")
    (external_data / "snapshot_context.sqlite3").write_text("context", encoding="utf-8")
    (repo_data / "jsonl_link.sqlite3").symlink_to(local_data / "jsonl_link.sqlite3")
    (repo_data / "bot_channel_queue.sqlite3").symlink_to(
        external_data / "bot_channel_queue.sqlite3"
    )
    (repo_data / "snapshot_context.sqlite3").symlink_to(
        external_data / "snapshot_context.sqlite3"
    )
    monkeypatch.setenv("BOT_LOGS_LOCAL_FALLBACK_ROOT", str(local_root))
    monkeypatch.setenv(
        "BOT_CHANNEL_QUEUE_DB", str(repo_data / "bot_channel_queue.sqlite3")
    ...
    payload = storage_failback_sync.build_sqlite_route_verification(
        project_root,
        external_root,
        mode="external",
        active_root=external_root,
    ...
    assert payload["certified_mode"] == "external_curated"
    assert payload["summary"]["active_local_count"] == 1
    assert payload["summary"]["active_external_count"] == 2
    assert payload["route_verification"]["verification_state"] == "ready"
    assert payload["route_verification"]["mismatches"] == []


def test_build_sqlite_skip_report_treats_smaller_standby_copy_as_curated(
    monkeypatch, tmp_path
):
    project_root = tmp_path / "project"
    local_root = project_root / "local_fallback_storage"
    ...
    (local_root / "data" / "jsonl_link.sqlite3").write_text(
        "local-primary-is-bigger-than-external", encoding="utf-8"
    )
    (external_root / "data" / "jsonl_link.sqlite3").write_text(
    ...
    (external_root / "data" / "bot_channel_queue.sqlite3").write_text(
        "queue-local-verified", encoding="utf-8"
    )
    (local_root / "data" / "snapshot_context.sqlite3").write_text(
        "snapshot-local", encoding="utf-8"
    )
...
def test_build_sqlite_skip_report_verifies_newer_active_external_smaller_than_standby(
    monkeypatch, tmp_path
):
    project_root = tmp_path / "project"
    local_root = project_root / "local_fallback_storage"
    ...
    local_jsonl.write_text(
        "local-standby-is-larger-than-current-external-route", encoding="utf-8"
    )
    external_jsonl.write_text("external-live", encoding="utf-8")
    (external_data / "bot_channel_queue.sqlite3").write_text(
        "queue-external", encoding="utf-8"
    )
    (external_data / "snapshot_context.sqlite3").write_text(
        "snapshot-external", encoding="utf-8"
    )
    ...
    by_rel = {row["relative_path"]: row for row in payload["entries"]}
    assert (
        by_rel["data/jsonl_link.sqlite3"]["classification"] == "active_external_route"
    )
    assert (
        by_rel["data/jsonl_link.sqlite3"]["route_verification"]["state"]
...
def test_build_sqlite_skip_report_treats_routed_queue_db_as_external(
    monkeypatch, tmp_path
):
    project_root = tmp_path / "project"
    local_root = project_root / "local_fallback_storage"
    ...
    (local_data / "jsonl_link.sqlite3").write_text("local-primary", encoding="utf-8")
    (external_data / "jsonl_link.sqlite3").write_text(
        "external-primary", encoding="utf-8"
    )
    (local_data / "bot_channel_queue.sqlite3").write_text(
        "queue-local", encoding="utf-8"
    )
    (external_data / "bot_channel_queue.sqlite3").write_text(
        "queue-external", encoding="utf-8"
...
def test_build_sqlite_skip_report_verifies_repo_passthrough_queue_db(
    monkeypatch, tmp_path
):
    project_root = tmp_path / "project"
    local_root = project_root / "local_fallback_storage"
    ...
    (repo_data / "bot_channel_queue.sqlite3").write_text(
        "active-queue", encoding="utf-8"
    )
    (local_data / "snapshot_context.sqlite3").write_text(
        "snapshot-local", encoding="utf-8"
    )
...
def test_refresh_frozen_sqlite_skip_report_updates_stale_queue_metadata(
    ...
    refreshed = storage_failback_sync._refresh_frozen_sqlite_skip_report(
        payload, external_root
    )

    by_rel = {
        row["relative_path"]: row for row in refreshed["sqlite_skip_report"]["entries"]
    }
    assert by_rel["data/bot_channel_queue.sqlite3"]["external"]["size_bytes"] == len(
        "small-valid-queue"
    )
    assert (
        by_rel["data/bot_channel_queue.sqlite3"]["route_verification"]["state"]
...
def test_support_freeze_bypass_reason_requires_real_routing_when_previous_not_external(
    monkeypatch, tmp_path
):
    ...
    reason = storage_failback_sync._support_freeze_bypass_reason(
        previous_path, tmp_path / "external"
    )

    assert reason == "previous_route_not_external:local_fallback_split_brain"


def test_support_freeze_bypass_reason_honors_explicit_local_route(
    monkeypatch, tmp_path
):
    previous_path = tmp_path / "storage_failback_sync_latest.json"
    previous_path.write_text(
...
def test_support_freeze_bypass_reason_allows_freeze_when_external_is_healthy(
    monkeypatch, tmp_path
):
    previous_path = tmp_path / "storage_failback_sync_latest.json"
    previous_path.write_text(
