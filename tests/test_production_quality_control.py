            ...
            "regression_guards": [
                {
                    ...
                    "actual": {
                        ...
                        "expansion_only_blockers": ["memory_pressure_above_paper_400_gate"],
                        "paper_policy": {
...
def test_production_quality_control_breaks_circular_soak_locks_when_operable(
    tmp_path: Path,
) -> None:
    project_root = tmp_path / "project"
    health = project_root / "governance" / "health"
            ...
            "gates": [
                ...
                },
                {
                    ...
                    "evidence": {
                        "paper_truth_status": "ready",
                        "runtime_paper_status": "ready",
                        "paper_ramp_blockers": ["ingestion_or_backpressure_above_paper_400_gate"],
                    },
                },
                {
                    "gate_id": "storage_pressure_clean",
    ...
    _write_json(
        health / "unattended_soak_readiness_latest.json",
        {
            ...
            "sections": {"storage": {"ready": False, "grade": "B", "ingestion_soak_ready": False}},
        },
    )
    _write_json(
        health / "paper_profitability_control_latest.json",
            ...
            "regression_guards": [
                {
                    ...
                    "actual": {
                        "paper_open": True,
                        "fail_closed_blockers": [],
                        "expansion_only_blockers": ["ingestion_or_backpressure_above_paper_400_gate"],
                        "paper_policy": {
                            "paper_execution_allowed": True,
                            "pause_paper_execution": False,
                        },
    ...
    _write_json(
        health / "ingestion_storage_control_latest.json",
        {
            "overall_status": "ready",
            "severity": "stable",
            "pressure_index": 0.362,
            "blockers": [],
            "storage_efficiency_contract": {"overall_status": "ready", "grade": "A+"},
            "backpressure": {
                "core_pending_lines": 4184,
                "total_pending_lines": 5194,
            },
        },
    )
    ...
    storage_lock = next(
        row["managed_lock"]
        for row in payload["managed_live_money_lock_lanes"]
        if row["lane_id"] == "storage_pressure_clean"
    )
    assert storage_lock["total_pending_lines"] == 5194
    assert storage_lock["ingestion_storage_operable"] is True
