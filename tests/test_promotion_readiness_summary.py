...
def test_promotion_readiness_summary_uses_effective_considered_floor(tmp_path: Path, monkeypatch, capsys) -> None:
    gate_path = tmp_path / "promotion_gate_latest.json"
    walk_forward_path = tmp_path / "walk_forward_latest.json"
    history_path = tmp_path / "promotion_readiness_history.jsonl"
    latest_path = tmp_path / "promotion_readiness_latest.json"
    ...
    assert rc == 0
    assert payload["coverage_shortfall_bots"] == 0
    assert payload["thresholds"]["min_considered_bots"] == 2
