...
def test_runtime_artifact_refresh_parses_pretty_printed_json_output() -> None:
    ...
    assert (
        runtime_artifact_refresh._parse_json_output(json.dumps(payload, indent=2))
        == payload
    )
...
def test_runtime_artifact_refresh_marks_child_maintenance_context(
    ...
    def fake_run(cmd, *, cwd, timeout_seconds, env):
        captured.update(
            {"cmd": cmd, "cwd": cwd, "timeout_seconds": timeout_seconds, "env": env}
        )
        return {
            "rc": 0,
...
def test_profitability_scope_holds_generation_lock_for_the_whole_epoch(
    ...
    monkeypatch.delenv(
        runtime_artifact_refresh.PAPER_PROFITABILITY_LOCK_ENV, raising=False
    )
    monkeypatch.setattr(
        runtime_artifact_refresh, "paper_profitability_generation_lock", fake_lock
    )
    ...
    assert (
        payload["single_writer_epoch_lock"][
            "prevents_interleaved_mutable_latest_publication"
        ]
        is True
    )
...
def test_runtime_artifact_refresh_reports_recovered_and_blocked_outputs(
    ...
    def runner(spec: dict, project_root: Path) -> dict:
        ...
        if name == "blocked_artifact":
            ...
            return {
                "cmd": list(spec["cmd"]),
                "rc": 2,
                "payload": {"ok": False, "overall_status": "blocked"},
                "stdout_tail": "",
                "stderr_tail": "",
                "duration_ms": 1.0,
            }
    ...
    payload = runtime_artifact_refresh.build_payload(
        tmp_path, specs=specs, runner=runner
    )
...
def test_runtime_artifact_refresh_is_degraded_when_outputs_exist_but_one_is_blocked(
    tmp_path: Path,
) -> None:
    health = tmp_path / "governance" / "health"
    ready_path = health / "ready_latest.json"
...
def test_runtime_artifact_refresh_counts_only_terminal_verifier_for_duplicate_artifact(
    tmp_path: Path,
) -> None:
    artifact_path = tmp_path / "governance" / "health" / "fixed_point_latest.json"
    specs = [
        {
            "name": "fixed_point_precheck",
            "payload_path": artifact_path,
            "cmd": ["precheck"],
        },
        {
            "name": "fixed_point_verified",
            "payload_path": artifact_path,
            "cmd": ["verified"],
        },
    ]
...
def test_runtime_artifact_refresh_retries_until_artifact_is_current_cycle_fresh(
    tmp_path: Path,
) -> None:
    ...
    specs = [
        {"name": "retry_artifact", "payload_path": artifact_path, "cmd": ["retry"]}
    ]
    calls = 0
...
def test_runtime_artifact_refresh_blocks_required_artifact_that_remains_stale(
    tmp_path: Path,
) -> None:
    ...
    os.utime(artifact_path, (1_600_000_000, 1_600_000_000))
    specs = [
        {"name": "stale_artifact", "payload_path": artifact_path, "cmd": ["stale"]}
    ]
    calls = 0
    ...
    assert (
        json.loads(artifact_path.read_text(encoding="utf-8"))["stale_source_rejected"]
        is True
    )


def test_runtime_artifact_refresh_publishes_current_stdout_when_producer_does_not_write(
    tmp_path: Path,
) -> None:
    ...
    os.utime(artifact_path, (1_600_000_000, 1_600_000_000))
    specs = [
        {"name": "stdout_artifact", "payload_path": artifact_path, "cmd": ["stdout"]}
    ]
...
def test_runtime_artifact_refresh_blocks_consumers_when_current_epoch_dependency_did_not_publish(
    ...
    assert downstream["operating_contract"]["complete"] is True
    assert downstream["operating_contract"]["owner"] == "downstream"
    assert downstream["operating_contract"]["why"] == "dependency_epoch_rejected"
    assert downstream["operating_contract"]["evidence_missing"] == ["upstream"]
    assert (
        "downstream_runtime_decision_from_rejected_epoch"
        in downstream["operating_contract"]["blocked_authority"]
    )


def test_runtime_artifact_refresh_training_scope_is_dependency_closed(
    tmp_path: Path,
) -> None:
...
def test_runtime_artifact_refresh_cell_health_scope_refreshes_every_cell_input(
    ...
    assert all(
        set(str(item) for item in row.get("depends_on", [])) <= names
        for row in selected
    )
    ...
    for terminal_name in (
        ...
        assert (
            "health_fast_cell_reconciled" in steps_by_name[terminal_name]["depends_on"]
        )
    ...
    assert ordered.index(
        "cell_sleeve_ticker_universe_pre_intelligence"
    ) < ordered.index("cell_whole_system_intelligence")
    assert ordered.index("cell_core_materialization_pre_intelligence") < ordered.index(
        "cell_federation_intelligence_pre"
    )
    assert ordered.index(
        "cell_backpressure_super_drainer_pre_intelligence"
    ) < ordered.index("cell_federation_intelligence_pre")
    assert ordered.index(
        "cell_backpressure_super_drainer_pre_intelligence"
    ) < ordered.index("cell_data_plane_recovery_pre_intelligence")
    assert ordered.index("cell_data_plane_recovery_pre_intelligence") < ordered.index(
        "cell_federation_intelligence_pre"
    )
    assert ordered.index("cell_federation_intelligence_pre") < ordered.index(
        "cell_whole_system_intelligence"
    )
    assert ordered.index("data_collection_observation_rollup_terminal") < ordered.index(
        "health_fast_terminal"
    )
    assert ordered.index("health_fast_terminal") < ordered.index(
        "runtime_gate_dashboard_cell_convergence"
    )
    assert ordered.index("runtime_gate_dashboard_cell_convergence") < ordered.index(
        "cell_infrabot_library_self_awareness_convergence"
    )
    assert ordered.index(
        "cell_infrabot_library_self_awareness_convergence"
    ) < ordered.index("distributed_cell_architecture_convergence_1")
    assert ordered.index("distributed_cell_architecture_convergence_1") < ordered.index(
        "cell_federation_intelligence_convergence"
    )
    assert ordered.index("cell_federation_intelligence_convergence") < ordered.index(
        "adaptive_regression_guard_cell_convergence"
    )
    assert ordered.index("stateful_storage_regression_guard_cell_pre") < ordered.index(
        "adaptive_regression_guard_cell_convergence"
    )
    assert ordered.index("livefeed_refresh_guard_cell_pre") < ordered.index(
        "adaptive_regression_guard_cell_convergence"
    )
    assert ordered.index("backlog_pcore_accelerator_cell_pre") < ordered.index(
        "backpressure_drainer_fleet_cell_pre"
    )
    assert ordered.index("backpressure_drainer_fleet_cell_pre") < ordered.index(
        "adaptive_regression_guard_cell_convergence"
    )
    assert ordered.index("adaptive_regression_guard_cell_convergence") < ordered.index(
        "system_architecture_hardening_cell_convergence"
    )
    assert ordered.index(
        "system_architecture_hardening_cell_convergence"
    ) < ordered.index("health_fast_cell_reconciled")
    assert ordered.index("health_fast_cell_reconciled") < ordered.index(
        "backlog_organizer_cell_verified"
    )
    assert ordered.index("backlog_organizer_cell_verified") < ordered.index(
        "system_architecture_contract_graph_cell_convergence"
    )
    assert ordered.index(
        "system_architecture_contract_graph_cell_convergence"
    ) < ordered.index("system_drift_guard_cell_probe")
    assert ordered.index("system_drift_guard_cell_probe") < ordered.index(
        "system_architecture_contract_graph_cell_reconciled"
    )
    assert ordered.index(
        "system_architecture_contract_graph_cell_reconciled"
    ) < ordered.index("system_architecture_autopilot_cell_convergence")
    assert ordered.index(
        "system_architecture_autopilot_cell_convergence"
    ) < ordered.index("system_drift_guard_cell_convergence")
    assert ordered.index("system_drift_guard_cell_convergence") < ordered.index(
        "system_architecture_contract_graph_cell_verified"
    )
    assert ordered.index("system_drift_guard_cell_convergence") < ordered.index(
        "master_infrastructure_supervisor_cell_convergence"
    ...
    assert ordered.index(
        "master_infrastructure_supervisor_cell_convergence"
    ) < ordered.index("system_drift_guard_cell_final")
    assert ordered.index("system_drift_guard_cell_final") < ordered.index(
        "system_architecture_contract_graph_cell_final"
    )
    assert ordered.index(
        "master_infrastructure_supervisor_cell_convergence"
    ) < ordered.index("cell_whole_system_intelligence_convergence")
    assert ordered.index("cell_whole_system_intelligence_convergence") < ordered.index(
        "distributed_cell_architecture_convergence_2"
    )
    assert ordered.index("distributed_cell_architecture_convergence_2") < ordered.index(
        "cell_federation_intelligence_terminal"
    )
    assert ordered.index("cell_federation_intelligence_terminal") < ordered.index(
        "cell_data_plane_recovery_terminal"
    )
    assert ordered.index("cell_data_plane_recovery_terminal") < ordered.index(
        "cell_whole_system_intelligence_terminal"
    )
    assert ordered.index("cell_whole_system_intelligence_terminal") < ordered.index(
        "grade_regression_guard_terminal"
    )
    assert ordered.index("distributed_cell_architecture_convergence_2") < ordered.index(
        "low_grade_finalizer_verified"
...
def test_runtime_artifact_refresh_manages_live_production_readiness_during_green_paper_soak() -> (
    None
):
    status = runtime_artifact_refresh._step_status(
...
def test_runtime_artifact_refresh_manages_alpha_evidence_collection_during_green_paper_soak() -> (
    None
):
    status = runtime_artifact_refresh._step_status(
        {
...
def test_runtime_artifact_refresh_profitability_scope_includes_every_epoch_input(
    tmp_path: Path,
) -> None:
    selected = runtime_artifact_refresh._select_scope_specs(
        runtime_artifact_refresh._step_specs(tmp_path),
    ...
    firewall = next(
        row for row in selected if row["name"] == "profitability_evidence_firewall"
    )
    assert set(firewall["depends_on"]) <= names
    assessment = next(
        row for row in selected if row["name"] == "profitability_self_assessment"
    )
    ...
    freshness = next(
        row for row in selected if row["name"] == "artifact_freshness_slo_post_master"
    )
    assert "control_surface_ownership" in freshness["depends_on"]
    assert "system_role_contract" in freshness["depends_on"]


def test_runtime_artifact_refresh_requires_secondary_outputs_from_same_producer_to_be_fresh(
    tmp_path: Path,
) -> None:
...
def test_runtime_artifact_refresh_accepts_fresh_markdown_secondary_output(
    tmp_path: Path,
) -> None:
    primary_path = tmp_path / "governance" / "health" / "primary_latest.json"
...
def test_runtime_artifact_refresh_preserves_markdown_format_for_failure_envelope(
    tmp_path: Path,
) -> None:
    ...
    written_path = runtime_artifact_refresh._write_refresh_failure_envelope(
        markdown_path, envelope
    )
...
def test_runtime_artifact_refresh_treats_managed_production_locks_as_ready(
    tmp_path: Path,
) -> None:
    health = tmp_path / "governance" / "health"
    ...
    specs = [
        {
            ...
            "payload_path": health / "live_money_readiness_contract_latest.json",
            "cmd": ["live-money"],
        },
        {
            "name": "promotion_packet_builder",
            "payload_path": champion / "promotion_packet_latest.json",
            "cmd": ["packet"],
        },
        {
            "name": "retrain_schema_compatibility",
            "payload_path": health / "retrain_schema_compatibility_latest.json",
            "cmd": ["schema"],
        },
    ]
...
def test_runtime_artifact_refresh_treats_protective_profitability_control_as_ready(
    tmp_path: Path,
) -> None:
    health = tmp_path / "governance" / "health"
    specs = [
        {
            ...
            "payload_path": health / "paper_profitability_control_latest.json",
            "cmd": ["paper-profit"],
        },
    ]
...
def test_runtime_artifact_refresh_tracks_paper_soak_proof_debt_as_managed(
    tmp_path: Path,
) -> None:
    health = tmp_path / "governance" / "health"
    _write_json(
    ...
    specs = [
        {
            ...
            "payload_path": health / "training_quality_control_latest.json",
            "cmd": ["training"],
        },
        {
            "name": "paper_execution_truth",
            "payload_path": health / "paper_execution_truth_layer_latest.json",
            "cmd": ["truth"],
        },
        {
            "name": "promotion_packet_builder",
        ...
        },
        {
            ...
            "payload_path": health / "canary_rollout_latest.json",
            "cmd": ["canary"],
            "optional": True,
        },
    ]
    ...
    def runner(spec: dict, project_root: Path) -> dict:
        ...
        payload = {
            "ok": False,
            "overall_status": "blocked",
            "failed_checks": ["future_live_money_proof"],
        }
        _write_json(Path(spec["payload_path"]), payload)
        return {
            "cmd": list(spec["cmd"]),
...
def test_runtime_artifact_refresh_manages_verified_suffix_for_paper_evidence_debt(
    tmp_path: Path,
) -> None:
    health = tmp_path / "governance" / "health"
    _write_json(
...
def test_runtime_artifact_refresh_manages_intentional_live_lock_halt_during_paper_soak(
    tmp_path: Path,
) -> None:
    health = tmp_path / "governance" / "health"
    _write_json(
    ...
    specs = [
        {
            ...
            "payload_path": health / "halt_trigger_control_plane_latest.json",
            "cmd": ["halt"],
        },
        {
            "name": "coordination_state_control_terminal",
            "payload_path": health / "coordination_state_latest.json",
            "cmd": ["coordination"],
        },
    ]
...
def test_runtime_artifact_refresh_rechecks_soak_after_refreshing_core_artifacts(
    tmp_path: Path,
) -> None:
    health = tmp_path / "governance" / "health"
    specs = [
        ...
        },
        {
            ...
            "payload_path": health / "unattended_soak_readiness_latest.json",
            "cmd": ["soak"],
        },
        {
            "name": "runtime_paper_regression_guard",
            "payload_path": health / "runtime_paper_regression_guard_latest.json",
            "cmd": ["paper"],
        },
    ]
    ...
    assert payload["managed_paper_soak_step_count"] == 2
    assert {row["name"]: row["status"] for row in payload["steps"]}[
...
def test_runtime_artifact_refresh_manages_stateful_sql_soft_quota_during_green_soak(
    tmp_path: Path,
) -> None:
    health = tmp_path / "governance" / "health"
    _write_json(
    ...
    _write_json(
        health / "storage_retention_unison_latest.json",
        {
            ...
            "continuous_run_contract": {
                "ready": True,
                "storage_controls": {"quota_ready": True},
            },
            "storage_growth_forecast": {
                "status": "stable_or_improving",
                "days_until_pressure_free": 45,
            },
    ...
    specs = [
        {
            ...
            "payload_path": health / "storage_quota_guard_latest.json",
            "cmd": ["quota"],
        },
    ]
...
def test_runtime_artifact_refresh_step_specs_include_training_storage_and_hardening_contracts(
    tmp_path: Path,
) -> None:
    ...
    assert names.index("tradingeconomics_route_context_refresh") < names.index(
        "collector_contracts"
    )
    assert names.index("options_flow_route_context_refresh") < names.index(
        "collector_contracts"
    )
    assert (
        next(
            row
            for row in specs
            if row["name"] == "tradingeconomics_route_context_refresh"
        )["optional"]
        is True
    )
    assert (
    ...
    assert names.index("source_verification_verified") < names.index(
        "capability_materialization"
    )
    assert names.index("capability_materialization") < names.index(
    ...
    assert "paper_live_data_standard" in names
    assert "trading_behavior_drill_program" in names
    assert "paper_profitability_control" in names
    assert names.index("paper_performance") < names.index(
        "trading_behavior_drill_program"
    )
    assert names.index("trading_behavior_drill_program") < names.index(
        "paper_profitability_control"
    )
    paper_profitability = next(
        row for row in specs if row["name"] == "paper_profitability_control"
    )
    assert set(paper_profitability["depends_on"]) == {
        "paper_performance",
        "trading_behavior_drill_program",
    }
    ...
    assert "strategy_market_fit_infrabot_verified" in names
    assert "quantitative_challenger_verified" in names
    assert "trading_behavior_drill_program_verified" in names
    ...
    self_model_post_evidence = next(
        row
        for row in specs
        if row["name"] == "system_self_model_post_evidence_verified"
    )
    ...
    assert (
        ...
        in self_model_post_evidence["depends_on"]
    )
    assert (
    ...
    assert names.index("sleeve_strategy_specialization_verified") < names.index(
        "strategy_market_fit_infrabot_verified"
    )
    strategy_market_fit = next(
        row for row in specs if row["name"] == "strategy_market_fit_infrabot_verified"
    )
    assert set(strategy_market_fit["depends_on"]) == {
        "sleeve_strategy_specialization_verified",
        "alpha_generation_control",
        "alpha_concept_report_verified",
    }
    assert "--force" in strategy_market_fit["cmd"]
    assert names.index("paper_performance_verified") < names.index(
    ...
    assert names.index("trading_behavior_drill_program_verified") < names.index(
        "paper_profitability_control_verified"
    )
    paper_profitability_verified = next(
        row for row in specs if row["name"] == "paper_profitability_control_verified"
    )
    assert set(paper_profitability_verified["depends_on"]) == {
    ...
    assert "source_mutation_guard_verified" in names
    ...
    assert names.index("storage_quota_guard") < names.index(
        "state_snapshot_restore_drill"
    )
    assert names.index("state_snapshot_restore_drill") < names.index(
        "storage_resilience_control"
    )
    assert names.index("storage_quota_guard") < names.index("ingestion_storage_control")
    assert names.index("ingestion_storage_control") < names.index(
        "storage_pressure_clearance"
    )
    assert names.index("storage_pressure_clearance") < names.index(
        "unattended_soak_readiness"
    )
    assert names.index("storage_resilience_control") < names.index(
        "storage_retention_unison"
    )
    assert names.index("storage_retention_unison") < names.index(
        "storage_resilience_control_terminal"
    )
    assert names.index("storage_resilience_control_terminal") < names.index(
        "blackstart_recovery"
    )
    assert names.index("storage_retention_unison") < names.index(
        "unattended_soak_readiness"
    )
    assert names.index("content_addressed_artifact_store") < names.index(
        "storage_disaster_recovery_verified"
    )
    assert names.index("security_evidence_autofix_verified") < names.index(
        "security_audit_verified"
    )
    assert names.index("storage_disaster_recovery_verified") < names.index(
        "blackstart_recovery_verified"
    )
    assert names.index("security_audit_verified") < names.index(
        "production_readiness_control"
    )
    assert names.index("remote_alert_control_verified") < names.index(
        "production_readiness_control"
    )
    assert names.index("blackstart_recovery_verified") < names.index(
        "production_readiness_control"
    )
    assert names.index("telemetry_redaction_canary_verified") < names.index(
        "production_readiness_control"
    )
    assert names.index("storage_disaster_recovery_verified") < names.index(
        "production_readiness_control"
    ...
    assert names.index("incident_review_packet") < names.index(
    ...
    assert names.index("live_runtime_separation_control") < names.index(
        "live_canary_control"
    )
    assert names.index("live_canary_control") < names.index("live_readiness_smoke")
    assert names.index("blackstart_recovery") < names.index(
        "live_money_readiness_contract"
    )
    assert names.index("live_money_readiness_contract") < names.index(
        "incident_timeline"
    )
    assert names.index("incident_closeout_autopilot") < names.index(
        "unattended_soak_readiness"
    )
    assert names.index("incident_closeout_autopilot") < names.index(
        "ingestion_backpressure_final"
    )
    assert names.index("ingestion_backpressure_final") < names.index(
        "ingestion_storage_control_final"
    )
    assert names.index("ingestion_storage_control_final") < names.index(
        "ingestion_storage_governor_final"
    )
    assert names.index("ingestion_storage_governor_final") < names.index(
        "ingestion_storage_control_post_governor"
    )
    assert names.index("ingestion_storage_control_post_governor") < names.index(
        "ingestion_storage_governor_verify"
    )
    assert names.index("ingestion_storage_governor_verify") < names.index(
        "ingestion_storage_control_verified"
    )
    assert names.index("ingestion_storage_control_verified") < names.index(
        "health_fast"
    )
    ...
    assert names.index("service_control_plane") < names.index(
        "runtime_throttle_control_verified"
    )
    assert names.index("runtime_throttle_control_verified") < names.index(
        "paper_400_ramp_verified"
    )
    assert names.index("paper_400_ramp_verified") < names.index(
        "runtime_paper_regression_guard_verified"
    )
    assert names.index("runtime_paper_regression_guard_verified") < names.index(
        "halt_trigger_control_plane_verified"
    )
    assert names.index("halt_trigger_control_plane_verified") < names.index(
        "coordination_state_control_verified"
    )
    assert names.index("coordination_state_control_verified") < names.index(
        "health_fast_verified"
    )
    assert names.index("health_fast_verified") < names.index(
        "unattended_soak_readiness_verified"
    )
    assert names.index("unattended_soak_readiness_verified") < names.index(
        "replay_hash_registry_final"
    )
    assert names.index("replay_hash_registry_final") < names.index(
        "golden_replay_regression_final"
    )
    assert names.index("golden_replay_regression_final") < names.index(
        "paper_execution_truth_verified"
    )
    assert names.index("paper_execution_truth_verified") < names.index(
        "stateful_storage_regression_guard_verified"
    )
    assert names.index("stateful_storage_regression_guard_verified") < names.index(
        "one_numbers_regression_guard_verified"
    )
    assert names.index("one_numbers_regression_guard_verified") < names.index(
        "grade_regression_guard_verified"
    )
    assert names.index("grade_regression_guard_verified") < names.index(
        "section_grade_guard_verified"
    )
    assert names.index("section_grade_guard_verified") < names.index(
        "system_drift_registry_verified"
    )
    assert names.index("system_drift_registry_verified") < names.index(
        "infrastructure_autofix_verified"
    )
    assert names.index("infrastructure_autofix_verified") < names.index(
        "master_infrastructure_supervisor_verified"
    )
    assert names.index("master_infrastructure_supervisor_verified") < names.index(
        "livefeed_refresh_guard_verified"
    )
    assert names.index("livefeed_refresh_guard_verified") < names.index(
        "adaptive_regression_guard_final"
    )
    assert names.index("livefeed_refresh_guard_verified") < names.index(
        "runtime_throttle_control_final"
    )
    assert names.index("runtime_throttle_control_final") < names.index(
        "paper_400_ramp_final"
    )
    assert names.index("paper_400_ramp_final") < names.index(
        "runtime_paper_regression_guard_final"
    )
    assert names.index("runtime_paper_regression_guard_final") < names.index(
        "halt_trigger_control_plane_final"
    )
    assert names.index("halt_trigger_control_plane_final") < names.index(
        "coordination_state_control_final"
    )
    assert names.index("coordination_state_control_final") < names.index(
        "health_fast_final"
    )
    assert names.index("health_fast_final") < names.index(
        "adaptive_regression_guard_final"
    )
    assert names.index("adaptive_regression_guard_final") < names.index(
        "system_drift_guard_pre_architecture"
    )
    assert names.index("system_drift_guard_pre_architecture") < names.index(
        "schwab_indicator_intelligence_verified"
    )
    assert names.index("schwab_indicator_intelligence_verified") < names.index(
        "system_expansion_execution_verified"
    )
    assert names.index("system_expansion_execution_verified") < names.index(
        "distributed_cell_architecture_verified"
    )
    assert names.index("distributed_cell_architecture_verified") < names.index(
        "system_architecture_hardening_verified"
    )
    assert names.index("system_architecture_hardening_verified") < names.index(
        "system_self_model_pre_architecture"
    )
    assert names.index("system_self_model_pre_architecture") < names.index(
        "system_architecture_contract_graph_final"
    )
    assert names.index("system_architecture_contract_graph_final") < names.index(
        "system_architecture_autopilot_final"
    )
    assert names.index("system_architecture_autopilot_final") < names.index(
        "system_drift_guard_verified"
    )
    assert names.index("system_drift_guard_verified") < names.index(
        "system_drift_autopilot_verified"
    )
    assert names.index("system_drift_autopilot_verified") < names.index(
        "master_infrastructure_supervisor_final"
    )
    assert names.index("master_infrastructure_supervisor_final") < names.index(
        "system_self_model_verified"
    )
    assert names.index("system_self_model_verified") < names.index(
        "system_architecture_contract_graph_verified"
    )
    assert names.index("system_architecture_contract_graph_verified") < names.index(
        "system_architecture_autopilot_verified"
    )
    assert names.index("system_architecture_autopilot_verified") < names.index(
        "broker_readiness_terminal"
    )
    assert names.index("broker_readiness_terminal") < names.index(
        "auth_lease_manager_terminal"
    )
    assert names.index("auth_lease_manager_terminal") < names.index(
        "schwab_auth_supervisor_terminal"
    )
    assert names.index("schwab_auth_supervisor_terminal") < names.index(
        "backlog_pcore_accelerator_terminal"
    )
    assert names.index("backlog_pcore_accelerator_terminal") < names.index(
        "backpressure_drainer_fleet_terminal"
    )
    assert names.index("backpressure_drainer_fleet_terminal") < names.index(
        "ingestion_storage_control_terminal"
    )
    assert names.index("ingestion_storage_control_terminal") < names.index(
        "runtime_throttle_control_terminal"
    )
    assert names.index("runtime_throttle_control_terminal") < names.index(
        "paper_400_ramp_terminal"
    )
    assert names.index("paper_400_ramp_terminal") < names.index(
        "runtime_paper_regression_guard_terminal"
    )
    assert names.index("runtime_paper_regression_guard_terminal") < names.index(
        "halt_trigger_control_plane_terminal"
    )
    assert names.index("halt_trigger_control_plane_terminal") < names.index(
        "coordination_state_control_terminal"
    )
    assert names.index("coordination_state_control_terminal") < names.index(
        "health_fast_terminal"
    )
    ...
    assert names.index("unattended_soak_readiness_terminal") < names.index(
        "one_numbers_regression_guard_terminal"
    )
    assert names.index("one_numbers_regression_guard_terminal") < names.index(
        "grade_regression_guard_terminal"
    )
    assert names.index("grade_regression_guard_terminal") < names.index(
        "section_grade_guard_terminal"
    )
    assert names.index("section_grade_guard_terminal") < names.index(
        "low_grade_finalizer_verified"
    )
    assert names.index("low_grade_finalizer_verified") < names.index(
        "livefeed_refresh_guard_terminal"
    )
    assert names.index("livefeed_refresh_guard_terminal") < names.index(
        "adaptive_regression_guard_terminal"
    )
    assert names.index("adaptive_regression_guard_terminal") < names.index(
        "system_self_model_convergence"
    )
    assert names.index("system_self_model_convergence") < names.index(
        "system_architecture_contract_graph_convergence"
    )
    assert names.index("system_architecture_contract_graph_convergence") < names.index(
        "system_architecture_autopilot_convergence"
    )
    assert names.index("system_architecture_autopilot_convergence") < names.index(
        "system_drift_guard_terminal"
    )
    assert names.index("system_drift_guard_terminal") < names.index(
        "system_drift_autopilot_terminal"
    )
    assert names.index("system_drift_autopilot_terminal") < names.index(
        "system_drift_guard_post_autopilot_terminal"
    )
    assert names.index("system_drift_guard_post_autopilot_terminal") < names.index(
        "master_infrastructure_supervisor_terminal"
    )
    assert names.index("master_infrastructure_supervisor_terminal") < names.index(
        "infrastructure_autofix_terminal"
    )
    assert names.index("infrastructure_autofix_terminal") < names.index(
        "system_self_model_final"
    )
    assert names.index("system_self_model_final") < names.index(
        "system_architecture_contract_graph_terminal"
    )
    assert names.index("system_architecture_contract_graph_terminal") < names.index(
        "system_architecture_autopilot_terminal"
    )
    assert names.index("system_architecture_autopilot_terminal") < names.index(
        "system_drift_guard_settled"
    )
    assert names.index("system_drift_guard_settled") < names.index(
        "system_drift_autopilot_settled"
    )
    assert names.index("system_drift_autopilot_settled") < names.index(
        "system_drift_guard_post_settled"
    )
    assert names.index("system_drift_guard_post_settled") < names.index(
        "master_infrastructure_supervisor_settled"
    )
    assert names.index("master_infrastructure_supervisor_settled") < names.index(
        "infrastructure_autofix_settled"
    )
    assert names.index("infrastructure_autofix_settled") < names.index(
        "system_self_model_settled"
    )
    assert names.index("system_self_model_settled") < names.index(
        "system_architecture_contract_graph_settled"
    )
    assert names.index("system_architecture_contract_graph_settled") < names.index(
        "system_architecture_autopilot_settled"
    )
    assert names.index("system_architecture_autopilot_settled") < names.index(
        "system_drift_guard_final"
    )
    assert names.index("system_drift_guard_final") < names.index(
        "master_infrastructure_supervisor_final_settled"
    )
    assert names.index("master_infrastructure_supervisor_final_settled") < names.index(
        "broker_readiness_post_settlement"
    )
    assert names.index("broker_readiness_post_settlement") < names.index(
        "auth_lease_manager_post_settlement"
    )
    assert names.index("auth_lease_manager_post_settlement") < names.index(
        "schwab_auth_supervisor_post_settlement"
    )
    assert names.index("schwab_auth_supervisor_post_settlement") < names.index(
        "backpressure_drainer_fleet_post_settlement"
    )
    assert names.index("backpressure_drainer_fleet_post_settlement") < names.index(
        "ingestion_storage_control_post_settlement"
    )
    assert names.index("ingestion_storage_control_post_settlement") < names.index(
        "runtime_throttle_control_post_settlement"
    )
    assert names.index("runtime_throttle_control_post_settlement") < names.index(
        "paper_400_ramp_post_settlement"
    )
    assert names.index("paper_400_ramp_post_settlement") < names.index(
        "runtime_throttle_control_post_settlement_verified"
    )
    assert names.index(
        "runtime_throttle_control_post_settlement_verified"
    ) < names.index("paper_400_ramp_post_settlement_verified")
    assert names.index("paper_400_ramp_post_settlement_verified") < names.index(
        "runtime_paper_regression_guard_post_settlement"
    )
    assert names.index("runtime_paper_regression_guard_post_settlement") < names.index(
        "halt_trigger_control_plane_post_settlement"
    )
    assert names.index("halt_trigger_control_plane_post_settlement") < names.index(
        "coordination_state_control_post_settlement"
    )
    assert names.index("coordination_state_control_post_settlement") < names.index(
        "health_fast_post_settlement"
    )
    assert names.index("health_fast_post_settlement") < names.index(
        "live_runtime_separation_post_settlement"
    )
    assert names.index("live_runtime_separation_post_settlement") < names.index(
        "incident_closeout_autopilot_post_settlement"
    )
    assert names.index("incident_closeout_autopilot_post_settlement") < names.index(
        "sleeve_isolation_guard_post_settlement"
    )
    assert names.index("sleeve_isolation_guard_post_settlement") < names.index(
        "section_grade_guard_post_settlement"
    )
    assert names.index("section_grade_guard_post_settlement") < names.index(
        "unattended_soak_readiness_post_settlement"
    )
    assert names.index("unattended_soak_readiness_post_settlement") < names.index(
        "livefeed_refresh_guard_post_settlement"
    )
    assert names.index("livefeed_refresh_guard_post_settlement") < names.index(
        "adaptive_regression_guard_post_settlement"
    )
    assert names.index("adaptive_regression_guard_post_settlement") < names.index(
        "system_drift_guard_post_evidence_probe"
    )
    assert names.index("system_drift_guard_post_evidence_probe") < names.index(
        "system_architecture_contract_graph_post_evidence_probe"
    )
    assert names.index(
        "system_architecture_contract_graph_post_evidence_probe"
    ) < names.index("system_drift_guard_post_evidence_reconciled")
    assert names.index("system_drift_guard_post_evidence_reconciled") < names.index(
        "system_architecture_contract_graph_post_evidence_verified"
    )
    assert names.index(
        "system_architecture_contract_graph_post_evidence_verified"
    ) < names.index("system_architecture_autopilot_post_evidence_verified")
    assert names.index(
        "system_architecture_autopilot_post_evidence_verified"
    ) < names.index("system_drift_guard_post_architecture_verified")
    assert names.index("system_drift_guard_post_architecture_verified") < names.index(
        "system_drift_autopilot_post_evidence_verified"
    )
    assert names.index("source_verification") < names.index(
        "source_verification_autorefresh"
    )
    assert names.index("source_verification_autorefresh") < names.index(
        "source_verification_verified"
    )
    assert names.index("multiple_testing_guard") < names.index(
        "profitability_evidence_firewall"
    )
    assert names.index("decay_monitor") < names.index("profitability_evidence_firewall")
    assert names.index("profitability_independent_validator") < names.index(
        "profitability_benchmark_hurdle"
    )
    assert names.index("profitability_benchmark_capture") < names.index(
        "profitability_benchmark_hurdle"
    )
    assert names.index("profitability_benchmark_hurdle") < names.index(
        "profitability_evidence_firewall"
    )
    assert names.index("profitability_evidence_firewall") < names.index(
        "production_excellence_control"
    )
    assert names.index("production_excellence_control") < names.index(
        "continuous_soak_integrity_control"
    )
    assert names.index("continuous_soak_integrity_control") < names.index(
        "live_transition_integrity_control"
    )
    assert names.index("live_transition_integrity_control") < names.index(
        "live_money_readiness_contract_verified"
    )
    assert names.index("live_money_readiness_contract_verified") < names.index(
        "runtime_gate_dashboard_pre_master"
    )
    assert names.index("system_drift_autopilot_post_evidence_verified") < names.index(
        "runtime_gate_dashboard_pre_master"
    )
    assert names.index("runtime_gate_dashboard_pre_master") < names.index(
        "master_infrastructure_supervisor_post_evidence_probe"
    )
    assert names.index(
        "master_infrastructure_supervisor_post_evidence_probe"
    ) < names.index("infrastructure_autofix_post_evidence_verified")
    assert names.index("infrastructure_autofix_post_evidence_verified") < names.index(
        "system_self_model_post_evidence_verified"
    )
    assert names.index("system_self_model_post_evidence_verified") < names.index(
        "master_infrastructure_supervisor_post_evidence_verified"
    )
    assert names.index(
        "master_infrastructure_supervisor_post_evidence_verified"
    ) < names.index("runtime_gate_dashboard_pre_operator")
    assert names.index("runtime_gate_dashboard_pre_operator") < names.index(
        "operator_cockpit"
    )
    assert names.index("system_architecture_contract_graph_verified") < names.index(
        "operator_cockpit"
    ...
    account_snapshot_spec = next(
        row for row in specs if row["name"] == "schwab_account_snapshot_refresh"
    )
    ...
    assert names.index("position_opportunity_watch") < names.index(
        "one_numbers_portfolio_prerequisite"
    )
    assert names.index("one_numbers_portfolio_prerequisite") < names.index(
        "sleeve_allocator"
    )
    assert names.index("sleeve_allocator") < names.index("portfolio_risk_ledger")
    assert names.index("portfolio_risk_ledger") < names.index(
        "portfolio_allocator_service"
    )
    assert names.index("portfolio_allocator_service") < names.index(
    ...
    paper_profitability_spec = next(
        row for row in specs if row["name"] == "paper_profitability_control"
    )
    assert "--apply" in paper_profitability_spec["cmd"]
    assert paper_profitability_spec["additional_payload_paths"]
    paper_performance_spec = next(
        row for row in specs if row["name"] == "paper_performance"
    )
    assert "--json-only" in paper_performance_spec["cmd"]
    source_refresh_spec = next(
        row for row in specs if row["name"] == "source_verification_autorefresh"
    )
    assert source_refresh_spec["optional"] is True
    assert (
        source_refresh_spec["cmd"][
            source_refresh_spec["cmd"].index("--max-commands") + 1
        ]
        == "1"
    )
    security_refresh_spec = next(
        row for row in specs if row["name"] == "security_evidence_autofix_verified"
    )
    assert "--force-secret-scan" in security_refresh_spec["cmd"]
    assert security_refresh_spec["additional_payload_paths"]
    readiness_spec = next(
        row for row in specs if row["name"] == "production_readiness_control"
    )
    ...
    indicator_spec = next(
        row for row in specs if row["name"] == "schwab_indicator_intelligence_verified"
    )
    assert "--offline" in indicator_spec["cmd"]
    runtime_throttle_spec = next(
        row for row in specs if row["name"] == "runtime_throttle_control"
    )
    assert "--apply" in runtime_throttle_spec["cmd"]
    ingestion_governor_spec = next(
        row for row in specs if row["name"] == "ingestion_storage_governor_final"
    )
    assert "apply" in ingestion_governor_spec["cmd"]
    ingestion_governor_verify_spec = next(
        row for row in specs if row["name"] == "ingestion_storage_governor_verify"
    )
