...
def test_storage_split_brain_reconciler_classifies_hash_match_ready(
    ...
    (external_root / "logs" / "state.json.local_fallback").write_text(
        "same", encoding="utf-8"
    )
    _write_json(
    ...
    assert payload["conflicts"][0]["classification"] in {
        "ready_to_prune_local",
        "duplicate_conflict_copy",
    }


def test_storage_split_brain_reconciler_fast_path_skips_full_tree_scan_when_manifest_is_clean(
    tmp_path: Path, monkeypatch
) -> None:
    project_root = tmp_path / "project"
    external_root = tmp_path / "external"
    ...
    monkeypatch.setattr(
        src,
        "_iter_conflict_files",
        lambda _external_root: (_ for _ in ()).throw(
            AssertionError("fast path should not scan")
        ),
    )
...
def test_storage_split_brain_reconciler_suppresses_stale_reported_conflicts_after_clean_scan(
    ...
    _write_json(
        project_root / "governance" / "health" / "storage_failback_sync_latest.json",
        {"split_brain_conflicts": 4},
    )
    _write_json(
        project_root / "governance" / "health" / "storage_mount_guard_latest.json",
        {"external_available": True, "storage_mode": "local_fallback_split_brain"},
    )
...
def test_storage_split_brain_reconciler_counts_router_conflicts_for_failback(
    tmp_path: Path, monkeypatch
) -> None:
    project_root = tmp_path / "project"
    external_root = tmp_path / "external"
...
def test_storage_split_brain_reconciler_repairs_log_router_conflicts(
    tmp_path: Path, monkeypatch
) -> None:
    project_root = tmp_path / "project"
    external_root = tmp_path / "external"
    ...
    external_gz = (
        external_root / "decision_explanations" / "paper" / "latest_decisions.log.gz"
    )
    local_gz = (
        local_root / "decision_explanations" / "paper" / "latest_decisions.log.gz"
    )
    ...
    result = src._repair_router_log_conflicts(
        project_root, archive_base=project_root / "archive", apply=True
    )
...
def test_storage_split_brain_reconciler_merges_decision_jsonl_with_dedupe(
    tmp_path: Path, monkeypatch
) -> None:
    project_root = tmp_path / "project"
    external_root = tmp_path / "external"
    local_root = project_root / "local_fallback_storage"
    external_decisions = (
        external_root / "decisions" / "paper" / "trade_decisions_20260630.jsonl"
    )
    local_decisions = (
        local_root / "decisions" / "paper" / "trade_decisions_20260630.jsonl"
    )
...
def test_storage_split_brain_reconciler_keeps_source_when_atomic_merge_fails(
    tmp_path: Path,
    monkeypatch,
) -> None:
    local_root = tmp_path / "local"
    external_root = tmp_path / "external"
    archive_root = external_root / "cold_archive"
    local_path = local_root / "logs" / "watchdog.log"
    external_path = external_root / "logs" / "watchdog.log"
    local_path.parent.mkdir(parents=True)
    external_path.parent.mkdir(parents=True)
    local_path.write_text("local-source\n", encoding="utf-8")
    external_path.write_text("external-source\n", encoding="utf-8")
    actions: list[dict] = []

    def _fail_merge(_local_path: Path, _external_path: Path) -> dict:
        raise OSError("simulated external write failure")

    monkeypatch.setattr(src, "_merge_text_file_transactionally", _fail_merge)

    repaired = src._append_then_archive_local_log(
        local_path,
        external_path,
    ...
    assert local_path.read_text(encoding="utf-8") == "local-source\n"
    assert external_path.read_text(encoding="utf-8") == "external-source\n"
    assert not archive_root.exists()
    assert actions[-1]["status"] == "error"


def test_storage_split_brain_reconciler_atomically_promotes_local_superset(
    tmp_path: Path,
) -> None:
    local_path = tmp_path / "local" / "logs" / "watchdog.log"
    external_path = tmp_path / "external" / "logs" / "watchdog.log"
    local_path.parent.mkdir(parents=True)
    external_path.parent.mkdir(parents=True)
    external_path.write_text("shared\n", encoding="utf-8")
    local_path.write_text("shared\nnew-local\n", encoding="utf-8")

    result = src._merge_text_file_transactionally(local_path, external_path)

    assert result == {"merge_state": "local_superset", "external_replaced": True}
    assert external_path.read_text(encoding="utf-8") == "shared\nnew-local\n"
    assert local_path.read_text(encoding="utf-8") == "shared\nnew-local\n"
    assert list(external_path.parent.glob(f".{external_path.name}.merge.*")) == []


def test_storage_split_brain_reconciler_preserves_external_superset_without_rewrite(
    tmp_path: Path,
) -> None:
    local_path = tmp_path / "local" / "logs" / "watchdog.log"
    external_path = tmp_path / "external" / "logs" / "watchdog.log"
    ...
    local_path.write_text("shared\n", encoding="utf-8")
    external_path.write_text("shared\nnew-external\n", encoding="utf-8")
    inode_before = external_path.stat().st_ino

    result = src._merge_text_file_transactionally(local_path, external_path)

    assert result == {"merge_state": "external_superset", "external_replaced": False}
    assert external_path.stat().st_ino == inode_before
    assert external_path.read_text(encoding="utf-8") == "shared\nnew-external\n"


def test_storage_split_brain_reconciler_places_data_archive_on_external_storage(
    tmp_path: Path,
) -> None:
    project_root = tmp_path / "project"
    external_root = tmp_path / "external"

    archive_root, manifest_root = src._repair_archive_roots(
        project_root,
        external_root,
    ...
    assert archive_root == (
        external_root / "cold_archive" / "storage_split_brain" / "20260823T000000Z"
    )
...
def test_storage_split_brain_reconciler_records_bounded_failback_timeout(
    tmp_path: Path, monkeypatch
) -> None:
...
def test_storage_split_brain_reconciler_records_successful_bounded_failback(
    tmp_path: Path, monkeypatch
) -> None:
    routing = SimpleNamespace(
        mode="external", active_root=tmp_path / "external", split_brain_conflicts=0
    )
    monkeypatch.setattr(
        src.storage_router, "route_runtime_storage", lambda _project_root: routing
    )
