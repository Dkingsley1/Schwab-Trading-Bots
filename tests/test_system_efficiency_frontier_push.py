...
from scripts.ops.system_efficiency_frontier_push import build_payload
...
def test_system_efficiency_frontier_applies_control_plane_then_pauses(tmp_path: Path):
    health_root = tmp_path / "governance" / "health"
    _write_json(
        health_root / "whole_system_safety_frontier_latest.json",
        {
            "overall_status": "whole_system_frontier_control_plane_applied_pause_for_soak",
            "safety_stop_active": True,
            "safety_stop_reason": ["promotion_gate_blocked"],
        },
    )
    _write_json(
        health_root / "safety_bounded_advancement_frontier_latest.json",
        {
            ...
            "safety_stop_active": True,
            "safety_stop_reason": ["large_training_batch_running_control_plane_only"],
        },
    )
    _write_json(
    ...
    assert payload["overall_status"] == "system_efficiency_frontier_control_plane_applied_pause_for_soak"
    assert payload["domain_count"] == 12
    assert payload["control_plane_domain_count"] == 12
    assert payload["efficiency_contract_domain_count"] == 12
    ...
    assert payload["destructive_cleanup_authority_enabled"] is False
    assert "destructive_cleanup" in payload["do_not_push_until_guard_clears"]
    ...
    assert all(domain["destructive_cleanup_authority_enabled"] is False for domain in payload["domains"])
