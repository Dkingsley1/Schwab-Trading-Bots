...
def test_maintenance_hold_engage_expire_and_release(tmp_path: Path, monkeypatch) -> None:
    ...
    assert maintenance_hold_blocks_runtime_start(engaged) is False
    monkeypatch.delenv(MAINTENANCE_HOLD_TOKEN_ENV)
    assert maintenance_hold_blocks_runtime_start(engaged) is True
    assert maintenance_hold_blocks_runtime_start(engaged, token="wrong-token") is True
    assert maintenance_hold_blocks_runtime_start(engaged, token=engaged["token"]) is False
    ...
    assert maintenance_hold_blocks_runtime_start(expired) is False
