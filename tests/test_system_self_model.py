...
def test_institutional_research_extensions_awareness_keeps_evidence_debt_advisory() -> (
    None
):
...
def test_alpha_generation_awareness_keeps_evidence_debt_advisory_and_live_locked() -> (
    None
):
...
def test_alpha_surface_marks_candidate_evidence_collection_advisory() -> None:
    ...
    assert (
        metadata["managed_control_state"]
        == "candidate_bound_economic_evidence_is_collecting"
    )
...
def test_master_infra_self_audit_cycle_is_advisory_only_for_green_paper_soak() -> None:
    ...
    status, metadata = src._normalize_guarded_paper_surface(
        "master_infra", "degraded", payload, context
    )
    ...
    assert (
        metadata["managed_control_state"]
        == "self_auditing_infra_debt_deferred_while_hot_path_is_green"
    )
...
def test_system_self_model_builds_awareness_domains_and_optimizations(
    tmp_path: Path,
) -> None:
    rows = [
        {
    ...
    _write_json(
        tmp_path / "master_bot_registry.json", {"summary": {}, "sub_bots": rows}
    )
    ...
    _write_json(
        ...
        {"overall_status": "ready", "severity": "stable", "pressure_index": 0.01},
    )
    _write_json(
        ...
        health / "whole_system_intelligence_latest.json",
        {
            ...
            "system_self_intelligence": {
                ...
                "action_effectiveness": {
                    "verdict": "effective",
                    "same_action_run_length": 2,
                },
                "causal_diagnosis": {
                    "primary_root_cause": "stable_or_observing",
                    "confidence": 0.82,
                },
                "integration_routing": {
                    "route_mode": "observe_and_refresh",
                    "primary_owner": "system_brain",
                },
                "capability_gaps": [],
                "reflex": {
            ...
            "codex_handoff": {
                "overall_status": "ready",
                "attention_packet": {
                    "needs_codex": ["observe_current_state_and_continue_safe_expansion"]
                },
    ...
    _write_json(
        ...
        {"overall_status": "ready", "metrics": {"backpressure_actionable": False}},
    )
    _write_json(
    ...
    _write_json(
        health / "auth_lease_manager_latest.json",
        {
            "overall_status": "ready",
            "lease_state": "healthy",
            "lease_budget": {"expires_in_seconds": 2400, "min_lease_seconds": 1200},
            "broker_state": {"auth_ok": True},
        },
    )
    _write_json(
        health / "data_plane_recovery_controller_latest.json",
        {"overall_status": "ready", "recovery_state": "ready", "queue_depth": 0},
    )
    _write_json(
    ...
    _write_json(
        health / "core_bot_materialization_guard_latest.json",
        {
            ...
            "summary": {
                ...
                "duplicate_core_version_count": 0,
            },
        },
    )
    _write_json(
    ...
    _write_json(
        tmp_path / "governance" / "alerts" / "incident_auto_halt_latest.json",
        {"overall_status": "ready", "event": "none"},
    )
    ...
    (scripts_ops / "writer_cycle_coordinator.py").write_text(
        "writer_process_intelligence\n", encoding="utf-8"
    )
    (scripts_ops / "sql_link_shard_manager.py").write_text(
        "writer_progress\nadmission_evidence\n", encoding="utf-8"
    )
    ...
    assert (
        payload["awareness_domains"]["host_pressure_intelligence"]["status"]
        == "advisory"
    )
    assert (
        payload["awareness_domains"]["host_pressure_intelligence"]["cpu_pressure_level"]
        == "watch"
    )
    assert (
        payload["awareness_domains"]["host_pressure_intelligence"][
            "recommended_intelligence_posture"
        ]
        == "foreground_safe"
    )
    assert (
        payload["awareness_domains"]["mlx_intelligence_awareness"]["status"] == "ready"
    )
    assert (
        payload["awareness_domains"]["mlx_intelligence_awareness"][
            "library_coverage_ratio"
        ]
        == 1.0
    )
    assert (
        payload["awareness_domains"]["library_utilization_awareness"]["status"]
        == "ready"
    )
    assert (
        payload["awareness_domains"]["library_utilization_awareness"][
            "mapped_package_ratio"
        ]
        == 1.0
    )
    assert (
    ...
    assert (
        payload["awareness_domains"]["drainer_intelligence"]["active_drainer"]
        == "core_decision_drainer"
    )
    assert (
        payload["awareness_domains"]["drainer_intelligence"]["single_writer_guard"]
        is True
    )
    assert (
        payload["awareness_domains"]["writer_process_intelligence"]["status"] == "ready"
    )
    assert (
        payload["awareness_domains"]["writer_process_intelligence"][
            "expanded_writer_lane_count"
        ]
        == 25
    )
    assert (
    ...
    assert (
        payload["awareness_domains"]["whole_system_intelligence"]["status"] == "ready"
    )
    assert (
        payload["awareness_domains"]["whole_system_intelligence"]["signal_count"] == 22
    )
    assert (
        payload["awareness_domains"]["whole_system_intelligence"]["action"]
        == "observe_and_expand_cautiously"
    )
    assert (
        payload["awareness_domains"]["whole_system_intelligence"]["contract_count"] == 7
    )
    assert (
        payload["awareness_domains"]["whole_system_intelligence"]["self_reflex_action"]
        == "follow_system_brain"
    )
    assert (
        payload["awareness_domains"]["whole_system_intelligence"][
            "self_uncertainty_level"
        ]
        == "low"
    )
    assert (
        payload["awareness_domains"]["whole_system_intelligence"]["self_causal_root"]
        == "stable_or_observing"
    )
    assert (
        payload["awareness_domains"]["whole_system_intelligence"][
            "self_action_effect_verdict"
        ]
        == "effective"
    )
    assert (
        payload["awareness_domains"]["whole_system_intelligence"][
            "self_integration_route"
        ]
        == "observe_and_refresh"
    )
    assert (
        payload["awareness_domains"]["whole_system_intelligence"][
            "codex_handoff_channel"
        ]
        == "artifact_handoff"
    )
    assert payload["awareness_domains"]["system_self_intelligence"]["status"] == "ready"
    assert (
        payload["awareness_domains"]["system_self_intelligence"]["trajectory"] == "flat"
    )
    assert (
        payload["awareness_domains"]["system_self_intelligence"]["reflex_action"]
        == "follow_system_brain"
    )
    assert (
        payload["awareness_domains"]["system_self_intelligence"]["uncertainty_level"]
        == "low"
    )
    assert (
        payload["awareness_domains"]["system_self_intelligence"]["causal_root"]
        == "stable_or_observing"
    )
    assert (
        payload["awareness_domains"]["system_self_intelligence"][
            "action_effect_verdict"
        ]
        == "effective"
    )
    assert (
        payload["awareness_domains"]["system_self_intelligence"][
            "integration_route_mode"
        ]
        == "observe_and_refresh"
    )
    assert payload["awareness_domains"]["codex_operator_bridge"]["status"] == "advisory"
    assert (
        payload["awareness_domains"]["codex_operator_bridge"]["needs_codex_count"] == 1
    )
    assert (
        payload["awareness_domains"]["codex_operator_bridge"]["paper_day_executions"]
        == 120
    )
    assert payload["awareness_domains"]["codex_operator_bridge"][
        "training_launch_blockers"
    ] == ["host_training_headroom_not_clear"]
    ...
    assert (
        payload["awareness_domains"]["system_role_contract"]["authority_conflict_count"]
        == 0
    )
    assert payload["awareness_domains"]["failure_memory"]["status"] == "ready"
    assert (
        payload["awareness_domains"]["halt_recovery_intelligence"]["status"] == "ready"
    )
    assert (
        payload["awareness_domains"]["halt_recovery_intelligence"]["live_lane_running"]
        is True
    )
    assert payload["awareness_domains"]["halt_recovery_intelligence"][
        "next_safe_command"
    ][-2:] == ["health-fast", "--json"]
    ...
    assert (
        "library_utilization_brain" in payload["upgrade_optimizer"]["implemented_lanes"]
    )
    assert (
        "host_pressure_intelligence"
        in payload["upgrade_optimizer"]["implemented_lanes"]
    )
    ...
    assert (
        "whole_system_intelligence" in payload["upgrade_optimizer"]["implemented_lanes"]
    )
    assert (
        "system_self_intelligence" in payload["upgrade_optimizer"]["implemented_lanes"]
    )
    assert (
        payload["control_contract"]["platform_brain_mode"]
        == "big_platform_brain_operational_control_plane"
    )
    assert len(payload["upgrades_and_optimizations"]) >= 6
    assert (
        payload["control_contract"]["consciousness_claim"]
        == "none_operational_self_model_only"
    )
...
def test_system_self_model_writes_json_and_markdown(tmp_path: Path) -> None:
    _write_json(
        ...
        {"summary": {"total_bots": 1, "active_bots": 1}, "sub_bots": []},
    )
    _write_json(
    ...
    _write_json(
        ...
        {"overall_status": "ready", "severity": "stable", "pressure_index": 0.0},
    )
    ...
    brief_path = (
        tmp_path / "exports" / "reports" / "operator" / "system_self_brief_latest.md"
    )
    dependency_path = (
        tmp_path / "governance" / "health" / "system_dependency_memory_latest.json"
    )
    failure_path = (
        tmp_path / "governance" / "health" / "system_failure_memory_latest.json"
    )
    registry_diff_path = (
        tmp_path / "governance" / "health" / "system_registry_diff_latest.json"
    )
    upgrade_path = (
        tmp_path / "governance" / "health" / "system_upgrade_optimizer_latest.json"
    )
    src.write_outputs(
        payload,
        out_path=out_path,
...
def test_system_self_model_builds_active_halt_recovery_plan(tmp_path: Path) -> None:
    ...
    _write_json(
        health / "data_plane_recovery_controller_latest.json",
        {
            ...
            "runtime_clearance_state": "awaiting_coverage_cycles",
            "queue_depth": 12,
        },
    )
    _write_json(
        health / "live_runtime_separation_control_latest.json",
        {"overall_status": "degraded", "live_plane": {"live_lane_running": False}},
    )
    _write_json(
        tmp_path / "governance" / "alerts" / "incident_auto_halt_latest.json",
        {"overall_status": "ready"},
    ...
    assert halt["next_safe_command"] == [
        "./scripts/ops/opsctl.sh",
        "token-refresh",
        "--json",
    ]
    assert ["./scripts/ops/opsctl.sh", "global-halt-auto-clear", "--json"] in halt[
        "recovery_sequence"
    ]
...
def test_system_self_model_escalates_auth_fallback_when_token_does_not_extend(
    tmp_path: Path,
) -> None:
    _write_json(
        tmp_path / "master_bot_registry.json",
    ...
    _write_json(
        health / "global_killswitch_latest.json",
        {
            ...
            "clear_ready": True,
            "clear_blockers": [],
            "global_halt_payload": {"reason": "softguard_api_circuit_open"},
        },
    )
    _write_json(
        health / "process_watchdog_latest.json",
        {
            "overall_status": "ready",
            "status": [{"name": "all_sleeves", "process_live": True}],
        },
    )
    _write_json(
        health / "auth_lease_manager_latest.json",
    ...
    halt = src.build_payload(tmp_path)["awareness_domains"][
    ...
    assert [
        ...
        "--force",
        "--json",
    ] in halt["recovery_sequence"]


def test_system_self_model_softens_guarded_paper_managed_surfaces(
    tmp_path: Path,
) -> None:
...
def test_guarded_paper_context_uses_authoritative_contract_when_dashboard_is_degraded(
    tmp_path: Path,
) -> None:
    health = tmp_path / "governance" / "health"
    _write_json(
        health / "runtime_gate_dashboard_latest.json",
        {
            "overall": {
                ...
                "forensic_attention": ["teacher_quality_guard_blocked"],
                "soak_management_context": {
                    "soak_ready": True,
                    "paper_guard_clean": True,
                    "paper_stage": "armed",
    ...
    _write_json(
        ...
        {"ok": True, "overall_status": "ready", "overall_grade": "A+"},
    )
    _write_json(
        health / "runtime_paper_regression_guard_latest.json",
        {"ok": True, "overall_status": "ready", "paper_stage": "armed"},
    )

    context = src._guarded_paper_management_context(health)

    assert context["enabled"] is True
    assert context["dashboard_contract_enabled"] is False
    assert context["authoritative_contract_enabled"] is True
    assert context["managed_by"] == "authoritative_guarded_paper_contract"
    assert context["soak_grade"] == "A+"
    assert context["raw_attention"] == [
        "retrain_artifact_freshness_not_ok",
        "training_quality_control_blocked",
    ]
    assert context["forensic_attention"] == ["teacher_quality_guard_blocked"]


def test_guarded_paper_context_fails_closed_when_authoritative_paper_guard_blocks(
    tmp_path: Path,
) -> None:
    health = tmp_path / "governance" / "health"
    _write_json(
        health / "runtime_gate_dashboard_latest.json",
        {"overall": {"status": "degraded", "ok": False}},
    )
    _write_json(
        health / "health_fast_latest.json",
        {
    ...
    _write_json(
        health / "runtime_paper_regression_guard_latest.json",
        {"ok": False, "overall_status": "blocked", "paper_stage": "blocked"},
    )
    ...
    assert context["dashboard_contract_enabled"] is False
    assert context["authoritative_contract_enabled"] is False
    assert context["managed_by"] == "none"


def test_system_self_model_keeps_optional_support_staleness_advisory_during_guarded_paper_soak(
    tmp_path: Path,
) -> None:
    now = datetime(2026, 7, 27, 16, 15, tzinfo=timezone.utc)
    ...
    for artifact, status in {
        ...
        _write_json(
            health / artifact,
            {"timestamp_utc": old.isoformat(), "overall_status": status},
        )
...
def test_system_self_model_includes_use_mode_compliance_awareness(
    tmp_path: Path,
) -> None:
    health = tmp_path / "governance" / "health"
    _write_json(
    ...
    assert (
        use_mode["next_after_production_personal"] == "operator_grade_personal_autonomy"
    )
    assert use_mode["live_execution_authority"] is False
    assert "use_mode_compliance" in payload["control_contract"]["memory_surfaces"]


def test_system_self_model_includes_commercial_readiness_awareness(
    tmp_path: Path,
) -> None:
    health = tmp_path / "governance" / "health"
    _write_json(
