...
# -----------------------------
# Extreme: jump diffusion + volatility spikes

def simulate_extreme(n=5000):
    prices = np.zeros(n)
    prices[0] = 100.0
    drift = 0.0001
    vol = 0.02
    ...
    for i in range(1, n):
        ...
        ret = drift + vol * np.random.randn() + jump
...
def train_brain():
    ...
    prices = simulate_extreme(n=5000)
    ...
    save_artifacts(brain, config, metrics, run_tag="brain_refinery_v6_extreme")
