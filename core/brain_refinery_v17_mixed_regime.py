...
# -----------------------------
# Mixed regime: alternates between trend and mean-reversion

def simulate_mixed_regime(n=5000):
    prices = np.zeros(n)
    prices[0] = 100.0
    mu = 100.0
    vol = 0.01
    ...
    for i in range(1, n):
        if (i // 1200) % 2 == 0:
            ...
            prices[i] = max(0.1, prices[i-1] + 0.2 * (mu - prices[i-1]) + 0.5 * np.random.randn())
    return prices
...
def train_brain():
    ...
    prices = simulate_mixed_regime(n=5000)
    ...
    save_artifacts(brain, config, metrics, run_tag="brain_refinery_v17_mixed_regime")
