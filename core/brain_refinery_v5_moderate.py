...
# -----------------------------
# Moderate: random walk with drift + regime shifts

def simulate_moderate(n=5000):
    prices = np.zeros(n)
    prices[0] = 100.0
    drift = 0.0002
    vol = 0.01
    for i in range(1, n):
        if i % 1000 == 0:
            vol = 0.005 if vol > 0.01 else 0.02
            drift = 0.0001 if drift > 0 else 0.0003
...
def train_brain():
    ...
    prices = simulate_moderate(n=5000)
    ...
    save_artifacts(brain, config, metrics, run_tag="brain_refinery_v5_moderate")
