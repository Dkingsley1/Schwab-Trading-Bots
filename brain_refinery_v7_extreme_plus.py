...
# -----------------------------
# Extreme+: crashes + halts + fat tails

def simulate_extreme_plus(n=5000):
    prices = np.zeros(n)
    prices[0] = 100.0
    drift = 0.0001
    vol = 0.025
    for i in range(1, n):
        if i % 1200 == 0:
            crash = np.random.uniform(0.7, 0.85)
            prices[i] = prices[i-1] * crash
            for j in range(1, 5):
                if i + j < n:
                    prices[i + j] = prices[i]
            continue
        ret = drift + vol * np.random.standard_t(df=3)
...
def train_brain():
    ...
    prices = simulate_extreme_plus(n=5000)
    ...
    save_artifacts(brain, config, metrics, run_tag="brain_refinery_v7_extreme_plus")
