...
| `fixed_income_trace_context` | FINRA corporate market breadth; FINRA Treasury daily aggregates (optional FINRA public OAuth credential) |
| `bis_global_liquidity_context` | BIS Global Liquidity Indicators bulk data |
| `tradingeconomics_guest` | Trading Economics guest feed, secondary cross-check only |

The registry currently contains 35 direct source contracts plus 32 expanded central-bank members. The count is inventory only: it does not raise readiness, alpha, profitability, or promotion grades.
...
- `finra_fixed_income_trace` adds official corporate advance/decline breadth and Treasury aggregate turnover/liquidity evidence through a bounded shared snapshot. FINRA currently requires a public OAuth API credential; without `FINRA_API_ACCESS_TOKEN`, this collector reports optional credential debt and does not make an unauthorized request.
- `bis_global_liquidity_indicators` adds quarterly global credit and cross-border liquidity context from the official BIS bulk file; publication lag is preserved rather than disguised as an intraday signal.
