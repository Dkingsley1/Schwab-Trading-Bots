...
# -----------------------------
# Microstructure noise: bid/ask jitter around a trend

def simulate_microstructure(n=5000):
    prices = np.zeros(n)
    prices[0] = 100.0
    drift = 0.0002
    vol = 0.005
    spread = 0.02
    for i in range(1, n):
        mid_ret = drift + vol * np.random.randn()
        mid = prices[i-1] * np.exp(mid_ret)
        jitter = np.random.choice([-1, 1]) * spread * np.random.rand()
        prices[i] = max(0.1, mid + jitter)
    return prices
...
def train_brain():
    ...
    prices = simulate_microstructure(n=5000)
    ...
    save_artifacts(brain, config, metrics, run_tag="brain_refinery_v14_microstructure")
