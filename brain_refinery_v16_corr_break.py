...
# -----------------------------
# Correlation breaks: mix two regimes with opposite correlation signals

def simulate_corr_break(n=5000):
    base = np.zeros(n)
    base[0] = 100.0
    trend = 0.0002
    vol = 0.01
    ...
    for i in range(1, n):
        if i % 1500 == 0:
            corr *= -1.0
        ret = trend * corr + vol * np.random.randn()
        base[i] = base[i-1] * np.exp(ret)
    return base
...
def train_brain():
    ...
    prices = simulate_corr_break(n=5000)
    ...
    save_artifacts(brain, config, metrics, run_tag="brain_refinery_v16_corr_break")
