...
# -----------------------------
# Trend-following: long bull/bear runs

def simulate_trend(n=5000):
    prices = np.zeros(n)
    prices[0] = 100.0
    drift = 0.0003
    vol = 0.01
    ...
    for i in range(1, n):
        if i % 1500 == 0:
            trend *= -1
        ret = trend * drift + vol * np.random.randn()
...
def train_brain():
    ...
    save_artifacts(brain, config, metrics, run_tag="brain_refinery_v9_trend")
