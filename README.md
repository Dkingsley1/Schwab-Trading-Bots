...
| Generation-aware developmental profitability | accepted candidate events joined to candidate- and interval-bound post-cost outcomes in `governance/health/profitability_self_assessment_latest.json` | earlier accepted soak changes may drive bounded paper-only collection, replay, fill-evidence, labeling, and containment work; mixed or unbound history remains diagnostic, never earns clean `720`-hour credit, and cannot change size, loosen thresholds, promote, or authorize an order |
