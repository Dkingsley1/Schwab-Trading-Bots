...
# -----------------------------
# Stochastic volatility: volatility clusters

def simulate_stoch_vol(n=5000):
    prices = np.zeros(n)
    prices[0] = 100.0
    vol = 0.01
    kappa = 0.05
    ...
    drift = 0.0002
    for i in range(1, n):
        vol = max(1e-4, vol + kappa * (vol_mean - vol) + vol_vol * np.random.randn())
...
def train_brain():
    ...
    prices = simulate_stoch_vol(n=5000)
    ...
    save_artifacts(brain, config, metrics, run_tag="brain_refinery_v11_stoch_vol")
