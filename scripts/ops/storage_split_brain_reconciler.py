...
DEFAULT_MARKDOWN_PATH = (
    ...
    / "reports"
    / "storage"
    / "storage_split_brain_reconciler_latest.md"
)
...
def _force_failback(
    project_root: Path, *, timeout_sec: int, fallback_conflicts: int
) -> dict[str, Any]:
    ...
    split_brain_conflicts = int(
        getattr(routing, "split_brain_conflicts", fallback_conflicts) or 0
    )
    return {
        "attempted": True,
...
def _local_fallback_root(project_root: Path) -> Path:
    return Path(
        os.getenv(
            ...
            str(project_root / storage_router.DEFAULT_LOCAL_FALLBACK),
        )
    ).expanduser()
...
def _archive_root(project_root: Path, *, stamp: str | None = None) -> Path:
    return (
        ...
        / "quarantine"
        / "storage_split_brain"
        / (stamp or _safe_stamp())
    )


def _external_archive_root(external_root: Path, *, stamp: str | None = None) -> Path:
    return (
        external_root
        / "cold_archive"
        / "storage_split_brain"
...
def _repair_archive_roots(
    ...
    *,
    ...
    return (
        _external_archive_root(external_root, stamp=stamp),
        _archive_root(project_root, stamp=stamp),
    )
...
def _archive_external_conflict_sidecars(
    ...
    *,
    ...
    del rows
    selected = [
        {"conflict_file": str(path)} for path in _iter_conflict_files(external_root)
    ]
    if not apply:
        return {
            "requested": True,
...
def _router_log_conflict_rows(project_root: Path) -> list[dict[str, Any]]:
    ...
    local_sig = storage_router._scan_tree_signature(
        local_root, storage_router.DEFAULT_LINK_DIRS, max_files=5000
    )
    if not local_sig:
        return []
    external_sig = storage_router._scan_tree_signature(
        external_root, storage_router.DEFAULT_LINK_DIRS, max_files=5000
    )
    rows: list[dict[str, Any]] = []
...
def _ranges_equal(
    ...
    *,
    ...
    with left_path.open("rb") as left_handle, right_path.open("rb") as right_handle:
        left_handle.seek(max(int(left_offset), 0))
        right_handle.seek(max(int(right_offset), 0))
        while remaining > 0:
...
def _atomic_merge_target(external_path: Path) -> Path:
    external_path.parent.mkdir(parents=True, exist_ok=True)
    descriptor, raw_path = tempfile.mkstemp(
        prefix=f".{external_path.name}.merge.",
        suffix=".tmp",
        dir=str(external_path.parent),
    )
    os.close(descriptor)
    return Path(raw_path)


def _sync_and_replace(temp_path: Path, external_path: Path) -> None:
    with temp_path.open("rb") as handle:
        os.fsync(handle.fileno())
    os.replace(temp_path, external_path)
    try:
        directory_fd = os.open(str(external_path.parent), os.O_RDONLY)
    except OSError:
        return
    try:
...
def _copy_to_atomic_target(source_path: Path, external_path: Path) -> None:
    temp_path = _atomic_merge_target(external_path)
    try:
        with source_path.open("rb") as source_handle, temp_path.open(
            "wb"
        ) as out_handle:
            shutil.copyfileobj(source_handle, out_handle, length=4 * 1024 * 1024)
            out_handle.flush()
            os.fsync(out_handle.fileno())
        _sync_and_replace(temp_path, external_path)
    finally:
        temp_path.unlink(missing_ok=True)


def _common_merge_state(local_path: Path, external_path: Path) -> str:
    if not external_path.exists():
        return "external_missing"
    local_size = local_path.stat().st_size
    external_size = external_path.stat().st_size
    if local_size == external_size and _ranges_equal(
        local_path, external_path, local_size
    ):
        return "identical"
    if external_size < local_size and _ranges_equal(
        local_path, external_path, external_size
    ):
        return "local_superset"
    if local_size < external_size and _ranges_equal(
        local_path, external_path, local_size
    ):
        return "external_superset"
    if local_size <= external_size and _ranges_equal(
        local_path,
        external_path,
        local_size,
        right_offset=external_size - local_size,
    ):
        return "local_already_appended"
    if external_size < local_size and _ranges_equal(
        external_path,
        local_path,
        external_size,
        right_offset=local_size - external_size,
    ):
        return "external_is_local_suffix"
    return "divergent"


def _merge_text_file_transactionally(
    local_path: Path,
    external_path: Path,
) -> dict[str, Any]:
    state = _common_merge_state(local_path, external_path)
    if state in {"external_missing", "local_superset", "external_is_local_suffix"}:
        _copy_to_atomic_target(local_path, external_path)
        return {"merge_state": state, "external_replaced": True}
    if state in {"identical", "external_superset", "local_already_appended"}:
        return {"merge_state": state, "external_replaced": False}

    temp_path = _atomic_merge_target(external_path)
    try:
        with temp_path.open("wb") as out_handle:
            if external_path.exists():
                with external_path.open("rb") as external_handle:
                    shutil.copyfileobj(
                        external_handle, out_handle, length=4 * 1024 * 1024
                    )
            if out_handle.tell() > 0:
                out_handle.write(
                    b"\n--- storage split-brain merge: local fallback continuation ---\n"
                )
            with local_path.open("rb") as local_handle:
                shutil.copyfileobj(local_handle, out_handle, length=4 * 1024 * 1024)
            out_handle.flush()
            os.fsync(out_handle.fileno())
        _sync_and_replace(temp_path, external_path)
    finally:
        temp_path.unlink(missing_ok=True)
    return {"merge_state": "divergent_appended", "external_replaced": True}


def _merge_jsonl_transactionally(
    local_path: Path,
    external_path: Path,
) -> dict[str, Any]:
    state = _common_merge_state(local_path, external_path)
    ...
    temp_path = _atomic_merge_target(external_path)
    index_descriptor, index_raw = tempfile.mkstemp(
        prefix=f".{external_path.name}.merge-index.",
        suffix=".sqlite3",
        dir=str(external_path.parent),
    )
    ...
    try:
        ...
        index_path.with_name(f"{index_path.name}-journal").unlink(missing_ok=True)
        index_path.with_name(f"{index_path.name}-wal").unlink(missing_ok=True)
        index_path.with_name(f"{index_path.name}-shm").unlink(missing_ok=True)
    return {
        "merge_state": "divergent_deduplicated",
        "external_replaced": True,
    ...
    *,
    ...
    try:
        ...
        if local_path.suffix.lower() == ".jsonl":
            merge_result = _merge_jsonl_transactionally(local_path, external_path)
        else:
            merge_result = _merge_text_file_transactionally(local_path, external_path)
    except Exception as exc:
        actions.append(
            {
                "action": "merge_local_log_to_external",
    ...
    return _archive_file(
        ...
        action="archive_local_router_conflict_after_merge",
        actions=actions,
    )


def _repair_router_log_conflicts(
    project_root: Path, *, archive_base: Path, apply: bool
) -> dict[str, Any]:
...
def build_payload(
    project_root: Path = PROJECT_ROOT, *, full_scan: bool = False
) -> dict[str, Any]:
    external_root = storage_router._external_project_root()
    local_root = _local_fallback_root(project_root)
    failback_payload = _load_json(
        project_root / "governance" / "health" / "storage_failback_sync_latest.json"
    )
    mount_payload = _load_json(
    ...
    artifact_reported_split_brain_conflicts = max(
        ...
        int(
            mount_payload.get("storage_mount_transition", {})
            .get("recovery", {})
            .get("payload", {})
            .get("split_brain_conflicts", 0)
            or 0
        ),
    )
    ...
    if full_scan or artifact_reported_split_brain_conflicts > 0:
        for conflict_path in _iter_conflict_files(external_root):
            ...
            relative_path = (
                str(canonical_external.relative_to(external_root))
                if canonical_external.exists() or external_root.exists()
                else canonical_external.name
            )
    ...
    unresolved_conflicts = sum(
        1 for row in rows if not bool(row.get("hashes_match", False))
    )
    ...
    force_failback_eligible = (
        bool(mount_payload.get("external_available", False))
    ...
    payload = {
        ...
        "current_storage_mode": str(
            mount_payload.get("storage_mode") or failback_payload.get("mode") or ""
        ),
        "scan_mode": scan_mode,
        "summary": {
            ...
            "router_repairable_conflicts": sum(
                1 for row in router_conflicts if bool(row.get("repair_allowed", False))
            ),
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Classify and report BOT_LOGS split-brain conflicts."
    )
    ...
    repair_requested = bool(
        args.archive_external_conflict_sidecars or args.repair_router_log_conflicts
    )
    ...
    if repair_requested:
        stamp = _safe_stamp()
        external_root = storage_router._external_project_root()
        archive_base, manifest_base = _repair_archive_roots(
            project_root,
            external_root,
            stamp=stamp,
        )
        ...
        if args.repair_router_log_conflicts:
            router_result = _repair_router_log_conflicts(
                project_root, archive_base=archive_base, apply=bool(args.apply)
            )
        ...
        if args.apply:
            ...
            repair_result["manifest_path"] = _write_manifest(
                manifest_base, repair_actions
            )
            repair_result["external_manifest_path"] = _write_manifest(
                archive_base,
                repair_actions,
            )
    ...
    if args.force_failback_if_hashes_match and bool(
        payload.get("summary", {}).get("force_failback_eligible", False)
    ):
        payload["forced_failback"] = _force_failback(
            project_root,
            timeout_sec=int(args.force_failback_timeout_sec),
            fallback_conflicts=int(
                payload.get("summary", {}).get("reported_split_brain_conflicts", 0) or 0
            ),
        )
    else:
