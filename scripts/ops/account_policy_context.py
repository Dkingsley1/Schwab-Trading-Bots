...
DEFAULT_OUT_PATH = (
    PROJECT_ROOT / "governance" / "health" / "account_policy_context_latest.json"
)
...
def _pdt_transition_contract(as_of_date: date) -> dict[str, Any]:
    ...
    return {
        ...
        "system_posture": (
            ...
            if broker_implementation_watch_required
            or not schwab_day_trade_count_retired
...
def _latest_schwab_account_metrics(project_root: Path) -> dict[str, Any]:
    ...
    for path in rows:
        ...
        try:
            age_seconds = max(
                datetime.now(timezone.utc).timestamp() - float(path.stat().st_mtime),
                0.0,
            )
        except Exception:
            age_seconds = 0.0
        fetched = (
            payload.get("fetched") if isinstance(payload.get("fetched"), dict) else {}
        )
        record = {
...
def _first_positive_from_balances(
    balance_rows: list[dict[str, Any]], keys: list[str]
) -> float:
...
def _extract_account_metrics_from_broker_truth(payload: Any) -> dict[str, Any]:
    ...
    account = (
        snapshot_payload.get("securitiesAccount")
    ...
    current_balances = (
        ...
        if isinstance(account.get("currentBalances"), dict)
        else {}
    )
    projected_balances = (
        account.get("projectedBalances")
        if isinstance(account.get("projectedBalances"), dict)
        else {}
    )
    initial_balances = (
        account.get("initialBalances")
        if isinstance(account.get("initialBalances"), dict)
        else {}
    )
    ...
    return {
        "equity": _first_positive_from_balances(
            balance_rows, ["liquidationValue", "equity", "accountValue"]
        ),
...
def _intraday_margin_probe_contract(
    ...
    *,
    ...
    metrics = (
        broker_metrics.get("metrics")
        if isinstance(broker_metrics.get("metrics"), dict)
        else {}
    )
...
def _paper_intraday_margin_deficit_simulator(
    *,
    ...
    simulated_exposure = max(
        _safe_float(os.getenv("PAPER_INTRADAY_MARGIN_SIM_EXPOSURE_USD"), 0.0), 0.0
    )
    ...
    return {
        ...
        "intraday_buying_power_observed": bool(
            probe_contract.get("intraday_buying_power_observed", False)
        ),
        "phase": str(pdt_contract.get("phase") or ""),
...
def _slot_margin_policy(
    row: dict[str, Any], pdt_contract: dict[str, Any]
) -> dict[str, Any]:
    account_type = str(row.get("account_type") or "unknown").strip().lower()
    broker = str(row.get("broker") or "unknown").strip().lower()
    trading_access = str(row.get("trading_access") or "unknown").strip().lower()
    limited_margin = trading_access == "limited_margin"
    margin_enabled = _as_bool(
        row.get("margin_enabled"),
        default=trading_access in {"full_margin", "portfolio_margin"}
        or account_type == "margin",
    )
    borrowing_allowed = _as_bool(
        row.get("borrowing_allowed"),
        default=trading_access in {"full_margin", "portfolio_margin"},
    )
    if limited_margin:
        margin_enabled = False
        borrowing_allowed = False
        applicability = "limited_margin_no_borrowing_or_interest_authority"
        day_trade_widening_allowed = False
    elif (
        account_type in {"cash", "roth", "ira", "traditional_ira"} or not margin_enabled
    ):
        applicability = "not_margin_day_trading_account"
        day_trade_widening_allowed = False
    elif broker == "schwab" and bool(
        pdt_contract.get("schwab_day_trade_count_retired", False)
    ):
        applicability = (
    ...
    return {
        ...
        "broker_margin_feature_enabled": limited_margin or margin_enabled,
        "margin_enabled": margin_enabled,
        "borrowing_allowed": borrowing_allowed,
        "margin_interest_possible": bool(margin_enabled and borrowing_allowed),
        "cash_only_live_budget": bool(row.get("cash_only_live_budget", False)),
        "pdt_or_intraday_margin_applicability": applicability,
        "legacy_pdt_framework_active_for_slot": (
            bool(
        ...
        "requires_intraday_margin_buying_power_confirmation": bool(
            margin_enabled and borrowing_allowed
        ),
...
def _slot_from_raw(raw: dict[str, Any]) -> dict[str, Any]:
    env_names = [
        str(item)
        for item in _as_list(raw.get("env_names") or raw.get("env_bindings"))
        if str(item)
    ]
    env_bindings = []
    for raw_name in env_names:
        name = str(
            raw_name.get("name") if isinstance(raw_name, dict) else raw_name
        ).strip()
        if not name:
            continue
        env_bindings.append({"name": name, "present": bool(os.environ.get(name))})
    account_type = str(raw.get("account_type") or "unknown").strip().lower()
    trading_access = (
        str(raw.get("trading_access") or raw.get("trading_type") or "").strip().lower()
    )
    if not trading_access:
        trading_access = (
            "full_margin"
            if account_type == "margin"
            else "cash" if account_type == "cash" else "unknown"
        )
    if trading_access == "margin":
    ...
    if limited_margin or trading_access == "cash":
        borrowing_allowed = False
    margin_enabled = _as_bool(
        raw.get("margin_enabled"),
        default=trading_access in {"full_margin", "portfolio_margin"},
    )
    if limited_margin:
        margin_enabled = False
    return {
        "account_policy_key": str(
            raw.get("account_policy_key") or raw.get("key") or "unknown_account"
        ),
        "account_label": str(
            raw.get("account_label") or raw.get("label") or "Unknown Account"
        ),
        "account_type": account_type,
        ...
        "trading_access": trading_access,
        "limited_margin": limited_margin,
        "broker_margin_feature_enabled": bool(limited_margin or margin_enabled),
        "margin_enabled": margin_enabled,
        "borrowing_allowed": borrowing_allowed,
        "margin_interest_possible": bool(margin_enabled and borrowing_allowed),
        "cash_only_live_budget": _as_bool(
            raw.get("cash_only_live_budget"),
            default=trading_access in {"cash", "limited_margin"},
        ),
        "canary_candidate": _as_bool(raw.get("canary_candidate"), False),
        "canary_cap_usd": round(
            max(_safe_float(raw.get("canary_cap_usd"), 0.0), 0.0), 2
        ),
        ...
        "existing_positions_authority": str(
            raw.get("existing_positions_authority") or "observe_only"
        ),
        "operator_verified": _as_bool(raw.get("operator_verified"), False),
        "verification_source": str(raw.get("verification_source") or "unverified"),
        ...
        "requires_operator_confirmation": bool(
            raw.get("requires_operator_confirmation", True)
        ),
    }
...
def _load_registry_slots(registry_path: Path) -> tuple[list[dict[str, Any]], bool]:
    ...
    rows = _as_list(
        registry.get("account_slots") or registry.get("configured_account_slots")
    )
    if rows:
...
def build_payload(
    ...
    *,
    ...
    probe_contract = _intraday_margin_probe_contract(
        project_root, as_of_date=effective_date, pdt_contract=pdt_contract
    )
    ...
    margin_slots = sum(
        1 for row in slot_margin_policies if bool(row.get("margin_enabled", False))
    )
    limited_margin_slots = sum(
        1 for row in slot_margin_policies if bool(row.get("limited_margin", False))
    )
    ...
    auto_order_enabled = any(
        bool(row.get("auto_order_enabled", False)) for row in slots
    )
    canary_candidates = [
        row for row in slots if bool(row.get("canary_candidate", False))
    ]
    missing_bindings = []
    for row in slots:
        hash_bindings = [
            ...
            and str(binding.get("name") or "").strip().endswith("_HASH")
        ]
        if hash_bindings and not any(
            bool(binding.get("present", False)) for binding in hash_bindings
        ):
            missing_bindings.append(
                {
                    "account_policy_key": row.get("account_policy_key"),
                    "required_binding_kind": "account_hash",
                    "acceptable_env_names": [
                        str(binding.get("name") or "") for binding in hash_bindings
                    ],
                    "raw_account_number_required": False,
                }
            )
    next_actions = []
    if missing_bindings:
        next_actions.append(
            "set account hash environment variables when live-micro account binding is intentionally approved"
        )
    if auto_order_enabled:
        next_actions.append(
            "turn off account auto-order flags before running any paper-to-live review"
        )
    if bool(pdt_contract.get("broker_implementation_watch_required", False)):
        next_actions.append(
            "keep Schwab day-trading widening under broker-cutover watch until the June 8 Schwab policy change is verified"
        )
    next_actions.append(
        "require broker-reported intraday margin buying power before any future live day-trading widening"
    )
    if bool(probe_contract.get("probe_required_now", False)):
        next_actions.append(
            "refresh Schwab broker-truth account metrics and confirm intraday buying power before live-micro review"
        )
    if canary_candidates:
        next_actions.append(
            "keep candidate accounts cash-capped and execution-disabled until account and route gates independently approve activation"
        )
    ...
    return {
        ...
        "account_policy_context": {
            ...
            "redaction_contract": {
                ...
                "operator_metadata_never_overwrites_provider_truth": True,
        ...
        "coverage": {
            ...
            "limited_margin_slots": limited_margin_slots,
            "borrowing_enabled_slots": sum(
                1
                for row in slot_margin_policies
                if bool(row.get("borrowing_allowed", False))
            ),
            "canary_candidate_slots": len(canary_candidates),
        ...
        "bot_contract": {
            ...
            "operator_confirmation_required": True,
            "supported_account_types": sorted(
                {str(row.get("account_type") or "unknown") for row in slots}
                | {"unknown"}
            ),
            "supported_trading_access": sorted(
                {str(row.get("trading_access") or "unknown") for row in slots}
                | {"unknown"}
            ),
            "limited_margin_is_not_borrowing_authority": True,
            "provider_account_type_is_not_operator_classification": True,
            "negative_margin_balance_is_not_automatically_debt": True,
            "account_truth_can_block_but_never_grant_live_execution": True,
            "canary_candidates": [
                {
                    "account_policy_key": row.get("account_policy_key"),
                    "account_label": row.get("account_label"),
                    "trading_access": row.get("trading_access"),
                    "canary_cap_usd": row.get("canary_cap_usd"),
                    "allowed_live_routes": row.get("allowed_live_routes", []),
                    "live_execution_authority": False,
                }
                for row in canary_candidates
            ],
            ...
            "schwab_day_trade_count_retired": bool(
                pdt_contract.get("schwab_day_trade_count_retired", False)
            ),
            "intraday_margin_buying_power_required": True,
            "intraday_margin_buying_power_observed": bool(
                probe_contract.get("intraday_buying_power_observed", False)
            ),
            "intraday_margin_probe_status": str(probe_contract.get("status") or ""),
            "paper_intraday_margin_simulator_status": str(
                margin_simulator.get("status") or ""
            ),
            "day_trade_widening_allowed": False,
            "live_execution_permission_unchanged": True,
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Refresh redacted account policy context for bots and income-readiness controls."
    )
    ...
    parser.add_argument(
        "--as-of-date",
        default="",
        help="Override policy date for testing/regression checks (YYYY-MM-DD).",
    )
    ...
    payload = build_payload(
        project_root, registry_path=registry_path, as_of_date=args.as_of_date or None
    )
