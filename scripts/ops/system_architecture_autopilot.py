...
DEFAULT_OUT_PATH = (
    PROJECT_ROOT / "governance" / "health" / "system_architecture_autopilot_latest.json"
)
DEFAULT_PLAN_PATH = (
    PROJECT_ROOT
    / "governance"
...
BENEFIT_BLUEPRINTS: tuple[dict[str, Any], ...] = (
    {
        ...
        "guards": [
            "does_not_stop_all_sleeves_launcher",
            "does_not_enable_live_execution",
            "paper_only_runtime_boundary",
        ],
        "acceptance_criteria": [
            "guarded_paper_ready",
            "paper_execution_cpu_below_hot_band",
            "runtime_status_ready_or_advisory",
        ],
    },
    {
        ...
        "signals": (
            "storage_writer_hot",
            "storage_backpressure_apply_failed",
            "runtime_degraded",
        ),
        ...
        "guards": [
            "single_writer_only",
            "bounded_storage_apply",
            "no_duplicate_sqlite_writers",
        ],
        "acceptance_criteria": [
            "storage_writer_hot_false",
            "raw_live_backlog_within_gate",
            "storage_backpressure_apply_ready",
        ],
    },
    {
        ...
        "phase": 2,
        "signals": (
            "adaptive_memory_blocked",
            "incident_closeout_blocked",
            "live_canary_blocked",
        ),
        ...
        "acceptance_criteria": [
            "open_incident_count_zero_or_waived",
            "live_canary_preclearance_ready",
            "section_grades_at_floor",
        ],
    },
    {
        ...
        "signals": (
            "stale_nodes",
            "adaptive_degraded_stale_surfaces",
            "system_drift_blocked",
        ),
        ...
        "guards": [
            "read_only_refresh_first",
            "no_destructive_cleanup",
            "surface_owner_required",
        ],
        "acceptance_criteria": [
            "stale_node_count_zero",
            "stale_degraded_surfaces_below_threshold",
        ],
    },
    {
        ...
        "acceptance_criteria": [
            "single_screen_summary",
            "top_safe_command_present",
            "paper_live_boundary_visible",
        ],
    },
    {
        ...
        "guards": [
            "simulation_before_apply",
            "bounded_timeout",
            "authority_filter_required",
        ],
        "acceptance_criteria": [
            "preflight_result_for_each_step",
            "root_blocker_not_retried_without_changed_evidence",
        ],
    },
    {
        ...
        "guards": [
            "paper_live_boundary_preserved",
            "training_yields_first",
            "reporting_yields_before_collectors",
        ],
        "acceptance_criteria": [
            "runtime_ready_or_advisory",
            "collector_health_ready",
            "paper_execution_not_hot",
        ],
    },
)
...
def _expected_earned_evidence_result(node_id: str, result: dict[str, Any]) -> bool:
    """Recognize a bounded, non-repairable training-evidence outcome."""
    if str(node_id or "").strip() != "architecture_hardening":
        return False
    if _safe_int(result.get("rc"), 1) != 2:
        return False

    payload = _as_dict(result.get("payload"))
    if not payload or bool(payload.get("ok", True)):
        return False
    if str(payload.get("overall_status") or "").strip().lower() not in {
        "degraded",
        "needs_work",
    }:
        return False
    if payload.get("read_only") is not True or bool(
        payload.get("started_heavy_reports", True)
    ):
        return False

    sections = {
        str(name): _as_dict(row)
        for name, row in _as_dict(payload.get("sections")).items()
    }
    non_ready_sections = {
        name
        for name, row in sections.items()
        if str(row.get("overall_status") or "").strip().lower()
        in {"blocked", "critical", "degraded", "needs_work"}
    }
    hard_sections = {
        str(item or "").strip()
        for item in _as_list(payload.get("hard_sections"))
        if str(item or "").strip()
    }
    if (
        non_ready_sections != {"training_evidence_contract"}
        or hard_sections != non_ready_sections
        or _safe_int(payload.get("hard_section_count"), -1) != 1
    ):
        return False

    safety = _as_dict(sections.get("safety_execution_boundary"))
    safety_evidence = _as_dict(safety.get("evidence"))
    if (
        str(safety.get("overall_status") or "").strip().lower() != "ready"
        or not bool(safety_evidence.get("guarded_paper_ok", False))
        or str(safety_evidence.get("live_execution_status") or "").strip().lower()
        != "blocked_read_only"
        or _as_list(safety_evidence.get("truthy_live_enable_flags"))
    ):
        return False

    training = _as_dict(sections.get("training_evidence_contract"))
    evidence = _as_dict(training.get("evidence"))
    managed = _as_dict(evidence.get("managed_training_evidence_contract"))
    return bool(
        not training.get("blocks_guarded_paper", True)
        and _safe_int(evidence.get("collector_count"), 0) > 0
        and _safe_float(evidence.get("coverage_ratio"), 0.0) >= 0.95
        and _safe_int(evidence.get("zero_observation_count"), 1) == 0
        and _safe_float(evidence.get("training_quality_score"), 0.0) >= 50.0
        and bool(managed.get("guarded_paper_ready", False))
        and bool(managed.get("collection_flowing", False))
    )
...
def _dependency_depth(
    node_id: str, nodes: dict[str, dict[str, Any]], visiting: set[str] | None = None
) -> int:
    ...
    deps = [
        str(dep)
        for dep in _as_list(nodes[node_id].get("depends_on"))
        if str(dep) in nodes
    ]
    if not deps:
        return 0
...
def _safe_repair_allowed(node: dict[str, Any]) -> bool:
    ...
    if (
        "live" in authority
        and "paper" not in authority
        and "observation" not in authority
    ):
        return False
    for cmd in _as_list(node.get("commands")):
        joined = " ".join(str(part) for part in _as_list(cmd))
        if (
            "start-live" in joined
            or "clear-all-halts" in joined
            or "operator-release" in joined
        ):
            return False
    return True
...
def _plan_from_graph(graph: dict[str, Any], *, max_steps: int) -> list[dict[str, Any]]:
    nodes = {
        ...
        for row in _as_list(graph.get("nodes"))
    ...
    candidates = [
        ...
        or bool(node.get("artifact_stale", False))
    ]
    ranked = sorted(
        candidates,
        key=lambda node: (
        ...
        plan.append(
            {
                ...
                "reason": (
                    "blocked"
                    if str(node.get("status") or "") in BLOCKED_STATUSES
                    else (
                        "stale"
    ...
    for step in plan:
        ...
        row = phases.setdefault(
            phase,
            {
                "phase": phase,
                "phase_name": str(
                    step.get("phase_name") or PHASE_NAMES.get(phase, "other")
                ),
                "step_count": 0,
                "safe_step_count": 0,
                "nodes": [],
...
def _dependency_explainability(graph: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "edge_count": _safe_int(
            graph.get("edge_count"), len(_as_list(graph.get("edges")))
        ),
...
def _repair_order_contract(repair_plan: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "safe_command_count": sum(
            1 for step in repair_plan if bool(step.get("safe_to_execute", False))
        ),
        "blocked_command_count": sum(
            1 for step in repair_plan if not bool(step.get("safe_to_execute", False))
        ),
    }


def _live_execution_firewall(
    graph: dict[str, Any], repair_plan: list[dict[str, Any]]
) -> dict[str, Any]:
    authority_violations = _as_list(graph.get("authority_violations"))
    unsafe_steps = [
        step for step in repair_plan if not bool(step.get("safe_to_execute", False))
    ]
    live_like_commands = [
        list(step.get("cmd") or [])
        for step in repair_plan
        if any(
            pattern in _command_key(step.get("cmd"))
            for pattern in LIVE_COMMAND_PATTERNS
        )
    ]
    status = "ready"
    ...
    return {
        ...
        "authority_violation_count": _safe_int(
            graph.get("authority_violation_count"), len(authority_violations)
        ),
...
def _paper_governance_split(
    project_root: Path, graph: dict[str, Any]
) -> dict[str, Any]:
    nodes = _nodes_by_id(graph)
    health = load_json(
    ...
    all_sleeves = _as_dict(
        _as_dict(health.get("process_watchdog")).get("all_sleeves_effective_runtime")
    )
    paper_statuses = {
        node_id: str(nodes.get(node_id, {}).get("status") or "missing")
        for node_id in sorted(PAPER_RUNTIME_NODES)
    }
    graph_paper_ready = bool(paper_statuses) and all(
        _readyish(status) for status in paper_statuses.values()
    )
    ...
    guarded_paper_ready = (
        health_paper_ready if health_paper_status_present else graph_paper_ready
    )
    strict_governance_ready = (
        str(graph.get("overall_status") or "").strip().lower() == "ready"
    )
    return {
        "guarded_paper_status": str(
            guarded_paper.get("status") or ("ready" if graph_paper_ready else "unknown")
        ),
        "guarded_paper_ready": guarded_paper_ready,
        "guarded_paper_blockers": _as_list(guarded_paper.get("blockers")),
        "all_sleeves_status": str(
            all_sleeves.get("status")
            or paper_statuses.get("all_sleeves_launcher")
            or "unknown"
        ),
        "all_sleeves_child_process_count": _safe_int(
            all_sleeves.get("child_process_count"), 0
        ),
        "live_execution_status": str(
            live_execution.get("status") or "blocked_read_only"
        ),
        ...
        "paper_governance_split_active": bool(
            guarded_paper_ready and not strict_governance_ready
        ),
...
def _adaptive_memory_pressure(project_root: Path) -> dict[str, Any]:
    artifact_path = (
        project_root / "governance" / "health" / "adaptive_regression_guard_latest.json"
    )
    payload = load_json(artifact_path)
    surfaces = [
        row for row in _as_list(payload.get("surfaces")) if isinstance(row, dict)
    ]
    critical = [
        {
            ...
            "adaptive_severity": str(
                row.get("adaptive_severity") or row.get("base_severity") or ""
            ),
            "summary": str(row.get("summary") or ""),
            "consecutive_non_ready_count": _safe_int(
                _as_dict(row.get("memory")).get("consecutive_non_ready_count"), 0
            ),
            "consecutive_blocked_count": _safe_int(
                _as_dict(row.get("memory")).get("consecutive_blocked_count"), 0
            ),
        }
        for row in surfaces
    ...
    degraded = [
        ...
        for row in surfaces
        if str(row.get("state") or row.get("status") or "").strip().lower()
        == "degraded"
    ]
    status = (
        "missing"
        if not payload
        else ("blocked" if critical else "tracking" if degraded else "ready")
    )
    return {
        "status": status,
        ...
        "persistent_regression_count": _safe_int(
            payload.get("persistent_regression_count"), 0
        ),
        "critical_regression_count": _safe_int(
            payload.get("critical_regression_count"), len(critical)
        ),
...
def _operator_visibility(
    project_root: Path, repair_plan: list[dict[str, Any]]
) -> dict[str, Any]:
    top_commands = [
        list(step.get("cmd") or []) for step in repair_plan[:8] if step.get("cmd")
    ]
    return {
        "artifact_paths": {
            "latest": str(
                ...
                / "governance"
                / "health"
                / "system_architecture_autopilot_latest.json"
            ),
            "plan": str(
                DEFAULT_PLAN_PATH
                if project_root == PROJECT_ROOT
        ...
        "operator_commands": {
            ...
            "graph_status": [
                "./scripts/ops/opsctl.sh",
                "system-architecture-contract-graph",
                "--apply",
                "--json",
            ],
            "phone_summary": [
...
def _architecture_expansion_layers(
    ...
    layers = [
        ...
        },
        {
            ...
            "status": (
                "ready"
                if degradation["root_non_ready_nodes"] or final_graph
                else "missing"
            ),
        ...
        },
        {
            ...
            "operator_command": operator_visibility["operator_commands"][
                "phone_summary"
            ],
        },
    ]
    return {
        ...
        "ready_layer_count": sum(
            1
            for layer in layers
            if layer["status"] in {"ready", "tracking", "guarded", "idle"}
        ),
        "blocked_layer_count": sum(
            1 for layer in layers if layer["status"] == "blocked"
        ),
        "layers": layers,
...
def _runtime_pressure_evidence(project_root: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "bot_owned_cpu_percent": _safe_float(
            measurements.get("bot_owned_cpu_percent"), 0.0
        ),
        "storage_writer_cpu_percent": _safe_float(
            measurements.get("storage_writer_cpu_percent"), 0.0
        ),
        "paper_execution_cpu_percent": _safe_float(
            measurements.get("paper_execution_cpu_percent"), 0.0
        ),
        ...
        "paper_execution_paused": bool(
            measurements.get("paper_execution_paused", False)
        ),
        "paper_ramp_armed": bool(measurements.get("paper_ramp_armed", False)),
        "bot_owned_pressure_dominant": bool(
            measurements.get("bot_owned_pressure_dominant", False)
        ),
    }
...
def _signal_context(
    ...
    critical_surfaces = {
        ...
        for row in _as_list(adaptive.get("critical_surfaces"))
        if isinstance(row, dict)
    }
    degraded_surfaces = [
        row
        for row in _as_list(adaptive.get("degraded_surfaces"))
        if isinstance(row, dict)
    ]
    signals = {
        "architecture_blocked": str(final_graph.get("overall_status") or "")
        == "blocked",
        "repair_plan_present": bool(repair_plan),
        "safe_repairs_present": any(
            bool(step.get("safe_to_execute", False)) for step in repair_plan
        ),
        "guarded_paper_blocked_runtime": (
            ...
            in {
                str(item)
                for item in _as_list(paper_split.get("guarded_paper_blockers"))
            }
        ),
        ...
        "storage_backpressure_apply_failed": str(
            storage_autopilot.get("overall_status") or ""
        )
        == "apply_failed",
        ...
        "adaptive_degraded_stale_surfaces": any(
            "stale" in str(row.get("summary") or "").lower()
            for row in degraded_surfaces
        ),
        "system_drift_blocked": "system_drift_guard"
        in {str(item) for item in _as_list(final_graph.get("blocked_nodes"))},
        "operator_visibility_needed": True,
        "bot_owned_pressure_dominant": bool(
            runtime.get("bot_owned_pressure_dominant", False)
        )
        or _safe_float(runtime.get("bot_owned_cpu_percent"), 0.0) >= 200.0,
        "active_regression_count_high": _safe_int(
            adaptive.get("active_regression_count"), 0
        )
        >= 5,
    }
    return {
        ...
        "adaptive": {
            ...
            "active_regression_count": _safe_int(
                adaptive.get("active_regression_count"), 0
            ),
            "persistent_regression_count": _safe_int(
                adaptive.get("persistent_regression_count"), 0
            ),
            "critical_regression_count": _safe_int(
                adaptive.get("critical_regression_count"), 0
            ),
        ...
        "paper": {
            ...
            "paper_contract_ready": bool(
                paper_split.get("paper_contract_ready", False)
            ),
            "all_sleeves_status": str(paper_split.get("all_sleeves_status") or ""),
            "all_sleeves_child_process_count": _safe_int(
                paper_split.get("all_sleeves_child_process_count"), 0
            ),
        },
    }
...
def _score_blueprint(
    blueprint: dict[str, Any], signal_context: dict[str, Any]
) -> dict[str, Any]:
    ...
    if (
        matched
        and str(blueprint.get("candidate_id") or "") == "paper_runtime_load_shedder"
    ):
        runtime = _as_dict(signal_context.get("runtime"))
        score += min(
            int(_safe_float(runtime.get("paper_execution_cpu_percent"), 0.0) // 10), 10
        )
    if (
        matched
        and str(blueprint.get("candidate_id") or "") == "governance_closeout_lane"
    ):
    ...
    return {
        ...
        "safe_commands": [
            list(cmd)
            for cmd in _as_list(blueprint.get("safe_commands"))
            if isinstance(cmd, list)
        ],
        "guards": [str(item) for item in _as_list(blueprint.get("guards"))],
        "acceptance_criteria": [
            str(item) for item in _as_list(blueprint.get("acceptance_criteria"))
        ],
        "authority_contract": {
...
def _architecture_benefit_backlog(
    ...
    candidates = [
        _score_blueprint(blueprint, signal_context) for blueprint in BENEFIT_BLUEPRINTS
    ]
    ranked = sorted(
        candidates,
        key=lambda row: (
            -_safe_int(row.get("score"), 0),
            _safe_int(row.get("phase"), 9),
            str(row.get("candidate_id") or ""),
        ),
    )
    active = [row for row in ranked if str(row.get("status") or "") == "active"]
    recommended = [
        row for row in ranked if str(row.get("status") or "") == "recommended"
    ]
...
def build_payload(
    ...
    *,
    ...
    executable_plan = [
        step for step in repair_plan if bool(step.get("safe_to_execute", False))
    ]
    ...
    if apply and execute_safe_repairs:
        for step in executable_plan:
            timeout_sec = min(
                _safe_int(step.get("timeout_sec"), max_step_timeout_sec),
                max(int(max_step_timeout_sec), 1),
            )
            result = run_step(list(step["cmd"]), project_root, timeout_sec)
            payload = _as_dict(result.get("payload"))
            accepted_non_ready = _expected_earned_evidence_result(
                str(step.get("node_id") or ""), result
            )
            ...
            attempts.append(
                {
                    ...
                    "accepted_non_ready": accepted_non_ready,
                    "outcome": (
                        "success"
                        if rc == 0
                        else (
                            "expected_earned_evidence_debt"
    ...
    recommended_commands = [
        list(step["cmd"]) for step in repair_plan if step.get("cmd")
    ]
    expansion = _architecture_expansion_layers(project_root, final_graph, repair_plan)
    benefit_backlog = _architecture_benefit_backlog(
        project_root, final_graph, expansion, repair_plan
    )
    payload = {
        ...
        "successful_attempt_count": sum(
            1 for row in attempts if _safe_int(row.get("rc"), 1) == 0
        ),
        "accepted_non_ready_attempt_count": sum(
            1 for row in attempts if bool(row.get("accepted_non_ready", False))
        ),
        "failed_attempt_count": sum(
            1
            for row in attempts
            if _safe_int(row.get("rc"), 1) != 0
            and not bool(row.get("accepted_non_ready", False))
        ),
        "hard_failed_attempt_count": sum(
            1
            for row in attempts
            if _safe_int(row.get("rc"), 1) != 0
            and not bool(row.get("accepted_non_ready", False))
        ...
        "initial_graph": {
            ...
            "degraded_node_count": _safe_int(
                initial_graph.get("degraded_node_count"), 0
            ),
            ...
            "authority_violation_count": _safe_int(
                initial_graph.get("authority_violation_count"), 0
            ),
            "blocked_nodes": [
                str(item) for item in _as_list(initial_graph.get("blocked_nodes"))
            ],
            "degraded_nodes": [
                str(item) for item in _as_list(initial_graph.get("degraded_nodes"))
            ],
            "stale_nodes": [
                str(item) for item in _as_list(initial_graph.get("stale_nodes"))
            ],
        },
        "final_graph": {
            "overall_status": final_status,
            ...
            "authority_violation_count": _safe_int(
                final_graph.get("authority_violation_count"), 0
            ),
            "blocked_nodes": [
                str(item) for item in _as_list(final_graph.get("blocked_nodes"))
            ],
            "degraded_nodes": [
                str(item) for item in _as_list(final_graph.get("degraded_nodes"))
            ],
            "stale_nodes": [
                str(item) for item in _as_list(final_graph.get("stale_nodes"))
            ],
        },
        ...
        "architecture_expansion_summary": {
            ...
            "paper_governance_split_active": bool(
                expansion["paper_governance_split"].get(
                    "paper_governance_split_active", False
                )
            ),
            "live_execution_firewall_status": str(
                expansion["live_execution_firewall"].get("status") or ""
            ),
            "adaptive_memory_status": str(
                expansion["adaptive_memory_pressure"].get("status") or ""
            ),
        },
        "architecture_benefit_summary": {
            ...
            "active_candidate_count": _safe_int(
                benefit_backlog.get("active_candidate_count"), 0
            ),
            "recommended_candidate_count": _safe_int(
                benefit_backlog.get("recommended_candidate_count"), 0
            ),
            "top_candidate_id": str(benefit_backlog.get("top_candidate_id") or ""),
            "top_candidate_score": _safe_int(
                benefit_backlog.get("top_candidate_score"), 0
            ),
            "top_candidate_title": str(
                benefit_backlog.get("top_candidate_title") or ""
            ),
        },
        ...
        "recommended_actions": ordered_unique(
            ...
            + [
                f"phase {row['phase']}: {row['phase_name']} has {row['step_count']} step(s)"
                for row in _phase_rollup(repair_plan)
            ]
        ),
    }
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Plan dependency-ordered architecture repairs from the system architecture contract graph."
    )
