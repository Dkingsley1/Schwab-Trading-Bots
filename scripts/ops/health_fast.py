...
def _alert_details(alerts: list[Any]) -> dict[str, Any]:
    ...
    for raw in alerts:
        ...
        severity = _status(
            alert.get("severity")
            or nested.get("severity")
            or row.get("severity")
            or "warn"
        )
        event = str(
            alert.get("event") or nested.get("event") or row.get("type") or ""
        ).strip()
        target = str(
            row.get("name") or row.get("target") or nested.get("target") or ""
        ).strip()
        rows.append(
            {
                "target": target,
...
def _paper_hot_path_storage_relief(
    storage: dict[str, Any], plumbing: dict[str, Any] | None = None
) -> dict[str, Any]:
    ...
    plumbing_managed_deferred = bool(
        plumbing_paper_relief.get("managed_deferred_backlog", False)
    )
    hot_path_ok = bool(
        plumbing_managed_deferred
        or plumbing_raw_live.get("ok", False)
        or (
            _status(plumbing_raw_live.get("status")) in {"ready", "ok", "advisory"}
            and _safe_int(plumbing_raw_live.get("core_pending_lines"), core_pending)
            <= _safe_int(plumbing_raw_live.get("max_core_pending_lines"), 5000)
            and _safe_int(
                plumbing_raw_live.get("total_pending_lines"),
                core_pending + support_pending,
            )
            <= _safe_int(plumbing_raw_live.get("max_total_pending_lines"), 15000)
            and _safe_float(plumbing_raw_live.get("oldest_pending_age_seconds"), 0.0)
            <= _safe_float(
                plumbing_raw_live.get("max_oldest_pending_age_seconds"), 900.0
            )
        )
    )
    if not plumbing_raw_live and not plumbing_managed_deferred:
        hot_path_ok = bool(
            core_pending <= 5000 and support_pending <= 12000 and oldest <= 3600.0
        )
    backlog_status = _status(
    ...
    return {
        ...
        "status": (
            ...
            if active and small_residual_drain_managed
                ...
                "managed_deferred_backlog_waiting_for_off_hours"
                if active
                else "inactive"
            )
        ),
...
def _storage_ready(
    ...
    severity = _status(
        storage.get("severity") or storage.get("overall_status") or "stable"
    )
    ...
    raw_live = (
        effective_raw_live
        if use_effective and effective_raw_live
        else (
    ...
    storage_section = _dict(storage.get("storage"))
    efficiency_status = _status(
        efficiency.get("overall_status") or storage.get("storage_efficiency_status")
    )
    efficiency_grade = (
        str(
            efficiency.get("grade")
            or storage_section.get("efficiency_grade")
            or storage.get("storage_efficiency_grade")
            or ""
        )
        .strip()
        .upper()
    ...
    bounded_drain_relief = bool(
        ...
        and total_pending < pending_threshold
        and bool(
            bounded_recovery.get("active_drain_progress")
            or bounded_recovery.get("drain_delta_signal_observed")
        )
    ...
    if total_pending >= pending_threshold and not bool(
        deferred_relief.get("active", False)
    ):
        blockers.append("storage_pending_above_threshold")
    return not blockers, blockers


def _collection_rollup_advisory_ready(
    rollup: dict[str, Any], rollup_status: str
) -> bool:
...
def _platform_repair_contract(
    *,
    ...
    managed_source_statuses: list[dict[str, Any]] = []
    source_freshness = {}
    for name, payload in sources.items():
        if not payload:
            source_freshness[name] = evidence_freshness(payload)
            continue
        ...
        if status_managed_by_owner_ok:
            managed_source_statuses.append(
                {
                    "source": name,
                    "overall_status": status,
                    "reason": "owner_reports_ok_true_and_no_guarded_paper_blockers",
                }
            )
            continue
        if not freshness["fresh"] and status not in {
            "ready",
            "ok",
            "guarded_ready",
            "watch",
        ...
        if status in {"blocked", "critical", "needs_work", "degraded"}:
            ...
            issues.append(
                {"source": name, "overall_status": status, "next_best_command": command}
            )
    return {
        "ok": not issues,
...
def _collector_repair_contract(
    alert_summary: dict[str, Any], isolated_contract: dict[str, Any]
) -> dict[str, Any]:
    ...
    execution_blocking_count = _safe_int(
        isolated_contract.get("execution_blocking_count"), 0
    )
    isolated_targets = [
        str(item)
        for item in _as_list(isolated_contract.get("isolated_targets"))
        if str(item).strip()
    ]
    isolated_target_set = set(isolated_targets)
    rows = [
        row if isinstance(row, dict) else {}
        for row in _as_list(alert_summary.get("rows"))
    ]
    warning_rows = [
        row for row in rows if not bool(row.get("blocks_guarded_paper", False))
    ]
    ...
    return {
        ...
        "status": (
            "ready"
            if ready
            else ("managed_isolated" if managed_isolated else "needs_repair")
        ),
...
def _all_sleeves_effective_runtime(process: dict[str, Any]) -> dict[str, Any]:
    rows = [
        row if isinstance(row, dict) else {} for row in _as_list(process.get("status"))
    ]
    for row in rows:
        if str(row.get("name") or "") != "all_sleeves":
            continue
        child_fanout = (
            row.get("child_fanout") if isinstance(row.get("child_fanout"), dict) else {}
        )
        launcher_artifact = (
        ...
        child_count = _safe_int(
            child_fanout.get("child_process_count"),
            _safe_int(row.get("alt_running"), 0),
        )
        effective_live = bool(
        ...
        ok = bool(
            effective_live and heartbeat_ok and child_fanout_ok and child_count > 0
        )
        return {
            "ok": ok,
...
def build_payload(project_root: Path = PROJECT_ROOT) -> dict[str, Any]:
    ...
    halt = _health(project_root, "global_halt_auto_clear_latest.json") or _health(
        project_root, "global_killswitch_latest.json"
    )
    ...
    schwab_futures = _health(
        project_root, "data_ingress_latest_schwab_futures_equities_schwab.json"
    )
    ...
    commercial_readiness = _health(
        project_root, "commercial_readiness_control_latest.json"
    )
    ...
    safety = (
        ...
        if isinstance(process.get("safety_pause"), dict)
        else {}
    )
    swap_payload = (
        swap.get("swap_pressure") if isinstance(swap.get("swap_pressure"), dict) else {}
    )
    ...
    halt_blockers = [
        str(item) for item in _as_list(halt.get("clear_blockers")) if str(item).strip()
    ]
    ...
    paper_ramp_gates = (
        paper_ramp.get("gates") if isinstance(paper_ramp.get("gates"), dict) else {}
    )
    paper_runtime_gate = (
        paper_ramp_gates.get("runtime")
        if isinstance(paper_ramp_gates.get("runtime"), dict)
        else {}
    )
    paper_memory_gate = (
        paper_ramp_gates.get("memory")
        if isinstance(paper_ramp_gates.get("memory"), dict)
        else {}
    )
    paper_global_halt_gate = (
        paper_ramp.get("gates", {}).get("global_halt")
        if isinstance(paper_ramp.get("gates"), dict)
        else {}
    )
    paper_clear_relief = (
        paper_global_halt_gate.get("clear_blocker_relief")
        if isinstance(paper_global_halt_gate, dict)
        and isinstance(paper_global_halt_gate.get("clear_blocker_relief"), dict)
        else {}
    )
    advisory_halt_blockers = (
        ...
        if bool(paper_clear_relief.get("active", False))
        and bool(paper_global_halt_gate.get("ok", False))
        else set()
    )
    if bool(storage_relief.get("active", False)) and not bool(halt.get("halt", False)):
        advisory_halt_blockers.update(
            {"queue_backpressure_active", "write_path_recovery_pending"}
        )
    blocking_halt_blockers = [
        item for item in halt_blockers if item not in advisory_halt_blockers
    ]
    ...
    plumbing_sections = (
        plumbing.get("sections") if isinstance(plumbing.get("sections"), dict) else {}
    )
    plumbing_runtime = (
        plumbing_sections.get("runtime_memory")
        if isinstance(plumbing_sections.get("runtime_memory"), dict)
        else {}
    )
    ...
    guarded_paper_blockers.extend(
        f"global_clear_blocker={item}" for item in blocking_halt_blockers
    )
    ...
    if (
        paper_ramp_present
        and (paper_ramp_stage not in {"armed", "ready"} or paper_ramp_blockers)
        and not paper_ramp_stale_global_blocker
    ):
    ...
    strict_ready = bool(
        guarded_paper_ready and collector_repair["ok"] and platform_repair["ok"]
    )
    overall_status = (
        "ready"
        if strict_ready
        else ("guarded_ready" if guarded_paper_ready else "degraded")
    )
    ...
    return {
        ...
        "operational_readiness": {
            ...
            "live_execution": {
                ...
                "blockers": [
                    "health_fast_is_read_only",
                    "live_execution_requires_explicit_operator_control",
                ],
        ...
        "collection": {
            ...
            "bots_with_observations": _safe_int(
                rollup.get("bots_with_observations"), 0
            ),
            "effective_bots_with_observations": _safe_int(
                rollup.get(
                    "effective_bots_with_observations",
                    rollup.get("bots_with_observations"),
                ),
                0,
            ),
            "zero_observation_count": _safe_int(
                rollup.get("zero_observation_count"), 0
            ),
            "unmanaged_zero_observation_count": _safe_int(
                rollup.get(
                    "unmanaged_zero_observation_count",
                    rollup.get("zero_observation_count"),
                ),
                0,
            ),
            "managed_zero_observation_count": _safe_int(
                rollup.get("managed_zero_observation_count"), 0
            ),
            "raw_zero_observation_count": _safe_int(
                rollup.get(
                    "raw_zero_observation_count", rollup.get("zero_observation_count")
                ),
                0,
            ),
        ...
        "platform_brain_v4": {
            ...
            "next_best_command": (
                (
                    (brain_v4.get("sections") or {}).get("executive_meta_orchestrator")
                    or {}
                ).get("next_best_command")
                if isinstance(brain_v4.get("sections"), dict)
                else ""
            ),
        },
        "platform_brain_v5": {
            ...
            "next_best_command": (
                (
                    (brain_v5.get("sections") or {}).get("reflex_action_router") or {}
                ).get("next_best_command")
                if isinstance(brain_v5.get("sections"), dict)
                else ""
            ),
        },
        "platform_stabilization_quality": {
            ...
            "expansion_allowed_now": (
                (
                    (stabilizer.get("sections") or {}).get("expansion_rehearsal_gate")
                    or {}
                ).get("expansion_allowed_now")
                if isinstance(stabilizer.get("sections"), dict)
                else None
            ),
        },
        "platform_settlement_stabilization": {
            ...
            "queue_backpressure_active": (
                ((settlement.get("sections") or {}).get("queue_decay_meter") or {}).get(
                    "queue_backpressure_active"
                )
                if isinstance(settlement.get("sections"), dict)
                else None
            ),
            "global_clear_status": (
                (
                    (settlement.get("sections") or {}).get(
        ...
        "system_architecture_hardening": {
            ...
            "watch_section_count": _safe_int(
                architecture.get("watch_section_count"), 0
            ),
        ...
        "system_plumbing_control": {
            ...
            "root_cause": (
                plumbing.get("root_cause", {})
                if isinstance(plumbing.get("root_cause"), dict)
                else {}
            ),
            "next_best_command": plumbing.get("next_best_command", ""),
            "global_clear_relief": (
                plumbing.get("global_clear_relief", {})
                if isinstance(plumbing.get("global_clear_relief"), dict)
                else {}
            ),
        },
        "schwab_futures": {
        ...
        "use_mode_compliance": {
            ...
            "perfect_personal_use_ready": bool(
                _dict(use_mode.get("personal_use")).get(
            ...
            "personal_strength_tier": _dict(
                _dict(use_mode.get("personal_use")).get(
                    "operator_grade_personal_autonomy"
                )
            ).get("tier"),
            "personal_strength_score": _safe_float(
                _dict(
                    _dict(use_mode.get("personal_use")).get(
                        "operator_grade_personal_autonomy"
                    )
            ...
            "personal_strength_blockers": _dict(
                _dict(use_mode.get("personal_use")).get(
                    "operator_grade_personal_autonomy"
                )
            ).get("blockers", []),
            "commercial_use_intent_detected": bool(
                _dict(use_mode.get("commercial_use")).get(
                    "commercial_use_intent_detected", False
                )
            ),
            "commercial_clearance_status": _dict(use_mode.get("commercial_use")).get(
                "commercial_clearance_status"
            ),
            "commercial_blockers": _dict(use_mode.get("commercial_use")).get(
                "blockers", []
            ),
            "live_execution_authority": bool(
                _dict(use_mode.get("authority_boundaries")).get(
        ...
        "commercial_readiness": {
            ...
            "commercial_product_mode": commercial_readiness.get(
                "commercial_product_mode"
            )
            or "personal_only",
            "commercial_intent": bool(
                commercial_readiness.get("commercial_intent", False)
            ),
            "commercial_release_ready": bool(
                commercial_readiness.get("commercial_release_ready", False)
            ),
            "commercial_release_blocked": bool(
                commercial_readiness.get("commercial_release_blocked", False)
            ),
            "grade": commercial_readiness.get("grade"),
            "ready_section_count": _safe_int(
                commercial_readiness.get("ready_section_count"), 0
            ),
            "section_count": _safe_int(commercial_readiness.get("section_count"), 0),
            "blockers": commercial_readiness.get("blockers", []),
            "live_execution_authority": bool(
                _dict(commercial_readiness.get("authority_boundaries")).get(
                    "live_execution_authority", False
                )
            ),
        ...
        "recommended_commands": [
            *(
                ...
                if collector_repair["status"] == "needs_repair"
                else []
            ),
            *(
                [["./scripts/ops/opsctl.sh", "pressure-relief", "--apply", "--json"]]
                if not platform_repair["ok"]
                else []
            ),
            *(
                [plumbing.get("root_cause", {}).get("next_command")]
                if isinstance(plumbing.get("root_cause"), dict)
                and isinstance(plumbing.get("root_cause", {}).get("next_command"), list)
                else []
            ),
            *(
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Fast read-only health summary. Does not refresh reports."
    )
