...
DEFAULT_LOCK_PATH = (
    PROJECT_ROOT / "governance" / "locks" / "backpressure_drainer_fleet.lock"
)
...
RAW_LIVE_EXPANSION_SUPPORT_DRAINERS = {
...
def _storage_control_fresh_for_backpressure(
    storage_control: dict[str, Any],
    backpressure: dict[str, Any],
    *,
    tolerance_seconds: float = 2.0,
) -> bool:
    """Prevent an older aggregate snapshot from resurrecting already-drained work."""
    storage_ts = _parse_iso_utc(storage_control.get("timestamp_utc"))
    backpressure_ts = _parse_iso_utc(backpressure.get("timestamp_utc"))
    if storage_ts is None or backpressure_ts is None:
        return True
    return (
        storage_ts.timestamp() + max(float(tolerance_seconds), 0.0)
        >= backpressure_ts.timestamp()
    )
...
def _p_core_foreground_reserve(
    *, p_cores: int, host_context: dict[str, Any] | None = None
) -> int:
    ...
    if any(
        ...
        return 3
    if any(
        _text_in(value, ("yield", "foreground", "music", "developer", "browser"))
        for value in (intent, primary_task, creative_kind)
    ):
        return 2
    return 1
...
def _p_core_preprocess_workers(
    *,
    critical: bool,
    ...
    foreground_reserve = _p_core_foreground_reserve(
        p_cores=p_cores, host_context=host_context
    )
    writer_reserve = 1
    ...
    age_threshold = max(
        _safe_float(pressure.get("oldest_age_threshold_seconds"), 240.0), 1.0
    )
    line_estimation = (
        pressure.get("line_estimation")
        if isinstance(pressure.get("line_estimation"), dict)
        else {}
    )
    backlog_ratio = max(
        core / max(threshold, 1), total / max(threshold, 1), oldest / age_threshold
    )
...
def _p_core_backlog_allocation_contract(
    env: dict[str, str], sparse_pressure: dict[str, Any] | None = None
) -> dict[str, Any]:
    sparse = sparse_pressure if isinstance(sparse_pressure, dict) else {}
    workers = _safe_int(
        env.get("BACKLOG_PCORE_PREPROCESS_WORKERS"),
        _safe_int(env.get("SQL_LINK_SERVICE_PREPROCESS_WORKERS"), 1),
    )
    shard_writer_lanes = _safe_int(
        env.get("SQL_LINK_SERVICE_SHARD_WRITER_LANES"), workers
    )
    max_shard_writer_lanes = _safe_int(
        env.get("SQL_LINK_SERVICE_MAX_SHARD_WRITER_LANES"), max(shard_writer_lanes, 1)
    )
    return {
        ...
        "single_writer_only": str(env.get("BACKLOG_DRAIN_SINGLE_WRITER_ONLY") or "")
        == "1",
        "avoid_background_taskpolicy": str(
            env.get("RUNTIME_THROTTLE_USE_TASKPOLICY_BACKGROUND") or "0"
        )
        != "1",
        "performance_core_primary": str(
            env.get("BOT_CPU_ALLOCATION_POLICY") or "performance_core_primary"
        ).startswith("performance_core"),
        "training_pcore_gate": {
            "allowed_when_backlog_green": str(
                env.get("TRAINING_PCORE_ALLOWED_WHEN_BACKLOG_GREEN") or ""
            )
            == "1",
        ...
        "sparse_huge_jsonl": {
            ...
            "estimated_pending_bytes": _safe_int(
                sparse.get("estimated_pending_bytes"), 0
            ),
...
def _pending_source_row(
    source_rel: str, row: dict[str, Any], *, pending_lines: int, age_seconds: float
) -> dict[str, Any]:
    out: dict[str, Any] = {
...
def _merge_pending_source_metadata(
    current: dict[str, Any], row: dict[str, Any]
) -> None:
    for key in (
        "shard",
        "stream",
    ...
    current["file_size_bytes"] = max(
        _safe_int(current.get("file_size_bytes"), 0),
        _safe_int(row.get("file_size_bytes"), 0),
    )
...
def _collect_sources(
    backpressure: dict[str, Any], prefixes: tuple[str, ...], *, keys: tuple[str, ...]
) -> list[dict[str, Any]]:
    ...
    for key in keys:
        for row in _rows_from(backpressure, key):
            ...
            if pending_lines <= 0:
                continue
            age_seconds = max(
                _safe_float(row.get("oldest_pending_age_seconds"), 0.0), 0.0
            )
            current = by_source.get(source_rel)
            if current is None:
                by_source[source_rel] = _pending_source_row(
                    source_rel,
                    row,
                ...
                continue
            current["pending_lines"] = max(
                _safe_int(current.get("pending_lines"), 0), pending_lines
            )
...
def _stale_decision_rows_from_storage_control(
    storage_control: dict[str, Any],
) -> list[dict[str, Any]]:
    locator = (
        storage_control.get("stale_pending_locator")
        if isinstance(storage_control.get("stale_pending_locator"), dict)
        else {}
    )
    rows = (
        locator.get("oldest_sources")
        if isinstance(locator.get("oldest_sources"), list)
        else []
    )
    by_source: dict[str, dict[str, Any]] = {}
    for row in rows:
        ...
        source_rel = str(row.get("source_rel") or "").strip()
        if not (
            source_rel.startswith("decisions/")
            or source_rel.startswith("governance/channels/decision/")
        ):
            continue
        pending_lines = max(_safe_int(row.get("pending_lines"), 0), 0)
        ...
        if current is None:
            by_source[source_rel] = _pending_source_row(
                source_rel, row, pending_lines=pending_lines, age_seconds=age_seconds
            )
        else:
            current["pending_lines"] = max(
                _safe_int(current.get("pending_lines"), 0), pending_lines
...
def _stale_decision_rows_from_backpressure(
    backpressure: dict[str, Any],
) -> list[dict[str, Any]]:
    age_threshold = max(
        _safe_float(
            backpressure.get("oldest_age_threshold_seconds"),
            DEFAULT_AGE_PRESSURE_THRESHOLD_SECONDS,
        ),
        1.0,
    )
    rows = (
...
def _overlay_rows_from_storage_control(
    storage_control: dict[str, Any], prefixes: tuple[str, ...]
) -> list[dict[str, Any]]:
    overlay = (
    ...
    rows = (
        overlay.get("top_pending_files")
        if isinstance(overlay.get("top_pending_files"), list)
        else []
    )
    by_source: dict[str, dict[str, Any]] = {}
    for row in rows:
        ...
        current["oldest_pending_age_seconds"] = round(
            max(
                _safe_float(current.get("oldest_pending_age_seconds"), 0.0), age_seconds
            ),
            3,
        )
        _merge_pending_source_metadata(current, row)
...
def _storage_overlay_authoritative(storage_control: dict[str, Any]) -> bool:
    ...
    if not bool(overlay.get("active", False)) or not bool(
        overlay.get("used_for_pressure", False)
    ):
        return False
    if (
        _safe_int(overlay.get("source_count"), 0) <= 0
        or _safe_int(overlay.get("fresh_source_count"), 0) <= 0
    ):
        return False
    truth = (
        storage_control.get("backlog_truth")
        if isinstance(storage_control.get("backlog_truth"), dict)
        else {}
    )
    ...
    if mode and not mode.startswith("overlay"):
        return False
    decay = (
        truth.get("overlay_decay")
        if isinstance(truth.get("overlay_decay"), dict)
        else {}
    )
...
def _storage_overlay_freshly_covers_prefixes(
    storage_control: dict[str, Any], prefixes: tuple[str, ...]
) -> bool:
    ...
    fresh_path_contains = (
        overlay.get("fresh_path_contains")
        if isinstance(overlay.get("fresh_path_contains"), list)
        else []
    )
    fresh_markers = [
        str(item or "").strip().lower()
        for item in fresh_path_contains
    ...
    for prefix in clean_prefixes:
        if any(
            marker.startswith(prefix) or prefix.startswith(marker)
...
def _preferred_source_rows(
    ...
    *,
    ...
    effective_storage = (
        storage_control
        if _storage_control_fresh_for_backpressure(storage_control, backpressure)
        else {}
    )
    overlay_rows = _overlay_rows_from_storage_control(effective_storage, prefixes)
    storage_raw_rows = _collect_sources(
        _storage_raw_live_backpressure(effective_storage), prefixes, keys=keys
    )
    if not overlay_rows and _storage_overlay_freshly_covers_prefixes(
        effective_storage, prefixes
    ):
        return storage_raw_rows
    raw_rows = _merge_source_rows(
        _collect_sources(backpressure, prefixes, keys=keys), storage_raw_rows
    )
    if tuple(keys) == ("top_pending_files",) and overlay_rows:
        raw_core_pending = max(_safe_int(backpressure.get("pending_lines"), 0), 0)
        overlay_pending = sum(
            max(_safe_int(row.get("pending_lines"), 0), 0) for row in overlay_rows
        )
        overlay = (
            effective_storage.get("sql_ingestion_pending_overlay")
            if isinstance(effective_storage.get("sql_ingestion_pending_overlay"), dict)
            else {}
        )
        fresh_path_contains = (
            overlay.get("fresh_path_contains")
        ...
        has_explicit_freshness_scope = any(
            str(item or "").strip() for item in fresh_path_contains
        )
        if (
            raw_core_pending < overlay_pending <= CORE_HARD_PENDING_LINES
            and not has_explicit_freshness_scope
        ):
            return raw_rows
    if _storage_overlay_authoritative(effective_storage) and overlay_rows:
        return overlay_rows
...
def _stale_sticky_decision_rows(
    core_rows: list[dict[str, Any]], stale_decision_rows: list[dict[str, Any]]
) -> list[dict[str, Any]]:
    stale_sources = {
        str(row.get("source_rel") or "").strip()
        for row in stale_decision_rows
    ...
    return [
        ...
        *[
            ...
            if str(row.get("source_rel") or "").strip() not in stale_sources
        ],
    ]
...
def _concentration_summary(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    top = sorted(
        (max(_safe_int(row.get("pending_lines"), 0), 0) for row in rows), reverse=True
    )
...
def _sparse_large_line_pressure(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for row in rows:
        ...
        if (
            ...
            or avg_bytes >= SPARSE_LARGE_LINE_BYTES_PER_LINE
        ):
            sparse_rows.append(row)
    pending_lines = sum(
        max(_safe_int(row.get("pending_lines"), 0), 0) for row in sparse_rows
    )
    file_bytes = sum(
        max(_safe_int(row.get("file_size_bytes"), 0), 0) for row in sparse_rows
    )
    pending_bytes = sum(
        max(_safe_int(row.get("estimated_pending_bytes"), 0), 0) for row in sparse_rows
    )
    top = sorted(
        sparse_rows,
        key=lambda row: (
        ...
        "top_files": [
            {
                ...
                "estimated_pending_bytes": _safe_int(
                    row.get("estimated_pending_bytes"), 0
                ),
                "estimated_avg_bytes_per_line": round(
                    _safe_float(row.get("estimated_avg_bytes_per_line"), 0.0), 3
                ),
            }
            for row in top[:5]
        ],
...
def _base_env(
    *,
    ...
    line_estimation = (
        raw_live.get("line_estimation")
        if isinstance(raw_live.get("line_estimation"), dict)
        else (
            bp.get("line_estimation")
            if isinstance(bp.get("line_estimation"), dict)
            else {}
        )
    )
    ...
    oldest_threshold = max(
        _safe_float(
            bp.get("oldest_age_threshold_seconds"),
            DEFAULT_AGE_PRESSURE_THRESHOLD_SECONDS,
        ),
        1.0,
    )
    core_pending = _safe_int(
        bp.get("core_pending_lines"), _safe_int(raw_live.get("core_pending_lines"), 0)
    )
    total_pending = _safe_int(
        bp.get("total_pending_lines"), _safe_int(raw_live.get("total_pending_lines"), 0)
    )
    oldest_age = _safe_float(
        bp.get("oldest_pending_age_seconds"),
    ...
    accelerator_wave_limit = (
        6
        if critical
        and p_core_workers >= 4
        and (
            sparse_active
            or sparse_pending_bytes >= 64 * 1024 * 1024
            or backlog_ratio >= 20.0
        )
        else 5 if critical and p_core_workers >= 4 else 3 if critical else 2
    )
    accelerator_merge_seconds = (
        150
        if accelerator_wave_limit >= 6
        else 120 if accelerator_wave_limit >= 5 else 90 if critical else 60
    )
    accelerator_shard_timeout = (
        480 if accelerator_wave_limit >= 6 else 420 if critical else 150
    )
    return {
        ...
        "BACKLOG_ACCELERATOR_MODE": (
            "drainer_pcore_wave_6"
            if accelerator_wave_limit >= 6
            else (
                "drainer_pcore_wave_5"
                if accelerator_wave_limit >= 5
                else "drainer_bounded_wave"
            )
        ),
...
def _profile_self_accommodation(
    *,
    ...
    return {
        ...
        "live_window_behavior": (
            "live_window_safe" if live_window_safe else "protected_window_or_force_only"
        ),
...
def _profile(
    *,
    ...
    oldest_age = max(
        [_safe_float(row.get("oldest_pending_age_seconds"), 0.0) for row in rows]
        or [0.0]
    )
    path_focus = [
        str(row.get("source_rel") or "").strip()
    ...
    stale_tail_ready = bool(
        pending_lines > 0
        and stale_ready_age_seconds > 0.0
        and oldest_age >= stale_ready_age_seconds
    )
    readiness_reason = (
        "material_pending"
        if material_ready
        else ("stale_tail" if stale_tail_ready else "below_threshold")
    )
    return {
        "name": name,
        "reason": reason,
        ...
        "p_core_backlog_allocation_contract": _p_core_backlog_allocation_contract(
            env, sparse_pressure
        ),
...
def _raw_live_expansion_guard(
    ...
    *,
    ...
    target_core = max(
        _safe_int(os.getenv("BACKPRESSURE_TARGET_CORE_PENDING_LINES"), 5_000), 1
    )
    reserve_core = max(
        _safe_int(
            ...
            (
                _safe_int(
                    (existing.get("targets") or {}).get("core_reserve_lines"),
    ...
    reserve_total = max(
        _safe_int(
            ...
            (
                _safe_int(
                    (existing.get("targets") or {}).get("total_reserve_lines"),
    ...
    reserve_age = max(
        _safe_float(
            ...
            (
                _safe_float(
                    (existing.get("targets") or {}).get("oldest_age_reserve_seconds"),
                    float(age_threshold) * 0.75,
                )
                if isinstance(existing.get("targets"), dict)
    ...
    storage_raw_live = (
        _storage_raw_live_backpressure(storage)
        if _storage_control_fresh_for_backpressure(storage, backpressure)
        else {}
    )
        ...
        for key in (
            ...
            for row in rows:
                ...
                if any(marker in rel for marker in hot_source_markers):
                    ...
                    if pending > 0:
                        current = hot_rows_by_source.setdefault(
                            rel, {"pending_lines": 0, "oldest_pending_age_seconds": 0.0}
                        )
                        current["pending_lines"] = max(
                            _safe_int(current.get("pending_lines"), 0), pending
                        )
                        ...
                        if not rel.startswith(age_guard_excluded_markers):
                            ...
                            age_current["pending_lines"] = max(
                                _safe_int(age_current.get("pending_lines"), 0), pending
                            )
                            age_current["oldest_pending_age_seconds"] = max(
                                _safe_float(
                                    age_current.get("oldest_pending_age_seconds"), 0.0
                                ),
                                _safe_float(row.get("oldest_pending_age_seconds"), 0.0),
                            )
    source_core_pending = sum(
        _safe_int(row.get("pending_lines"), 0) for row in hot_rows_by_source.values()
    )
    source_core_oldest = max(
        [
            _safe_float(row.get("oldest_pending_age_seconds"), 0.0)
            for row in hot_rows_by_source.values()
        ]
        or [0.0]
    )
    age_guard_source_pending = sum(
        _safe_int(row.get("pending_lines"), 0)
        for row in age_guard_rows_by_source.values()
    )
    age_guard_source_oldest = max(
        [
            _safe_float(row.get("oldest_pending_age_seconds"), 0.0)
            for row in age_guard_rows_by_source.values()
        ]
    ...
    raw_total = max(
        _safe_int(backpressure.get("pending_lines_total"), raw_core),
        _safe_int(storage_raw_live.get("total_pending_lines"), 0),
    )
    raw_oldest = max(
    ...
    guard_oldest = max(
        age_guard_source_oldest if raw_hot_age_material else 0.0,
        raw_oldest if raw_hot_material else 0.0,
    )
    core_ratio = raw_core / max(float(reserve_core), 1.0)
    total_ratio = (
        guard_total / max(float(reserve_total), 1.0) if guard_total > 0 else 0.0
    )
    age_ratio = (
        guard_oldest / max(float(reserve_age), 1.0)
        if raw_hot_material or raw_hot_age_material
        else 0.0
    )
    pressure_ratio = max(core_ratio, total_ratio, age_ratio)
    active = bool(pressure_ratio > 1.0)
    live_priority_bonus = (
        int(min(max(110_000.0 + (pressure_ratio - 1.0) * 180_000.0, 0.0), 450_000.0))
        if active
        else 0
    )
    cold_stage_penalty = (
        int(min(max(40_000.0 + (pressure_ratio - 1.0) * 90_000.0, 0.0), 260_000.0))
        if active
        else 0
    )
    return {
        ...
        "raw_live": {
            ...
            "hot_source_oldest_pending_age_seconds": round(
                float(source_core_oldest), 3
            ),
            "age_guard_source_pending_lines": int(age_guard_source_pending),
            "age_guard_source_oldest_pending_age_seconds": round(
                float(age_guard_source_oldest), 3
            ),
            "deferred_or_support_hot_source_pending_lines": int(
                max(source_core_pending - age_guard_source_pending, 0)
            ),
            "deferred_or_support_hot_source_oldest_pending_age_seconds": round(
                float(
                    source_core_oldest
                    if source_core_pending > age_guard_source_pending
                    else 0.0
                ),
                3,
            ),
            ...
            "excluded_deferred_or_cold_pending_lines": int(
                max(raw_total - guard_total, 0)
            ),
        },
        "targets": {
...
def _apply_age_pressure_priority(
    ...
    *,
    ...
    live_priority_bonus = (
        _safe_int(guard.get("live_priority_bonus"), 0) if guard_active else 0
    )
    cold_stage_penalty = (
        _safe_int(guard.get("cold_stage_penalty"), 0) if guard_active else 0
    )
    guard_raw_live = (
        guard.get("raw_live") if isinstance(guard.get("raw_live"), dict) else {}
    )
    guard_targets = (
        guard.get("targets") if isinstance(guard.get("targets"), dict) else {}
    )
    canonical_core_pending = _safe_int(
        guard_raw_live.get("canonical_core_pending_lines"),
        _safe_int(guard_raw_live.get("core_pending_lines"), 0),
    )
    age_guard_source_oldest = _safe_float(
        guard_raw_live.get("age_guard_source_oldest_pending_age_seconds"), 0.0
    )
    reserve_core = max(_safe_int(guard_targets.get("core_reserve_lines"), 0), 1)
    reserve_total = max(
        _safe_int(guard_targets.get("total_reserve_lines"), reserve_core),
        reserve_core,
    )
    material_pending_threshold = max(int(reserve_core * 0.25), 1_000)
    material_drain_required = (
        _safe_int(backpressure.get("pending_lines_total"), 0) > reserve_total
    )
    core_reserve_breached = canonical_core_pending >= reserve_core
    raw_core_needs_first_handoff = bool(
        core_reserve_breached
        or (
            age_guard_source_oldest >= threshold_seconds
            and _safe_int(guard_raw_live.get("age_guard_source_pending_lines"), 0)
            >= MIN_MATERIAL_PENDING_LINES
        )
    )
    for row in profiles:
        pending_lines = _safe_int(row.get("pending_lines"), 0)
        min_pending_lines = _safe_int(
            row.get("min_pending_lines"), MIN_MATERIAL_PENDING_LINES
        )
        ...
        eligible = bool(
            ...
            and (
                pending_lines >= max(min_pending_lines, 1)
                or readiness_reason == "stale_tail"
            )
        )
        bonus = 0
        if eligible:
        ...
        dominant_risk_support_pressure = bool(
            ...
            and name == "risk_support_drainer"
            and pending_lines
            >= max(1_000_000, reserve_core * 100, canonical_core_pending * 100)
        ...
        cold_penalty = int(
            cold_stage_penalty if guard_active and name == "cold_stage_drainer" else 0
        )
        ...
        row["raw_live_expansion_risk_channel_pressure"] = bool(
            risk_channel_pressure or dominant_risk_support_pressure
        )
        row["raw_live_expansion_dominant_risk_support_pressure"] = (
            dominant_risk_support_pressure
        )
        row["material_backlog_preemption_tier"] = int(
            material_drain_required and pending_lines >= material_pending_threshold
        )
        row["material_backlog_preemption_threshold_lines"] = int(
            material_pending_threshold
        )
        row["effective_priority_score"] = int(
            priority_score + bonus + raw_bonus + raw_size_bonus - cold_penalty
        )
        if guard_active and (raw_bonus or raw_size_bonus or cold_penalty):
            env = (
                row.get("env_overrides")
                if isinstance(row.get("env_overrides"), dict)
                else {}
            )
            env.update(
                {
                    ...
                    "SQL_LINK_SERVICE_RAW_LIVE_SIZE_PRIORITY_BOOST": str(
                        raw_size_bonus
                    ),
...
def _decision_drainer_env(
    base: dict[str, str], core_rows: list[dict[str, Any]]
    ...
    if concentrated or sparse_large_line_active:
        env.update(
            {
                "SQL_LINK_SERVICE_CONCENTRATED_CORE_DRAIN": (
                    "1" if concentrated else "0"
                ),
                "SQL_LINK_SERVICE_CONCENTRATED_CORE_TOP1_SHARE": str(
                    concentration.get("top1_share", 0.0)
                ),
                "SQL_LINK_SERVICE_CONCENTRATED_CORE_TOP3_SHARE": str(
                    concentration.get("top3_share", 0.0)
                ),
    ...
    if sparse_large_line_active:
        env.update(
            {
                ...
                "SQL_LINK_SERVICE_SPARSE_LARGE_DECISION_FILE_COUNT": str(
                    sparse_pressure.get("file_count", 0)
                ),
                "INGEST_MAX_BYTES_PER_FILE": str(
                    SPARSE_LARGE_DECISION_MAX_BYTES_PER_FILE
                ),
                "SQLITE_BATCH_MAX_BYTES": str(
                    SPARSE_LARGE_DECISION_SQLITE_BATCH_MAX_BYTES
                ),
                "SQL_LINK_SERVICE_MERGE_MAX_SECONDS_PER_CYCLE": "180",
            }
        )
        if sparse_regular_focus:
            env.update(
                {
                    "SQL_LINK_SERVICE_SHARD_TRADING_STATE_CHECKPOINT_LINES": str(
                        SPARSE_DECISION_CATCH_UP_MERGE_MAX_JSONL_ROWS
                    ),
                    "SQL_LINK_SERVICE_SHARD_TRADING_MERGE_MAX_JSONL_ROWS": str(
                        SPARSE_DECISION_CATCH_UP_MERGE_MAX_JSONL_ROWS
                    ),
                }
            )
        if sparse_aggressive_focus:
            env.update(
                {
                    "SQL_LINK_SERVICE_SHARD_AGGRESSIVE_TRADING_STATE_CHECKPOINT_LINES": str(
                        SPARSE_DECISION_CATCH_UP_MERGE_MAX_JSONL_ROWS
                    ),
                    "SQL_LINK_SERVICE_SHARD_AGGRESSIVE_TRADING_MERGE_MAX_JSONL_ROWS": str(
                        SPARSE_DECISION_CATCH_UP_MERGE_MAX_JSONL_ROWS
                    ),
                }
            )
        if sparse_crypto_focus:
            env.update(
                {
                    "SQL_LINK_SERVICE_SHARD_CRYPTO_TRADING_STATE_CHECKPOINT_LINES": str(
                        SPARSE_DECISION_CATCH_UP_MERGE_MAX_JSONL_ROWS
                    ),
                    "SQL_LINK_SERVICE_SHARD_CRYPTO_TRADING_MERGE_MAX_JSONL_ROWS": str(
                        SPARSE_DECISION_CATCH_UP_MERGE_MAX_JSONL_ROWS
                    ),
                }
            )
    if regular_focus:
        ...
        env["SQL_LINK_SERVICE_SHARD_TRADING_PATH_CONTAINS"] = ",".join(
            regular_focus[:8]
        )
        env["SQL_LINK_SERVICE_SHARD_TRADING_MAX_FILES"] = (
            "8" if regular_focus_has_sparse_rows else "16"
        )
        ...
        env["SQL_LINK_SERVICE_SHARD_AGGRESSIVE_TRADING_MAX_FILES"] = (
            "8" if aggressive_focus_has_sparse_rows else "14"
        )
        ...
        if "crypto_trading" in regular_focus_shards and crypto_focus:
            ...
            env["SQL_LINK_SERVICE_SHARD_CRYPTO_TRADING_PATH_CONTAINS"] = ",".join(
                crypto_focus[:8]
            )
    ...
    if aggressive_focus:
        if "aggressive_trading" not in shards:
            shards.append("aggressive_trading")
        env["SQL_LINK_SERVICE_SHARD_AGGRESSIVE_TRADING_PATH_CONTAINS"] = ",".join(
            aggressive_focus[:8]
        )
        env["SQL_LINK_SERVICE_SHARD_AGGRESSIVE_TRADING_MAX_FILES"] = (
            "8" if aggressive_focus_has_sparse_rows else "14"
    ...
    if signal_focus:
        ...
        env["SQL_LINK_SERVICE_SHARD_GOVERNANCE_PATH_CONTAINS"] = ",".join(
            signal_focus[:8]
        )
        env["SQL_LINK_SERVICE_SHARD_GOVERNANCE_MAX_FILES"] = "8"
        env["SQL_LINK_SERVICE_SHARD_GOVERNANCE_MAX_LINES_PER_FILE"] = str(
            SIGNAL_GENERATION_CATCH_UP_MAX_LINES
        )
        env["SQL_LINK_SERVICE_SHARD_GOVERNANCE_STATE_CHECKPOINT_LINES"] = "8000"
        env["SQL_LINK_SERVICE_SHARD_GOVERNANCE_MERGE_MAX_JSONL_ROWS"] = str(
            SIGNAL_GENERATION_CATCH_UP_MERGE_MAX_JSONL_ROWS
        )
        env["INGEST_MAX_BYTES_PER_FILE"] = str(
            SIGNAL_GENERATION_CATCH_UP_MAX_BYTES_PER_FILE
        )
        env["SQLITE_BATCH_MAX_BYTES"] = str(
            SIGNAL_GENERATION_CATCH_UP_SQLITE_BATCH_MAX_BYTES
        )
...
def _api_ingress_drainer_env(
    ...
    if crypto_focus:
        ...
        env["SQL_LINK_SERVICE_SHARD_CRYPTO_API_INGRESS_PATH_CONTAINS"] = ",".join(
            crypto_focus[:8]
        )
        env["SQL_LINK_SERVICE_SHARD_CRYPTO_API_INGRESS_MAX_FILES"] = (
            "12" if critical else "8"
        )
        env["SQL_LINK_SERVICE_SHARD_CRYPTO_API_INGRESS_MAX_LINES_PER_FILE"] = (
            "32000" if critical else "16000"
        )
        env["SQL_LINK_SERVICE_SHARD_CRYPTO_API_INGRESS_STATE_CHECKPOINT_LINES"] = "1500"
    if regular_focus:
        shards.append("api_ingress")
        env["SQL_LINK_SERVICE_SHARD_API_INGRESS_PATH_CONTAINS"] = ",".join(
            regular_focus[:12]
        )
        env["SQL_LINK_SERVICE_SHARD_API_INGRESS_MAX_FILES"] = "24" if critical else "16"
        env["SQL_LINK_SERVICE_SHARD_API_INGRESS_MAX_LINES_PER_FILE"] = (
            "64000" if critical else "32000"
        )
...
def _fast_trade_bridge_drainer_env(
    base: dict[str, str], rows: list[dict[str, Any]], *, critical: bool
) -> tuple[list[str], dict[str, str]]:
    regular_focus: list[str] = []
    crypto_focus: list[str] = []
    ...
    if regular_focus:
        ...
        env["SQL_LINK_SERVICE_SHARD_TRADING_FAST_PATH_CONTAINS"] = ",".join(
            regular_focus[:8]
        )
        env["SQL_LINK_SERVICE_SHARD_TRADING_FAST_MAX_FILES"] = "12" if critical else "8"
        env["SQL_LINK_SERVICE_SHARD_TRADING_FAST_MAX_LINES_PER_FILE"] = (
            "32000" if critical else "16000"
        )
        env["SQL_LINK_SERVICE_SHARD_TRADING_FAST_STATE_CHECKPOINT_LINES"] = "1500"
    if crypto_focus:
        shards.append("crypto_trading_fast")
        env["SQL_LINK_SERVICE_SHARD_CRYPTO_TRADING_FAST_PATH_CONTAINS"] = ",".join(
            crypto_focus[:8]
        )
        env["SQL_LINK_SERVICE_SHARD_CRYPTO_TRADING_FAST_MAX_FILES"] = (
            "12" if critical else "8"
        )
        env["SQL_LINK_SERVICE_SHARD_CRYPTO_TRADING_FAST_MAX_LINES_PER_FILE"] = (
            "32000" if critical else "16000"
        )
        env["SQL_LINK_SERVICE_SHARD_CRYPTO_TRADING_FAST_STATE_CHECKPOINT_LINES"] = (
            "1500"
        )
...
def _attribution_drainer_env(
    ...
    if regular_focus:
        ...
        env["SQL_LINK_SERVICE_SHARD_SHADOW_ATTRIBUTION_PATH_CONTAINS"] = ",".join(
            regular_focus[:8]
        )
        env["SQL_LINK_SERVICE_SHARD_SHADOW_ATTRIBUTION_MAX_FILES"] = (
            "8" if critical else "5"
        )
        env["SQL_LINK_SERVICE_SHARD_SHADOW_ATTRIBUTION_MAX_LINES_PER_FILE"] = (
            "32000" if critical else "16000"
        )
        env["SQL_LINK_SERVICE_SHARD_SHADOW_ATTRIBUTION_STATE_CHECKPOINT_LINES"] = "2000"
    if crypto_focus:
        shards.append("crypto_shadow_attribution")
        env["SQL_LINK_SERVICE_SHARD_CRYPTO_SHADOW_ATTRIBUTION_PATH_CONTAINS"] = (
            ",".join(crypto_focus[:8])
        )
        env["SQL_LINK_SERVICE_SHARD_CRYPTO_SHADOW_ATTRIBUTION_MAX_FILES"] = (
            "8" if critical else "5"
        )
        env["SQL_LINK_SERVICE_SHARD_CRYPTO_SHADOW_ATTRIBUTION_MAX_LINES_PER_FILE"] = (
            "32000" if critical else "16000"
        )
        env[
            "SQL_LINK_SERVICE_SHARD_CRYPTO_SHADOW_ATTRIBUTION_STATE_CHECKPOINT_LINES"
        ] = "2000"
...
def _focused_shard_env(
    ...
    *,
    ...
    final_shards = ordered_unique(
        [*shards, *(["health_fast"] if include_health_fast else [])]
    )
    ...
    focused_max_lines = str(
        max(max_lines_per_file if critical else max_lines_per_file // 2, 4000)
    )
...
def _derivatives_drainer_env(
    ...
    if crypto_focus:
        ...
        env["SQL_LINK_SERVICE_SHARD_CRYPTO_TRADING_MAX_FILES"] = (
            "10" if critical else "6"
        )
        env["SQL_LINK_SERVICE_SHARD_CRYPTO_TRADING_MAX_LINES_PER_FILE"] = (
            "32000" if critical else "16000"
        )
    if explanation_focus:
        shards.append("explanations")
        env["SQL_LINK_SERVICE_SHARD_EXPLANATIONS_PATH_CONTAINS"] = ",".join(
            explanation_focus[:8]
        )
        env["SQL_LINK_SERVICE_SHARD_EXPLANATIONS_MAX_FILES"] = "12" if critical else "8"
        env["SQL_LINK_SERVICE_SHARD_EXPLANATIONS_MAX_LINES_PER_FILE"] = (
            "64000" if critical else "24000"
        )
    ...
    if crypto_explanation_focus:
        ...
        env["SQL_LINK_SERVICE_SHARD_CRYPTO_EXPLANATIONS_PATH_CONTAINS"] = ",".join(
            crypto_explanation_focus[:8]
        )
        env["SQL_LINK_SERVICE_SHARD_CRYPTO_EXPLANATIONS_MAX_FILES"] = (
            "12" if critical else "8"
        )
        env["SQL_LINK_SERVICE_SHARD_CRYPTO_EXPLANATIONS_MAX_LINES_PER_FILE"] = (
            "64000" if critical else "24000"
        )
        env["SQL_LINK_SERVICE_SHARD_CRYPTO_EXPLANATIONS_STATE_CHECKPOINT_LINES"] = (
            "2000"
        )
    if not shards:
...
def _candidate_drainers(
    ...
    *,
    ...
    base = _base_env(
        critical=critical, backpressure=backpressure, host_context=host_context
    )
    storage = storage_control if isinstance(storage_control, dict) else {}
    ...
    stale_decision_focus_rows = (
        ...
        else _merge_source_rows(
            stale_decision_rows, _stale_decision_rows_from_backpressure(backpressure)
        )
    )
    ...
    storage_route_rows = _collect_sources_by_contains(
        ...
        (
            ...
            "storage_switch",
            "storage_quota",
            "manifest_backed_offload_restore",
        ),
        keys=(
            "top_pending_files",
            "top_deferred_pending_files",
    ...
    decision_focus_rows = _stale_sticky_decision_rows(
        core_rows, stale_decision_focus_rows
    )
        ...
        decision_env.update(
            {
                ...
                "SQL_LINK_SERVICE_SHARD_TRADING_MERGE_MAX_JSONL_ROWS": str(
                    STALE_DECISION_CATCH_UP_MERGE_MAX_JSONL_ROWS
                ),
                "SQL_LINK_SERVICE_SHARD_AGGRESSIVE_TRADING_MERGE_MAX_JSONL_ROWS": str(
                    STALE_DECISION_CATCH_UP_MERGE_MAX_JSONL_ROWS
                ),
                ...
                "SQL_LINK_SERVICE_SHARD_TRADING_MAX_BYTES_PER_FILE": str(
                    STALE_DECISION_CATCH_UP_MAX_BYTES_PER_FILE
                ),
                "SQL_LINK_SERVICE_SHARD_AGGRESSIVE_TRADING_MAX_BYTES_PER_FILE": str(
                    STALE_DECISION_CATCH_UP_MAX_BYTES_PER_FILE
                ),
                "SQL_LINK_SERVICE_SHARD_CRYPTO_TRADING_MAX_BYTES_PER_FILE": str(
                    STALE_DECISION_CATCH_UP_MAX_BYTES_PER_FILE
                ),
                "SQL_LINK_SERVICE_SHARD_TRADING_SQLITE_BATCH_MAX_BYTES": str(
                    STALE_DECISION_CATCH_UP_SQLITE_BATCH_MAX_BYTES
                ),
                "SQL_LINK_SERVICE_SHARD_AGGRESSIVE_TRADING_SQLITE_BATCH_MAX_BYTES": str(
                    STALE_DECISION_CATCH_UP_SQLITE_BATCH_MAX_BYTES
                ),
                "SQL_LINK_SERVICE_SHARD_CRYPTO_TRADING_SQLITE_BATCH_MAX_BYTES": str(
                    STALE_DECISION_CATCH_UP_SQLITE_BATCH_MAX_BYTES
                ),
                "INGEST_MAX_BYTES_PER_FILE": str(
                    STALE_DECISION_CATCH_UP_MAX_BYTES_PER_FILE
    ...
    bridge_shards, bridge_env = _fast_trade_bridge_drainer_env(
        base, bridge_rows, critical=critical
    )
    attribution_shards, attribution_env = _attribution_drainer_env(
        base, attribution_rows, critical=critical
    )
    derivatives_shards, derivatives_env = _derivatives_drainer_env(
        base, derivatives_rows, critical=critical
    )
    provider_shards, provider_env = _focused_shard_env(
        base,
        provider_rows,
        shards=["data", "governance"],
        critical=critical,
        max_files=12,
        max_lines_per_file=32000,
        state_checkpoint_lines=1000,
    )
    macro_shards, macro_env = _focused_shard_env(
        base,
        macro_rows,
        shards=["data", "governance"],
        critical=critical,
        max_files=10,
        max_lines_per_file=24000,
        state_checkpoint_lines=1000,
    )
    model_shards, model_env = _focused_shard_env(
        base,
        model_rows,
        shards=["governance", "data"],
        critical=critical,
        max_files=8,
        max_lines_per_file=24000,
        state_checkpoint_lines=1200,
    )
    feature_shards, feature_env = _focused_shard_env(
        base,
        feature_rows,
        shards=["data", "governance"],
        critical=critical,
    ...
    report_shards, report_env = _focused_shard_env(
        base,
        report_rows,
        shards=["governance", "data"],
        critical=critical,
    ...
    settlement_shards, settlement_env = _focused_shard_env(
        base,
        settlement_rows,
        shards=["governance", "trading_fast"],
        critical=critical,
        max_files=10,
        max_lines_per_file=32000,
        state_checkpoint_lines=1000,
    )
    alert_shards, alert_env = _focused_shard_env(
        base,
        alert_rows,
        shards=["support_watchdog", "governance"],
        critical=critical,
        max_files=8,
        max_lines_per_file=16000,
        state_checkpoint_lines=1000,
    )
    memory_runtime_shards, memory_runtime_env = _focused_shard_env(
        base,
        memory_runtime_rows,
        shards=["runtime", "support_watchdog"],
        critical=critical,
        max_files=10,
        max_lines_per_file=24000,
        state_checkpoint_lines=1000,
    )
    data_quality_shards, data_quality_env = _focused_shard_env(
        base,
        data_quality_rows,
        shards=["data", "governance", "schema_violations"],
        critical=critical,
        max_files=10,
        max_lines_per_file=24000,
        state_checkpoint_lines=1000,
    )
    predictive_shards, predictive_env = _focused_shard_env(
        base,
        predictive_rows,
        shards=["runtime", "governance", "support_watchdog"],
        critical=critical,
        max_files=8,
        max_lines_per_file=16000,
        state_checkpoint_lines=900,
    )
    self_healing_shards, self_healing_env = _focused_shard_env(
        base,
        self_healing_rows,
        shards=["support_watchdog", "governance"],
        critical=critical,
    ...
    collector_utility_shards, collector_utility_env = _focused_shard_env(
        base,
        collector_utility_rows,
        shards=["data", "governance"],
        critical=critical,
        max_files=8,
        max_lines_per_file=16000,
    ...
    hot_path_budget_shards, hot_path_budget_env = _focused_shard_env(
        base,
        hot_path_budget_rows,
        shards=["governance", "data"],
        critical=critical,
        max_files=10,
        max_lines_per_file=24000,
    ...
    admission_evidence_shards, admission_evidence_env = _focused_shard_env(
        base,
        admission_evidence_rows,
        shards=["governance", "data"],
        critical=critical,
        max_files=8,
        max_lines_per_file=16000,
    ...
    writer_progress_shards, writer_progress_env = _focused_shard_env(
        base,
        writer_progress_rows,
        shards=["support_watchdog", "health_fast", "governance"],
        critical=critical,
        max_files=6,
        max_lines_per_file=12000,
        state_checkpoint_lines=600,
        include_health_fast=False,
    )
    ...
    training_lineage_shards, training_lineage_env = _focused_shard_env(
        base,
        training_lineage_rows,
        shards=["governance", "data"],
        critical=critical,
        max_files=8,
        max_lines_per_file=16000,
    ...
    label_contract_shards, label_contract_env = _focused_shard_env(
        base,
        label_contract_rows,
        shards=["data", "governance", "schema_violations"],
        critical=critical,
    ...
    collector_telemetry_shards, collector_telemetry_env = _focused_shard_env(
        base,
        collector_telemetry_rows,
        shards=["data", "governance"],
        critical=critical,
        max_files=8,
        max_lines_per_file=16000,
    ...
    storage_route_shards, storage_route_env = _focused_shard_env(
        base,
        storage_route_rows,
        shards=["governance", "support_watchdog"],
        critical=critical,
        max_files=6,
        max_lines_per_file=12000,
        state_checkpoint_lines=700,
    )
    ingestion_priority_shards, ingestion_priority_env = _focused_shard_env(
        base,
        ingestion_priority_rows,
        shards=["governance", "data", "support_watchdog"],
        critical=critical,
        max_files=8,
        max_lines_per_file=16000,
        state_checkpoint_lines=800,
    )
    ...
    regular_cold_focus_paths = [
        path for path in cold_focus_paths if path not in set(crypto_cold_focus_paths)
    ]
    stale_decision_shards, stale_decision_env = _decision_drainer_env(
        base, stale_decision_rows
    )
    if stale_decision_rows:
        stale_wave_limit = max(
            _safe_int(
                stale_decision_env.get("WRITER_CYCLE_MAX_CATCH_UP_WAVES"),
                3 if critical else 2,
            ),
            5 if critical else 3,
        )
        stale_merge_seconds = max(
            _safe_int(
                stale_decision_env.get("SQL_LINK_SERVICE_MERGE_MAX_SECONDS_PER_CYCLE"),
                120 if critical else 90,
            ),
            150 if stale_wave_limit >= 6 else 120 if critical else 90,
        )
        stale_shard_timeout = max(
            _safe_int(
                stale_decision_env.get("SQL_LINK_SERVICE_SHARD_LINK_TIMEOUT_SECONDS"),
                420,
            ),
            480 if stale_wave_limit >= 6 else 420,
        )
        stale_decision_env.update(
            {
                "SQL_LINK_SERVICE_STALE_DECISION_SOURCE_CATCH_UP": "1",
                "SQL_LINK_SERVICE_MERGE_MAX_SECONDS_PER_CYCLE": str(
                    stale_merge_seconds
                ),
    ...
    profiles = [
        ...
        _profile(
            ...
            priority_boost=(
                100_000
                if _safe_int(backpressure.get("pending_lines"), 0)
                >= CORE_HARD_PENDING_LINES
                else 60_000
            ),
            live_window_safe=True,
            env=decision_env,
        ...
        _profile(
            ...
            env={
                **base,
                ...
                "SQL_LINK_SERVICE_SHARD_GOVERNANCE_PATH_CONTAINS": ",".join(
                    str(row["source_rel"]) for row in governance_rows[:8]
                ),
        ...
        _profile(
            ...
            env={
                **base,
                ...
                "SQL_LINK_SERVICE_SHARD_SCHEMA_VIOLATIONS_PATH_CONTAINS": ",".join(
                    str(row["source_rel"]) for row in schema_rows[:8]
                ),
                "SQL_LINK_SERVICE_SHARD_SCHEMA_VIOLATIONS_MAX_FILES": (
                    "12" if critical else "6"
                ),
                "SQL_LINK_SERVICE_SHARD_SCHEMA_VIOLATIONS_MAX_LINES_PER_FILE": (
                    "32000" if critical else "16000"
                ),
        ...
        _profile(
            ...
            env={
                **base,
                ...
                "SQL_LINK_SERVICE_SHARD_SUPPORT_WATCHDOG_PATH_CONTAINS": ",".join(
                    str(row["source_rel"]) for row in support_rows[:8]
                ),
        ...
        _profile(
            ...
            env={
                **base,
                ...
                "SQL_LINK_SERVICE_SHARD_EXPLANATIONS_PATH_CONTAINS": ",".join(
                    regular_cold_focus_paths
                ),
                "SQL_LINK_SERVICE_SHARD_CRYPTO_EXPLANATIONS_PATH_CONTAINS": ",".join(
                    crypto_cold_focus_paths
                ),
    ...
    profiles = _apply_age_pressure_priority(
        profiles, backpressure, raw_live_guard=raw_live_guard
    )
    ...
    ready.sort(
        key=lambda row: (
            ...
            _safe_int(row.get("material_backlog_preemption_tier"), 0),
            _safe_int(
                row.get("effective_priority_score"),
...
def _service_request_identity(request: dict[str, Any]) -> tuple[str, str, str]:
    env = (
    ...
    return (
        str(request.get("requested_at") or request.get("timestamp_utc") or "").strip(),
        str(request.get("reason") or "").strip(),
        str(env.get("SQL_LINK_SERVICE_SHARDS") or "").strip(),
    )


def _service_request_consumed(path: Path, request: dict[str, Any]) -> bool:
    consumed = load_json(
        path.with_name("sql_link_service_request_consumed_latest.json")
    )
    consumed_request = (
    ...
    if consumed_request and _service_request_identity(request) == (
        _service_request_identity(consumed_request)
    ):
        return True
    progress = load_json(path.with_name("sql_link_service_progress_latest.json"))
    observed = (
        progress.get("active_request")
        if isinstance(progress.get("active_request"), dict)
        else {}
    )
    request_identity = _service_request_identity(request)
    same_request = bool(
        request_identity[0]
        and request_identity == _service_request_identity(observed)
    )
    return bool(
        same_request
        and progress.get("running") is False
        and str(progress.get("current_step") or "") == "complete"
    )


def _write_service_request(
    path: Path, *, active_drainer: dict[str, Any], now_utc: datetime, ttl_seconds: int
) -> dict[str, Any]:
    env = (
        active_drainer.get("env_overrides")
        if isinstance(active_drainer.get("env_overrides"), dict)
        else {}
    )
    existing = load_json(path)
    if existing and existing.get("active") is not False:
        expires_at = _parse_iso_utc(existing.get("expires_utc"))
        existing_env = (
            existing.get("env_overrides")
            if isinstance(existing.get("env_overrides"), dict)
            else {}
        )
        request_kind = str(existing.get("request_kind") or "")
        keep_external_drain = request_kind == "external_backlog_drain"
        keep_unconsumed_fleet_request = bool(
            request_kind == "backpressure_drainer_fleet"
            and not _service_request_consumed(path, existing)
        )
        if (
            (expires_at is None or expires_at > now_utc)
            and str(existing_env.get("SQL_LINK_SERVICE_SHARDS") or "").strip()
            and (keep_external_drain or keep_unconsumed_fleet_request)
        ):
            if keep_external_drain:
                existing_shards = [
                    item.strip()
                    for item in str(
                        existing_env.get("SQL_LINK_SERVICE_SHARDS") or ""
                    ).split(",")
                    if item.strip()
                ]
                requested_shards = [
                    item.strip()
                    for item in str(env.get("SQL_LINK_SERVICE_SHARDS") or "").split(",")
                    if item.strip()
                ]
                expanded_shards = ordered_unique(
                    [*existing_shards, *requested_shards]
                )
                if expanded_shards != existing_shards:
                    expanded = dict(existing)
                    expanded_env = {
                        str(key): str(value)
                        for key, value in existing_env.items()
                        if str(key).strip()
                    }
                    expanded_env["SQL_LINK_SERVICE_SHARDS"] = ",".join(
                        expanded_shards
                    )
                    expanded["timestamp_utc"] = now_utc.isoformat()
                    expanded["env_overrides"] = expanded_env
                    history = (
                        list(expanded.get("coalesced_focus_requests") or [])
                        if isinstance(expanded.get("coalesced_focus_requests"), list)
                        else []
                    )
                    history.append(
                        {
                            "timestamp_utc": now_utc.isoformat(),
                            "drainer": str(active_drainer.get("name") or ""),
                            "added_shards": [
                                shard
                                for shard in requested_shards
                                if shard not in existing_shards
                            ],
                        }
                    )
                    expanded["coalesced_focus_requests"] = history[-20:]
                    write_payload(path, expanded)
                    expanded["preserved_existing_request"] = True
                    expanded["preserved_by"] = "backpressure_drainer_fleet"
                    expanded["preserved_reason"] = "external_drain_scope_expanded"
                    expanded["scope_expanded"] = True
                    return expanded
            preserved = dict(existing)
            preserved["preserved_existing_request"] = True
            preserved["preserved_by"] = "backpressure_drainer_fleet"
            preserved["preserved_reason"] = (
                "external_drain_lease"
                if keep_external_drain
                else "focused_handoff_not_yet_consumed"
            )
            return preserved
    ...
    payload = {
        ...
        "assigned_pressure_lane": str(
            active_drainer.get("assigned_pressure_lane") or ""
        ),
        "ops_infrabots": list(active_drainer.get("ops_infrabots") or []),
        "self_accommodation": (
            active_drainer.get("self_accommodation")
            if isinstance(active_drainer.get("self_accommodation"), dict)
            else {}
        ),
        "p_core_backlog_allocation_contract": (
            active_drainer.get("p_core_backlog_allocation_contract")
            if isinstance(
                active_drainer.get("p_core_backlog_allocation_contract"), dict
            )
            else {}
        ),
        ...
        "env_overrides": {
            str(key): str(value) for key, value in env.items() if str(key).strip()
        },
    }
...
def _fleet_self_accommodation(
    *,
    ...
    if not active_drainer:
        next_safe_action = (
            "refresh_backpressure_snapshot"
            if "missing_backpressure_artifact" in blocked_reasons
            else "idle_no_ready_drainer"
        )
        mode = "idle"
    elif writer_held:
        ...
        next_safe_action = (
            "run_backpressure_drainer_fleet_apply_or_bounded_super_drainer_wave"
        )
        mode = "preview_ready"
...
def build_payload(
    ...
    *,
    ...
    storage_control_fresh = _storage_control_fresh_for_backpressure(
        storage_control, backpressure
    )
    effective_storage_control = storage_control if storage_control_fresh else {}
    storage_backpressure = (
        effective_storage_control.get("backpressure")
        if isinstance(effective_storage_control.get("backpressure"), dict)
        else {}
    )
    storage_overlay = (
        effective_storage_control.get("sql_ingestion_pending_overlay")
        if isinstance(
            effective_storage_control.get("sql_ingestion_pending_overlay"), dict
        )
        else {}
    )
    critical = bool(
        ...
        or _safe_int(storage_backpressure.get("total_pending_lines"), 0)
        >= CORE_HARD_PENDING_LINES
        or _safe_int(storage_overlay.get("total_pending_lines"), 0)
        >= CORE_HARD_PENDING_LINES
    )
    ...
    raw_live_guard = _raw_live_expansion_guard(
        backpressure, storage_control=effective_storage_control
    )
    drainers = _candidate_drainers(
        backpressure,
        critical=critical,
        host_context=host_context,
        storage_control=effective_storage_control,
    )
    ready_drainers = [
        row for row in drainers if str(row.get("status") or "") == "ready"
    ]
    ...
    if apply and active_drainer and not blocked_reasons:
        service_request = _write_service_request(
            ...
            / "governance"
            / "health"
            / "sql_link_service_request_latest.json",
    ...
    payload = {
        ...
        "overall_status": (
            ...
            if service_request
                ...
                "ready"
                if active_drainer and not blocked_reasons
                else ("idle" if not blocked_reasons else "blocked")
            )
        ),
        "apply_requested": bool(apply),
        "service_request": service_request,
        "service_request_path": str(
            project_root
            / "governance"
            / "health"
            / "sql_link_service_request_latest.json"
        ...
        "metrics": {
            "storage_control_fresh_for_backpressure": storage_control_fresh,
            "core_pending_lines": _safe_int(backpressure.get("pending_lines"), 0),
            "total_pending_lines": _safe_int(
                backpressure.get("pending_lines_total"), 0
            ),
            "deferred_pending_lines": _safe_int(
                backpressure.get("pending_lines_deferred"), 0
            ),
            ...
            "stale_tail_ready_count": sum(
                1
                for row in ready_drainers
                if str(row.get("readiness_reason") or "") == "stale_tail"
            ),
            "live_window_safe_ready_count": sum(
                1 for row in ready_drainers if bool(row.get("live_window_safe", False))
            ),
            "active_concentrated_backlog": (
                bool(active_drainer.get("concentration", {}).get("concentrated", False))
...
def main() -> int:
    ...
    return (
        ...
        in {"already_running", "ready", "idle", "handoff_requested"}
        else 2
    )
