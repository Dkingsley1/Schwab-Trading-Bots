...
def build_payload(
    ...
    *,
    max_canary_weight: float = DEFAULT_MAX_CANARY_WEIGHT,
) -> dict[str, Any]:
    ...
    canary_graduation = load_json(health_root / "live_canary_graduation_latest.json")
    live_money_contract = load_json(
        health_root / "live_money_readiness_contract_latest.json"
    )
    canary_allowlist = evaluate_live_canary_allowlist(project_root)
    readiness_policy = load_json(
        project_root / "config" / "production_readiness_control_v1.json"
    )
    live_firewall = (
        readiness_policy.get("live_execution_risk_firewall")
        if isinstance(readiness_policy.get("live_execution_risk_firewall"), dict)
        else {}
    )
    canary_preflight_enforced = bool(
        live_firewall.get("require_live_canary_preflight", False)
    )
    if canary_preflight_enforced:
        canary_preflight = evaluate_live_canary_preflight(
            project_root,
            symbol=(canary_allowlist.get("symbols") or ["SCHD"])[0],
            action="BUY",
        )
        canary_preflight["enforced"] = True
    else:
        canary_preflight = {
            "ready": True,
            "enforced": False,
    ...
    clearance_state = (
        str(((runtime.get("clearance_plan") or {}).get("clearance_state") or ""))
        .strip()
        .lower()
    )
    ...
    release_contract = (
        runtime.get("release_contract")
        if isinstance(runtime.get("release_contract"), dict)
        else {}
    )
    live_lane_should_be_read_only = bool(
        release_contract.get("live_lane_should_be_read_only", False)
    )
    ...
    packet_preclearance_ready = bool(
        ...
        or (
            promotion_autopilot
            and str(promotion_autopilot.get("overall_status") or "").strip().lower()
            in {"degraded", "needs_attention", "ready"}
            and bool(
                promotion_autopilot.get("committee_packet_seed_ready", False)
                or (
                    (promotion_autopilot.get("signability_contract") or {}).get(
                        "committee_packet_seed_ready", False
                    )
                )
                or (
                    (promotion_autopilot.get("approval_record") or {}).get(
                        "committee_packet_seed_ready", False
                    )
                )
            ...
            and int(
                (
                    (promotion_autopilot.get("readiness_repair_contract") or {}).get(
                        "critical_repair_gate_count", 0
                    )
                    or 0
                )
            )
    ...
    target_canary_weight = _safe_float(
        canary_auto_tuner.get("target_canary_max_weight"), 0.0
    )
    applied_weight = _safe_float(
        canary_rollout.get("applied_weight"), target_canary_weight
    )
    ...
    canary_signal_seed_ready = bool(
        target_canary_weight > 0.0 and weight_ok and canary_auto_tuner
    )
    canary_signal_ready = bool(
        (
            canary_rollout.get("eligible", False)
            and canary_rollout.get("promote_canary", False)
        )
    ...
    live_money_contract_ready = bool(
        ...
        or live_money_contract.get(
            "faithful_live_money_ready", live_money_contract.get("ok", False)
        )
    )
    prereq_ready = bool(
    ...
    blocking_reasons = ordered_unique(
        [
            (
                ...
                if live_money_contract_enforced and not live_money_contract_ready
                else ""
            ),
            ...
            (
                ...
                if not packet_preclearance_ready
                else ("promotion_packet_preclearance_only" if not packet_ready else "")
            ),
            ...
            (
                ...
                if not canary_allowlist.get("ready", False)
                else ""
            ),
            (
                "live_canary_preflight_not_ready"
                if not canary_preflight.get("ready", False)
                else ""
            ),
        ]
    )
    ...
    if canary_preflight.get("ready", False):
        preclearance_score += 5.0
    ...
    recommended_mode = (
        ...
        if supervised_canary_ready
            ...
            "runnable_pending_release_window"
            if runnable_after_release_window
            else (
                "preapproved_supervised"
                if preapproved_supervised_ready
                else (
                    "staged_preclearance"
    ...
    contract_hard_block = bool(
        live_money_contract_enforced and not live_money_contract_ready
    )
    overall_status = (
        "ready"
        if supervised_canary_ready
            ...
            if contract_hard_block
                ...
                "degraded"
                if staged_preclearance_ready
                or (broker_ready and session_ready and storage_ok)
                else "blocked"
            )
        )
    )
    ...
    recommended_actions = ordered_unique(
        [
            (
                "run the live canary only after the promotion packet reaches a signed awaiting-approval or canary-ready state"
                if not packet_ready
                else ""
            ),
            (
                "treat the current promotion packet as canary-seeded only; finish the remaining promotion repairs before supervised submit"
                if packet_preclearance_ready and not packet_ready
                else ""
            ),
            (
                "clear runtime separation blockers before allowing even a supervised canary write path"
                if not clearance_ready or live_lane_should_be_read_only
                else ""
            ),
            (
                "keep live money locked until the A/A+ faithful-live contract clears"
                if live_money_contract_enforced and not live_money_contract_ready
                else ""
            ),
            (
                "refresh the canary rollout guard so supervised canary permission is backed by recent paper evidence"
                if not canary_signal_ready
                else ""
            ),
            (
                "create a short-lived candidate-bound canary allowlist only after the promotion gates clear"
                if not canary_allowlist.get("ready", False)
                else ""
            ),
            (
                "refresh account, risk, order-ledger, release, and tax evidence; then issue the short-lived operator attestation before supervised submit"
                if not canary_preflight.get("ready", False)
                else ""
            ),
            (
                "after the first reconciled canary, follow the post-canary graduation milestones; one profitable fill cannot advance stage or size"
                if canary_graduation
                else "refresh the post-canary graduation control before any stage-above-one review"
            ),
            (
                "the canary package is runnable once the runtime release window opens; the remaining blockers are only the cold-lane/runtime release protections"
                if runnable_after_release_window
                else ""
            ),
            (
                f"keep the canary weight at or under {float(max_canary_weight):.2f} before supervised live submit"
                if not weight_ok
                else ""
            ),
            (
                "finish the cold-lane/runtime clearance steps to convert staged preclearance into supervised canary permission"
                if staged_preclearance_ready
                else ""
            ),
            (
                "treat the canary as preapproved and waiting only on the cold-lane/runtime clearance window before supervised submit"
                if preapproved_supervised_ready
                else ""
            ),
            (
                "keep the system in validate-only mode until broker, session, storage, and live-lane readiness are all green"
                if blocking_reasons
                else ""
            ),
        ]
    ...
    return {
        ...
        "live_canary_preflight_ready": bool(canary_preflight.get("ready", False)),
        "live_canary_preflight_enforced": canary_preflight_enforced,
        "live_canary_preflight": canary_preflight,
        "post_canary_graduation": canary_graduation
        or {
            "overall_status": "ready_idle",
            "phase": "awaiting_first_canary",
            "live_execution_authority": False,
        ...
        "source_artifacts": {
            ...
            "storage_route_status": str(
                health_root / "storage_route_status_latest.json"
            ),
            ...
            "promotion_autopilot_packet": str(
                project_root
                / "governance"
                / "champion_challenger"
                / "promotion_autopilot_packet_latest.json"
            ...
            "live_money_readiness_contract": str(
                health_root / "live_money_readiness_contract_latest.json"
            ),
            "live_canary_preflight": str(
                project_root / "scripts" / "ops" / "live_canary_preflight.py"
            ),
            "live_canary_graduation": str(
                health_root / "live_canary_graduation_latest.json"
            ),
        },
    }
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Publish a supervised live-canary contract for broker submit gating."
    )
    parser.add_argument("--project-root", default=str(PROJECT_ROOT))
    parser.add_argument("--out-file", default=str(DEFAULT_OUT_PATH))
    parser.add_argument(
        "--max-canary-weight", type=float, default=DEFAULT_MAX_CANARY_WEIGHT
    )
    ...
    payload = build_payload(
        ...
        max_canary_weight=float(args.max_canary_weight),
    )
