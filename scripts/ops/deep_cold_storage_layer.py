...
def _is_protected_volume(path: Path) -> bool:
    return _volume_name(path) in PROTECTED_VOLUME_NAMES
...
def build_payload(
    ...
    *,
    ...
    payload = {
        ...
        "next_action": (
            "deep cold manifest is current; storage control can treat retention-locked stale-stage archives as managed cold evidence"
            if ready
