...
if __package__ in {None, ""}:
    ...
    from core.live_transition_safety import (
        canary_stage_contract,
        evaluate_release_interlock,
        reconcile_broker_truth,
    )
    from core.order_intent import (
        build_order_intent_evidence,
        verify_order_intent_evidence,
    )
    ...
    from . import live_execution_rehearsal_control, live_transition_chaos_harness
...
def _control(
    control_id: str, title: str, implemented: bool, evidence: dict[str, Any]
) -> dict[str, Any]:
    return {
...
def _auth_runtime_signals(auth: dict[str, Any]) -> dict[str, bool]:
    ...
    supervisor_ready = bool(
        auth.get("ok", False) and _status(auth.get("overall_status")) == "ready"
    )
    ...
    auth_probe_ready = bool(
        broker.get("auth_ok", auth.get("auth_ok", supervisor_ready))
    )
...
def _capability_self_tests(
    ...
    rehearsal = live_execution_rehearsal_control.build_payload(project_root)
    ledger_control = load_json(
        project_root / "governance" / "health" / "live_order_ledger_control_latest.json"
    )
    ...
    controls = [
        _control(
            ...
            {
                "verification": intent_verification,
                "parity_hash_equal": parity_ok,
                "intent_sha256": sample.get("intent_sha256"),
            },
        ),
        _control(
            ...
            bool(
                intent_verification.get("ok")
                and len(_as_dict(sample.get("component_hashes"))) == 4
            ),
            {
                "component_hashes": sample.get("component_hashes", {}),
                "adapter_excluded_from_hash": sample.get("adapter_excluded_from_hash"),
            },
        ),
        _control(
            ...
            {
                "ledger_status": ledger_control.get("overall_status"),
        ...
        _control(
            ...
            bool(
                reconcile_self_test.get("ok")
                and len(reconcile_self_test.get("surfaces_checked", [])) == 5
            ),
            reconcile_self_test,
        ),
        _control(
            ...
            bool(
                relock_self_test.get("auto_relocked")
                and "auth_not_ready" in relock_self_test.get("entry_lock_reasons", [])
            ),
            relock_self_test,
        ),
        _control(
            ...
            bool(
                chaos.get("ok", False) and int(chaos.get("scenario_count", 0) or 0) >= 7
            ),
            {
                "grade": chaos.get("grade"),
                "passed": chaos.get("passed_scenario_count"),
                "total": chaos.get("scenario_count"),
            },
        ),
        _control(
            ...
            bool(
                canary_self_test.get("ok")
                and not canary_self_test.get("automatic_scaling_allowed", True)
            ),
            canary_self_test,
        ),
        _control(
            ...
            bool(
                exit_self_test.get("risk_reducing_exit_allowed")
                and not exit_self_test.get("entry_allowed")
            ),
            exit_self_test,
        ),
        _control(
            "09_sealed_validate_only_rehearsal",
            "The exact future live path passes sealed no-network rehearsal and negative tests",
            bool(
                rehearsal.get("ok", False)
                and rehearsal.get("control_grade") == "A+"
                and rehearsal.get("authority", {}).get("live_execution_authority")
                is False
            ),
            {
                "grade": rehearsal.get("control_grade"),
                "implemented": rehearsal.get("implemented_control_count"),
                "total": rehearsal.get("control_count"),
                "overall_status": rehearsal.get("overall_status"),
                "authority": rehearsal.get("authority", {}),
            },
        ),
    ]
    return controls, chaos, rehearsal


def build_payload(project_root: Path = PROJECT_ROOT) -> dict[str, Any]:
    health = project_root / "governance" / "health"
    controls, chaos, rehearsal = _capability_self_tests(project_root)
    ...
    broker_gate = _as_dict(
        _as_dict(paper_truth.get("gates")).get("paper_broker_truth_reconciliation")
    )
    ...
    signals = {
        ...
        "quote_fresh": bool(
            source_quote.get("fresh", False) and source_quote.get("ok", False)
        ),
        "sources_ready": bool(
            source.get("ok", False) and _status(source.get("overall_status")) == "ready"
        ),
        "reconciliation_clean": bool(
            broker_gate.get("ok", False)
            and int(broker_gate.get("mismatch_count", 0) or 0) == 0
        ),
        "drawdown_within_limit": bool(
            profitability.get("promotion_evidence_ready", False)
        ),
        "storage_ready": bool(
            storage.get("ok", False)
            and _status(storage.get("overall_status")) == "ready"
        ),
    ...
    transition_ready = bool(
        control_ready
        and not interlock.get("entry_lock_reasons")
        and current_canary.get("ok", False)
    )
    ...
    return {
        ...
        "overall_status": (
            "ready_locked"
            if control_ready and not transition_ready
            else "ready" if transition_ready else "blocked"
        ),
        ...
        "validate_only_live_execution_rehearsal": {
            "ok": bool(rehearsal.get("ok", False)),
            "control_grade": str(rehearsal.get("control_grade") or ""),
            "implemented_control_count": int(
                rehearsal.get("implemented_control_count", 0) or 0
            ),
            "control_count": int(rehearsal.get("control_count", 0) or 0),
            "live_execution_authority": bool(
                rehearsal.get("authority", {}).get("live_execution_authority", False)
            ),
        },
        "blockers": blockers,
        "grading_contract": {
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Evaluate the integrated seamless paper-to-live transition contract."
    )
