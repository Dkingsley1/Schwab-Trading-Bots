...
if __package__ in {None, ""}:
    ...
    from scripts.ops import system_cleanliness_autopilot
else:
    PROJECT_ROOT = Path(__file__).resolve().parents[2]
    from .long_runtime_common import iso_now, write_payload
    from . import system_cleanliness_autopilot
...
def build_payload(project_root: Path = PROJECT_ROOT, *, apply: bool = False, timeout_sec: int = 300) -> dict[str, Any]:
    ...
    return {
        ...
        "recommended_actions": autopilot.get("recommended_actions") if isinstance(autopilot.get("recommended_actions"), list) else [],
    }
