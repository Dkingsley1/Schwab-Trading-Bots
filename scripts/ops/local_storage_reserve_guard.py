...
DEFAULT_HISTORY_PATH = PROJECT_ROOT / "governance" / "health" / "local_storage_reserve_guard_history.jsonl"
DEFAULT_LOCK_PATH = PROJECT_ROOT / "governance" / "locks" / "local_storage_reserve_guard.lock"
...
def _history_event_required(previous: dict[str, Any], current: dict[str, Any]) -> bool:
    if not previous or _history_signature(previous) != _history_signature(current):
        return True
    logs = current.get("launchd_log_guard") if isinstance(current.get("launchd_log_guard"), dict) else {}
    return bool(
...
def _append_history(path: Path, payload: dict[str, Any], *, max_lines: int) -> dict[str, Any]:
    reserve = payload.get("local_storage_reserve") if isinstance(payload.get("local_storage_reserve"), dict) else {}
    cleanup = payload.get("cleanup_verification") if isinstance(payload.get("cleanup_verification"), dict) else {}
    recovery = payload.get("recovery_request") if isinstance(payload.get("recovery_request"), dict) else {}
    row = {
    ...
    try:
        ...
        return {"ok": True, "appended": True, "path": str(path), "max_lines": limit}
...
def cap_launchd_logs(
    ...
    *,
    ...
    oversized = [
        row for row in rows if row.get("status") in {"would_cap", "capped", "verification_failed", "error"}
    ]
    errors = [row for row in rows if row.get("status") in {"verification_failed", "error"}]
    return {
        ...
        "verification_failed_count": sum(1 for row in rows if row.get("status") == "verification_failed"),
...
def cap_launchd_log_roots(
    ...
    *,
    ...
    return {
        ...
        "max_file_bytes": int(max_bytes),
        "tail_bytes": int(tail_bytes),
        "file_count": sum(int(report.get("file_count", 0) or 0) for report in root_reports),
        "oversized_count": sum(int(report.get("oversized_count", 0) or 0) for report in root_reports),
        "capped_count": sum(int(report.get("capped_count", 0) or 0) for report in root_reports),
        "verification_failed_count": sum(
            int(report.get("verification_failed_count", 0) or 0) for report in root_reports
        ),
        "error_count": sum(int(report.get("error_count", 0) or 0) for report in root_reports),
        "bytes_reclaimed": sum(int(report.get("bytes_reclaimed", 0) or 0) for report in root_reports),
        "rows": [row for report in root_reports for row in list(report.get("rows") or [])][:80],
        "roots": root_reports,
    }
...
def _recovery_request(
    ...
    *,
    ...
    if not known:
        ...
        severity = "none"
    paper_pause_required = bool(reserve.get("pressure_active", False) or reserve.get("hard_block", False))
    reasons = {
    ...
    target_free_gb = max(float(reserve.get("target_free_gb", DEFAULT_TARGET_FREE_GB) or 0.0), 0.0)
...
def build_payload(
    ...
    *,
    ...
    status = "blocked" if hard_blockers else ("degraded" if pressure_active else ("watch" if warnings else "ready"))
    ...
    return {
        ...
        "grade": "F" if hard_blockers else ("C" if pressure_active else ("A" if warnings else "A+")),
        "apply": bool(apply),
        ...
        "safety_contract": {
            ...
            "reserve_hysteresis": {
                ...
                "recovery_target_free_gb": round(
                    max(float(target_free_gb), 0.0) + max(float(recovery_headroom_gb), 0.0),
                    3,
                ),
            },
        },
        ...
        "next_action": (
            ...
            if status == "ready"
                ...
                if recovery.get("active") and not recovery.get("paper_pause_required") and not hard_blockers
...
def main() -> int:
    ...
    parser.add_argument("--history-max-lines", type=int, default=DEFAULT_HISTORY_MAX_LINES)
    ...
    parser.add_argument(
        "--pressure-free-gb",
        type=float,
        default=_env_float("BOT_LOCAL_STORAGE_PRESSURE_FREE_GB", DEFAULT_PRESSURE_FREE_GB),
    )
    parser.add_argument(
        "--recovery-headroom-gb",
        type=float,
        default=_env_float("BOT_LOCAL_STORAGE_RECOVERY_HEADROOM_GB", DEFAULT_RECOVERY_HEADROOM_GB),
    )
    parser.add_argument(
        "--hard-free-gb",
        type=float,
    ...
    parser.add_argument(
        "--emergency-free-gb",
        type=float,
        default=_env_float("BOT_LOCAL_STORAGE_EMERGENCY_FREE_GB", DEFAULT_EMERGENCY_FREE_GB),
    )
    ...
    if configured_additional is None:
        env_roots = [
            part.strip()
            for part in os.getenv("BOT_ADDITIONAL_LAUNCHD_LOG_ROOTS", "").split(os.pathsep)
            if part.strip()
        ]
    ...
    try:
        payload = build_payload(
            ...
            additional_log_roots=[Path(path).expanduser() for path in configured_additional],
        ...
        if not bool(history_result.get("ok", False)):
            payload["warnings"] = [*list(payload.get("warnings") or []), "storage_guard_history_write_failed"]
