...
def _artifact_contract(project_root: Path) -> dict[str, dict[str, Any]]:
    contract = {
        ...
        "sleeve_scalability_selector": {
            ...
            "max_age_minutes": 120.0,
            "required": True,
            "refresh_command": "./scripts/ops/opsctl.sh sleeve-scalability-selector --json",
        },
        ...
        "control_surface_ownership": {
            ...
            / "governance"
            / "health"
            / "control_surface_ownership_latest.json",
        ...
        "independent_runtime_monitor": {
            ...
            / "governance"
            / "health"
            / "independent_runtime_monitor_latest.json",
        ...
        "production_resilience_control": {
            ...
            / "governance"
            / "health"
            / "production_resilience_control_latest.json",
            "max_age_minutes": 30.0,
            "required": True,
        ...
        "soak_reliability_sentinel": {
            ...
            / "governance"
            / "health"
            / "soak_reliability_sentinel_latest.json",
        ...
        "live_readiness_smoke": {
            ...
            / "governance"
            / "health"
            / "live_readiness_smoke_latest.json",
...
def build_payload(project_root: Path = PROJECT_ROOT) -> dict[str, Any]:
        ...
        breaches.append(
            {
                ...
                "exists": exists,
                "age_minutes": (
                    round(float(age_minutes), 4) if age_minutes is not None else None
                ),
    ...
    recommended_actions = ordered_unique(
        [row["refresh_command"] for row in breaches if bool(row.get("stale"))]
    )[:8]
    ...
    receipt_sha = hashlib.sha256(
        json.dumps(
            receipt_rows, ensure_ascii=True, sort_keys=True, separators=(",", ":")
        ).encode("utf-8")
    ).hexdigest()
    return {
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Enforce freshness SLAs across the core runtime artifacts."
    )
