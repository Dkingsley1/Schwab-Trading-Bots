...
DEFAULT_STATE_PATH = (
    PROJECT_ROOT / "governance" / "health" / "process_watchdog_state.json"
)
DEFAULT_OUT_PATH = (
...
DEFAULT_CREATIVE_PAUSE_PATH = (
    PROJECT_ROOT / "governance" / "health" / "creative_heavy_research_pause_latest.json"
)
DEFAULT_RUNTIME_RESOURCE_GUARD_OVERRIDE_PATH = (
...
def _safety_pause_state() -> Dict[str, Any]:
    ...
    restart_fence_blocks = bool(
        restart_fence.get("active", False) and not restart_fence_authorized
    )
...
def _launcher_priority_state(target: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if required_raw is None:
        return {
            "required": False,
            "compliant": True,
            "reason": "no_launcher_priority_contract",
        }
    ...
    return {
        ...
        "reason": (
            "launcher_priority_compliant"
            if compliant
            else "launcher_priority_noncompliant"
        ),
    }
...
def _creative_cotenant_pause_state() -> Dict[str, Any]:
    ...
    hard_pause = (
        payload.get("hard_pause") if isinstance(payload.get("hard_pause"), dict) else {}
    )
    ...
    stale = bool(
        active
        and (
            age_seconds is None or age_seconds > DEFAULT_CREATIVE_PAUSE_MAX_AGE_SECONDS
        )
    )
    if stale:
        return {
    ...
    return {
        "active": active,
        "reason": str(
            payload.get("reason") or kind or level or "creative_cotenant_pause_active"
        ),
        "creative_session_level": level,
        "creative_session_kind": kind,
        "timestamp_utc": timestamp_utc,
    ...
    if "music" in kind or "audio" in kind:
        ...
        soft_audio_pause = bool(
            ...
            and (
                ...
                or hard_pause_action
                in {
                    "soft_pause_optional_heavy_research",
                    "lightweight_pause_contract_refresh",
                }
            )
        )
        if soft_audio_pause:
...
def _process_probe_seed(pattern: str) -> str:
    pattern_text = str(pattern or "").strip()
    if not pattern_text:
        return pattern_text
    try:
        tokens = shlex.split(pattern_text)
    except ValueError:
...
PROCESS_SCAN_UNKNOWN = -1


def _pgrep_process_rows(pattern: str) -> List[Tuple[int, str]] | None:
    seed = _process_probe_seed(pattern)
    if not seed:
        return []
    try:
        p = subprocess.run(
            ["pgrep", "-fl", seed], capture_output=True, text=True, check=False
        )
    except (OSError, PermissionError):
        return None
    out = p.stdout or ""
    if not out and int(getattr(p, "returncode", 0) or 0) not in {0, 1}:
        return None
...
def _proc_running(pattern: str, exclude_patterns: List[str] | None = None) -> int:
    ...
    try:
        ...
        out = ""
    if not out:
        pgrep_rows = _pgrep_process_rows(pattern)
        if pgrep_rows is None:
            return PROCESS_SCAN_UNKNOWN
        for _pid, command in pgrep_rows:
            if _command_matches_pattern(command, pattern) and not any(
                marker in command for marker in excludes
            ):
                running += 1
        return running
    for line in out.splitlines():
...
def _matching_pids(
    pattern: str, exclude_patterns: List[str] | None = None
) -> List[int]:
    excludes = _effective_process_excludes(exclude_patterns)
    pids: List[int] = []
    try:
    ...
    for line in out.splitlines()[1:]:
        ...
        if len(parts) != 2:
            continue
        pid_raw, command = parts
        if not _command_matches_pattern(command, pattern) or any(
            marker in command for marker in excludes
        ):
            continue
        try:
...
def _proc_elapsed_seconds(
    ...
    for line in out.splitlines():
        ...
        if len(parts) != 2:
            continue
        etime_raw, cmd = parts
        if not _command_matches_pattern(cmd, pattern) or any(
            marker in cmd for marker in excludes
        ):
            continue
...
def _run(
    ...
    try:
        ...
        stdout = (
            ...
            else (exc.stdout or b"").decode("utf-8", errors="ignore")
        )
        stderr = (
            exc.stderr
            if isinstance(exc.stderr, str)
            else (exc.stderr or b"").decode("utf-8", errors="ignore")
        )
        if p is not None:
...
def _child_fanout_health(
    ...
    *,
    ...
    grace_seconds = max(
        float(target.get("child_fanout_grace_seconds", 0.0) or 0.0), 0.0
    )
    ...
    if int(running) <= 0:
        ...
        return {
            ...
            "ok": bool(ok),
            "reason": (
                "parent_missing_child_fanout_present"
                if ok
                else "parent_missing_child_fanout_below_floor"
            ),
    ...
    if (
        parent_elapsed_seconds is not None
        and float(parent_elapsed_seconds) < grace_seconds
    ):
        return {
            "required": True,
            "ok": True,
    ...
    return {
        ...
        "parent_elapsed_seconds": (
            round(float(parent_elapsed_seconds), 3)
...
def _all_sleeves_launcher_artifact_health(
    ...
    *,
    ...
    phase = (
        str(payload.get("phase") or payload.get("current_step") or "").strip().lower()
    )
    ...
    paper_execution_ready = bool(readiness_contract.get("paper_execution_ready", True))
    ...
    exact_needs = (
        payload.get("exact_needs")
        if isinstance(payload.get("exact_needs"), list)
        else []
    )
    if not exact_needs:
        exact_needs = (
    ...
    execution_only_needs = bool(exact_needs) and all(
        isinstance(need, dict)
        and (
            str(need.get("job_class") or "").strip().lower() == "execution_lane"
            or str(need.get("target") or "").strip().lower()
            in {"paper_executor", "execution_lane"}
        )
        for need in exact_needs
    )
    needs_block_collection_fanout = bool(exact_needs and not execution_only_needs)
    ...
    complete = bool(
        ...
        and exited == 0
        and problem == 0
        and not needs_block_collection_fanout
    )
    ...
    return {
        ...
        "execution_only_exact_needs": execution_only_needs,
        "needs_block_collection_fanout": needs_block_collection_fanout,
        "collection_fanout_ready": collection_fanout_ready,
        "paper_execution_ready": paper_execution_ready,
        "execution_guarded": bool(
            collection_fanout_ready and not paper_execution_ready
        ),
        "execution_attention": (
...
def _paper_execution_runtime_pause_state(
    ...
    path = (
        ...
        else DEFAULT_RUNTIME_RESOURCE_GUARD_OVERRIDE_PATH
    )
...
def _within_budget(events: List[Dict[str, Any]], name: str, max_per_hour: int) -> bool:
    ...
    recent = [
        e
        for e in events
        if e.get("name") == name and float(e.get("ts_epoch", 0)) >= cutoff
    ]
...
def _last_restart_age_seconds(
    ...
    *,
    ...
    latest = 0.0
    for event in events:
        if event.get("event") != "restart" or str(event.get("name") or "") != str(
            name or ""
        ):
            continue
...
def _restart_storm_quarantine_allowed(
    name: str, row: Dict[str, Any] | None = None
) -> bool:
    row = row or {}
    ...
    live_execution_critical = _safe_bool(
        row.get("live_execution_critical"), default=(impact == "execution_lane")
    )
    return bool(
        impact == "read_only_collection" and allowed and not live_execution_critical
    )


def _restart_storm_isolation_contract(
    ...
    for storm in restart_storms:
        ...
        blocks_execution_clear = _safe_bool(
            storm.get("blocks_execution_clear"), default=not quarantinable
        )
    ...
    *,
    ...
    return {
        ...
        "last_restart_age_seconds": (
            round(float(last_age), 3) if last_age is not None else None
        ),
        "cooldown_seconds": int(cooldown),
    ...
    *,
    ...
    for name, count in sorted(by_name.items()):
        ...
        row_min_healthy_seconds = max(
            int(
                row.get(
                    "restart_storm_min_healthy_seconds", min(row_settle_seconds, 120)
                )
                or min(row_settle_seconds, 120)
            ),
            60,
        )
        if bool(row.get("parent_process_required", False)):
            if bool(row.get("effective_process_live", False)) or bool(
                row.get("launcher_artifact_certified_fanout", False)
            ):
                running_count = 1
            else:
        ...
        heartbeat_max_age_seconds = float(
            row.get("heartbeat_max_age_seconds", 0.0) or 0.0
        )
        paused_by_safety_flags = bool(row.get("paused_by_safety_flags", False))
        paused_by_creative_cotenant = bool(
        ...
        unresolved = (
            running_count <= 0 or not heartbeat_ok or last_age < row_settle_seconds
        )
        healthy_and_fresh = (
            running_count > 0
            and heartbeat_ok
        ...
        if (
            ...
            or paused_by_creative_cotenant
            or paused_by_runtime_gate
        ):
            unresolved = False
        ...
        blocks_execution_clear = bool(
            live_execution_critical or impact == "execution_lane"
        )
        storm = {
            "name": name,
            "count": int(count),
            ...
            "quarantine_state": (
                "isolated_read_only_collection" if unresolved and quarantinable else ""
            ),
        ...
        if paused_by_safety_flags:
            storm["resolution_reason"] = str(
                row.get("safety_pause_reason") or "paused_by_safety_flags"
            )
        elif paused_by_creative_cotenant:
            storm["resolution_reason"] = str(
                row.get("creative_pause_reason") or "creative_cotenant_pause_active"
            )
...
def _forgive_resolved_restart_debt(
    ...
    for event in events:
        if (
            event.get("event") == "restart"
            and str(event.get("name") or "") in forgiven_names
        ):
            removed += 1
            continue
        kept.append(event)
...
def _sql_link_writer_idle_health() -> Dict[str, Any]:
    ...
    def _state_idle(state: Dict[str, Any]) -> bool:
        ...
        if state.get("ok") is False:
            return False
        status = (
            str(state.get("overall_status") or state.get("status") or "")
            .strip()
            .lower()
        )
        if status in {"critical", "degraded", "error", "failed", "failure"}:
            return False
        current_step = str(
            state.get("effective_current_step") or state.get("current_step") or ""
        ).strip()
    ...
    for key in (
        ...
        "writer_state_after_remediation",
        "writer_state_before",
    ):
    ...
    total_pending = _safe_int(
        queue_payload.get(
            "total_pending_lines", queue_payload.get("pending_lines_total")
        ),
        core_pending,
    )
    oldest_pending_age = _safe_float(
        queue_payload.get("oldest_pending_age_seconds"), 0.0
    )
    idle_max_core = max(
        _safe_int(
            os.getenv("OPS_WATCHDOG_SQL_WRITER_IDLE_MAX_CORE_PENDING_LINES"), 500
        ),
        0,
    )
    idle_max_total = max(
        _safe_int(
            os.getenv("OPS_WATCHDOG_SQL_WRITER_IDLE_MAX_TOTAL_PENDING_LINES"), 1500
        ),
        idle_max_core,
    )
    idle_max_age = max(
        _safe_float(os.getenv("OPS_WATCHDOG_SQL_WRITER_IDLE_MAX_AGE_SECONDS"), 300.0),
        0.0,
    )
    ...
    return {
        ...
        "cycle_current_step": str(
            cycle_state.get("effective_current_step")
            or cycle_state.get("current_step")
            or ""
        ),
        "process_current_step": str(process_health.get("current_step") or ""),
        "progress_current_step": str(
            progress_payload.get("effective_current_step")
            or progress_payload.get("current_step")
            or ""
        ),
        "completed_shard_count": _safe_int(
            cycle_state.get(
                "completed_shard_count", process_health.get("completed_shard_count")
            ),
            0,
        ),
        "planned_shard_count": _safe_int(
            cycle_state.get(
                "planned_shard_count", process_health.get("planned_shard_count")
            ),
            0,
        ),
...
def _sql_link_writer_recovery_health() -> Dict[str, Any]:
    ...
    current_step = str(
        state.get("effective_current_step")
        or state.get("current_step")
        or process_health.get("state")
        or ""
    ).strip()
    ...
    active = bool(
        ...
        or str(process_health.get("state") or "").strip().lower()
        == "active_progressing"
    )
    orphaned = bool(
        state.get("progress_orphaned", process_health.get("progress_orphaned", False))
    )
    handoff_needed = bool(
        state.get(
            "complete_lock_handoff_needed",
            state.get("completed_lock_handoff_needed", False),
        )
    ...
    return {
        "ok": ok,
        "reason": (
            "sql_writer_active_progress_recovered"
            if ok
            else "sql_writer_active_progress_not_clear"
        ),
        "cycle_artifact_fresh": _fresh(cycle_path),
        "process_artifact_fresh": _fresh(process_path),
        ...
        "writer_lock_held": bool(
            state.get("writer_lock_held", process_health.get("writer_lock_held", False))
        ),
        "child_writer_active": bool(
            state.get(
                "child_writer_active", process_health.get("child_writer_active", False)
            )
        ),
        "completed_shard_count": _safe_int(
            state.get(
...
def _row_intentionally_held(row: Dict[str, Any]) -> bool:
    ...
    reason = str(
        row.get("reason")
        or row.get("safety_pause_reason")
        or row.get("creative_pause_reason")
        or ""
    ).strip()
    if (
...
def _watchdog_need_for_row(row: Dict[str, Any]) -> Dict[str, Any] | None:
    ...
    reason = str(
        row.get("reason") or row.get("restart_reason") or restart_skipped or "unknown"
    )
    ...
    if not process_live:
        ...
        expected = (
            "restart the missing read-only watchdog target and refresh its heartbeat"
        )
    ...
    if restart_skipped == "budget_exhausted":
        ...
        expected = (
            "prevent a restart storm by pausing restarts until the target is stable"
        )
    ...
    severity = (
        "critical"
        if blocker in {"restart_budget_exhausted", "process_missing"}
        else "warn"
    )
    if quarantinable:
        severity = "warn"
...
def _watchdog_intelligence_contract(
    *,
    ...
    intentional_holds = [
        need for need in needs if need.get("status") == "intentional_hold"
    ]
    stale_targets = [
        str(row.get("name") or "")
        for row in status_rows
    ...
    score = 100.0
    score -= min(
        float(restart_storm_isolation["execution_blocking_count"]) * 28.0, 56.0
    )
    ...
    if (
        int(restart_storm_isolation["execution_blocking_count"]) > 0
        or restart_budget_execution_blocks
    ):
    ...
    return {
        ...
        "restart_budget_execution_blocks": [
            name for name in restart_budget_execution_blocks if name
        ],
        "restart_budget_isolated_blocks": [
            name for name in restart_budget_isolated_blocks if name
        ],
        "exact_needs": needs,
...
def _resource_guard_allows_job(
    job_name: str, profile: str = "optional"
) -> tuple[bool, str]:
...
def _refresh_runtime_reports(
    ...
    *,
    ...
    stuck_refresh_seconds = max(
        int(os.getenv("OPS_WATCHDOG_REFRESH_STUCK_SECONDS", "900") or 900), 120
    )
    ...
    if _file_age_seconds(one_numbers) > float(max_age_seconds):
        guard_ok, guard_detail = _resource_guard_allows_job(
            "one_numbers_refresh", profile="refresh"
        )
        ...
        if not guard_ok:
            ...
            running_seconds = _proc_elapsed_seconds(
                "scripts/build_one_numbers_report.py"
            )
            out["one_numbers"]["running_seconds"] = (
                round(float(running_seconds), 2)
            ...
            out["one_numbers"]["error"] = (
                ...
                if running_seconds is not None
                and float(running_seconds) >= float(stuck_refresh_seconds)
    ...
    out["one_numbers"]["synced_health"] = _copy_if_exists(
        one_numbers, one_numbers_health
    )
    ...
    if _file_age_seconds(paper_performance) > float(max_age_seconds):
        guard_ok, guard_detail = _resource_guard_allows_job(
            "paper_performance_refresh", profile="refresh"
        )
        ...
        if not guard_ok:
            ...
            out["paper_performance"]["running_seconds"] = (
                round(float(running_seconds), 2)
                if running_seconds is not None
                else None
            )
    ...
    out["paper_performance"]["age_seconds_after"] = round(
        _file_age_seconds(paper_performance), 2
    )

    if _file_age_seconds(daily_summary) > float(max_age_seconds):
        guard_profile = str(
            os.getenv("OPS_WATCHDOG_DAILY_SUMMARY_RESOURCE_GUARD_PROFILE", "refresh")
            or "refresh"
        )
        guard_ok, guard_detail = _resource_guard_allows_job(
            "daily_runtime_summary_refresh", profile=guard_profile
        )
        ...
        if not guard_ok:
            ...
            rc, stdout, err = _run(
                [
                    str(PY),
                    str(PROJECT_ROOT / "scripts" / "daily_runtime_summary.py"),
                    "--day",
                    day,
                    "--json",
                ]
            ...
            if rc == 0 and stdout:
                try:
                    ...
                    out["daily_runtime_summary"][
                        "error"
                    ] = f"parse_or_write_failed:{exc}"

    out["daily_runtime_summary"]["synced_health"] = _copy_if_exists(
        daily_summary, daily_summary_health
    )
    out["daily_runtime_summary"]["age_seconds_after"] = round(
        _file_age_seconds(daily_summary), 2
    )

    if _file_age_seconds(backpressure) > float(backpressure_age):
        guard_ok, guard_detail = _resource_guard_allows_job(
            "ingestion_backpressure_refresh", profile="refresh"
        )
        ...
        if not guard_ok:
            ...
            out["ingestion_backpressure"]["running_seconds"] = (
                round(float(running_seconds), 2)
                if running_seconds is not None
                else None
            )
    ...
    out["ingestion_backpressure"]["age_seconds_after"] = round(
        _file_age_seconds(backpressure), 2
    )

    if _file_age_seconds(divergence) > float(divergence_age):
        guard_ok, guard_detail = _resource_guard_allows_job(
            "data_source_divergence_refresh", profile="refresh"
        )
        ...
        if not guard_ok:
            ...
            out["data_source_divergence"]["running_seconds"] = (
                round(float(running_seconds), 2)
                if running_seconds is not None
                else None
            )
    ...
    out["data_source_divergence"]["age_seconds_after"] = round(
        _file_age_seconds(divergence), 2
    )
    return out
...
def _fanout_guard_allows_core_sleeve_restart() -> Tuple[bool, str]:
    ...
    startup_policy = (
        ...
        if isinstance(payload.get("startup_policy"), dict)
        else {}
    )
    ...
    kill_plan = (
        payload.get("kill_plan") if isinstance(payload.get("kill_plan"), list) else []
    )
...
def _operator_mode_allows_core_sleeve_restart() -> Tuple[bool, str]:
    ...
    if _env_flag("ALL_SLEEVES_PAUSED_BY_OPERATOR_MODE", "0") or _env_flag(
        "LIVE_SLEEVES_PAUSED_BY_OPERATOR_MODE", "0"
    ):
        return False, "all_sleeves_explicitly_paused_by_operator_mode"
    if _env_flag("ALL_SLEEVES_PAUSED_FOR_COMPUTER_TASK", "0") or _env_flag(
        "LIVE_SLEEVES_PAUSED_FOR_COMPUTER_TASK", "0"
    ):
        return False, "all_sleeves_explicitly_paused_for_computer_task"
    allow_readonly_restart = _env_flag(
        "OPS_WATCHDOG_ALLOW_READONLY_SLEEVE_RESTART_DURING_DOWNSHIFT", "1"
    )
    if _env_flag("TRAINING_RUNTIME_PAUSED_BY_OPERATOR_MODE", "0") or _env_flag(
        "SHADOW_RESEARCH_PAUSED_BY_OPERATOR_MODE", "0"
    ):
        if allow_readonly_restart:
            return (
                True,
                "training_or_research_paused_but_readonly_sleeve_restart_allowed",
            )
        return False, "operator_mode_paused_training_or_research"
    if _env_flag("TRAINING_RUNTIME_PAUSED_FOR_COMPUTER_TASK", "0") or _env_flag(
        "SHADOW_RESEARCH_PAUSED_FOR_COMPUTER_TASK", "0"
    ):
        if allow_readonly_restart:
            return (
                True,
                "computer_task_paused_training_or_research_but_readonly_sleeve_restart_allowed",
            )
    ...
    if _env_flag("COMPUTER_NORMAL_USE_GOVERNOR_ACTIVE", "0") and not _env_flag(
        "ROSTER_EXPANSION_ALLOWED", "0"
    ):
        if allow_readonly_restart:
            return (
                True,
                "computer_normal_use_governor_but_readonly_sleeve_restart_allowed",
            )
        return False, "computer_normal_use_governor_active"
    if _env_flag("BACKLOG_INTAKE_GOVERNOR_ACTIVE", "0") and not _env_flag(
        "ROSTER_EXPANSION_ALLOWED", "0"
    ):
        if allow_readonly_restart:
...
def _all_sleeves_start_ready(broker: str, simulate: bool) -> Tuple[bool, str]:
    ...
    if not _split_csv(os.getenv("SHADOW_SYMBOLS_DEFENSIVE", "")) and not _split_csv(
...
def _build_all_sleeves_target(heartbeat_max_age_seconds: int) -> Dict[str, Any]:
    ...
    return {
        ...
        "parent_process_required": _env_flag(
            "OPS_WATCHDOG_ALL_SLEEVES_PARENT_REQUIRED", "1"
        ),
        ...
        "orphan_cleanup_grace_seconds": float(
            os.getenv("OPS_WATCHDOG_ALL_SLEEVES_ORPHAN_CLEANUP_GRACE_SECONDS", "3") or 3
        ),
        "min_child_processes": max(
            _safe_int(os.getenv("OPS_WATCHDOG_ALL_SLEEVES_MIN_CHILDREN", "4"), 4), 0
        ),
        "child_fanout_grace_seconds": child_fanout_grace_seconds,
        "heartbeat_startup_grace_seconds": max(
            float(
                os.getenv(
                    "OPS_WATCHDOG_ALL_SLEEVES_HEARTBEAT_STARTUP_GRACE_SECONDS",
                    str(int(child_fanout_grace_seconds)),
                )
                or child_fanout_grace_seconds
            ),
            child_fanout_grace_seconds,
        ),
        "launcher_health_path": str(
            PROJECT_ROOT / "governance" / "health" / "all_sleeves_launcher_latest.json"
        ),
        "repair_infrabots": [
        ...
        "restart_storm_min_healthy_seconds": max(
            int(
                os.getenv("OPS_WATCHDOG_ALL_SLEEVES_MIN_HEALTHY_SECONDS", "90") or "90"
            ),
            60,
        ),
    }
...
def _build_execution_lane_target(
    mode: str, *, heartbeat_max_age_seconds: int
) -> Dict[str, Any]:
    ...
    return {
        ...
        "heartbeat_glob": str(
            ...
            / "governance"
            / "health"
            / f"execution_lane_{safe_mode}_latest.json"
        ),
        "heartbeat_max_age_seconds": max(int(heartbeat_max_age_seconds), 60),
        ...
        "duplicate_cleanup_grace_seconds": max(
            float(
                os.getenv(
                    "OPS_WATCHDOG_EXECUTION_DUPLICATE_CLEANUP_GRACE_SECONDS", "1.0"
                )
                or 1.0
            ),
            0.2,
        ...
        "restart_storm_min_healthy_seconds": max(
            int(min_healthy_seconds_env or "120"), 60
        ),
...
def _probe_host(hostport: str, timeout_seconds: float) -> Dict[str, Any]:
    ...
    try:
        with socket.create_connection(
            (host, port), timeout=max(float(timeout_seconds), 0.2)
        ):
...
def _probe_storage_mount() -> Dict[str, Any]:
    ...
    external_available = bool(
        mount_present
        and external_root_exists
        and external_root_writable
        and not external_low_space
    )
    return {
        "mount_root": str(mount_root),
        "external_root": str(external_root),
...
def _evaluate_storage_mount_transition(
    previous_mount_present: Any, mount_present_now: bool
) -> Dict[str, Any]:
...
def _kickstart_labels(labels: List[str]) -> List[Dict[str, Any]]:
    ...
    for label in labels:
        ...
        actions.append(
            {"label": label, "rc": int(rc), "stdout": out[-200:], "stderr": err[-200:]}
        )
    return actions
...
def _storage_mode_transition_alert(
    previous_mode: str, current_mode: str, *, suppress_seconds: int
) -> Dict[str, Any] | None:
    ...
    } and current_mode in {"external", "external_curated"}:
        return _alert(
            "info",
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Watchdog: restart key loops with bounded backoff."
    )
    parser.add_argument(
        "--max-restarts-per-hour",
        type=int,
        default=int(os.getenv("OPS_WATCHDOG_MAX_RESTARTS_PER_HOUR", "6")),
    )
    parser.add_argument(
        "--require-all-sleeves",
        action="store_true",
        default=os.getenv("OPS_WATCHDOG_REQUIRE_ALL_SLEEVES", "1") == "1",
    )
    parser.add_argument(
        "--require-coinbase",
        action="store_true",
        default=os.getenv("OPS_WATCHDOG_REQUIRE_COINBASE", "1") == "1",
    )
    parser.add_argument(
        "--require-coinbase-futures",
        action="store_true",
        default=os.getenv("OPS_WATCHDOG_REQUIRE_COINBASE_FUTURES", "1") == "1",
    )
    parser.add_argument(
        "--require-paper-executor",
        action="store_true",
        default=_default_require_paper_executor(),
    )
    parser.add_argument(
        "--refresh-reports",
        action="store_true",
        default=os.getenv("OPS_WATCHDOG_REFRESH_REPORTS", "1") == "1",
    )
    parser.add_argument(
        "--refresh-max-age-seconds",
        type=int,
        default=int(os.getenv("OPS_WATCHDOG_REFRESH_MAX_AGE_SECONDS", "7200")),
    )
    parser.add_argument(
        "--all-sleeves-heartbeat-stale-seconds",
        type=int,
        default=int(
            os.getenv("OPS_WATCHDOG_ALL_SLEEVES_HEARTBEAT_STALE_SECONDS", "360")
        ),
    )
    parser.add_argument(
        "--coinbase-heartbeat-stale-seconds",
        type=int,
        default=int(os.getenv("OPS_WATCHDOG_COINBASE_HEARTBEAT_STALE_SECONDS", "420")),
    )
    parser.add_argument(
        "--paper-executor-heartbeat-stale-seconds",
        type=int,
        default=int(
            os.getenv("OPS_WATCHDOG_PAPER_EXECUTOR_HEARTBEAT_STALE_SECONDS", "240")
        ),
    )
    parser.add_argument(
        "--network-guard",
        action="store_true",
        default=_env_flag("OPS_WATCHDOG_NETWORK_GUARD", "1"),
    )
    parser.add_argument(
        "--network-hosts",
        default=os.getenv(
            "OPS_WATCHDOG_NETWORK_HOSTS",
            "api.schwabapi.com:443,api.exchange.coinbase.com:443",
        ),
    )
    parser.add_argument(
        "--network-timeout-seconds",
        type=float,
        default=float(os.getenv("OPS_WATCHDOG_NETWORK_TIMEOUT_SECONDS", "2.5")),
    )
    parser.add_argument(
        "--network-fail-threshold",
        type=int,
        default=int(os.getenv("OPS_WATCHDOG_NETWORK_FAIL_THRESHOLD", "3")),
    )
    parser.add_argument(
        "--restart-storm-threshold",
        type=int,
        default=int(os.getenv("OPS_WATCHDOG_RESTART_STORM_THRESHOLD", "4")),
    )
    parser.add_argument(
        "--restart-storm-window-seconds",
        type=int,
        default=int(os.getenv("OPS_WATCHDOG_RESTART_STORM_WINDOW_SECONDS", "3600")),
    )
    parser.add_argument(
        "--restart-storm-settle-seconds",
        type=int,
        default=int(os.getenv("OPS_WATCHDOG_RESTART_STORM_SETTLE_SECONDS", "900")),
    )
    parser.add_argument(
        "--alert-suppress-seconds",
        type=int,
        default=int(os.getenv("OPS_WATCHDOG_ALERT_SUPPRESS_SECONDS", "600")),
    )
    parser.add_argument(
        "--maintenance-timeout-seconds",
        type=float,
        default=DEFAULT_MAINTENANCE_TIMEOUT_SECONDS,
    )
    parser.add_argument(
        "--singleton-lock-path", default=str(DEFAULT_SINGLETON_LOCK_PATH)
    )
    parser.add_argument(
        "--readonly-repair-probe-cooldown-seconds",
        type=int,
        default=int(
            os.getenv("OPS_WATCHDOG_READONLY_REPAIR_PROBE_COOLDOWN_SECONDS", "900")
        ),
    )
    parser.add_argument("--json", action="store_true")
    args = parser.parse_args()

    singleton_handle, singleton_owner = _acquire_singleton_lock(
        Path(args.singleton_lock_path).expanduser()
    )
    if singleton_handle is None:
        ...
        if args.json:
            ...
            print(
                f"process_watchdog skipped=already_running owner={singleton_owner or 'unknown'}"
            )
        return 0
    ...
    storage_mount_prev = (
        None if storage_mount_prev_raw is None else bool(storage_mount_prev_raw)
    )
    ...
    maintenance_jobs = [
        (
            ...
            [
                str(PY),
                str(PROJECT_ROOT / "scripts" / "ops" / "lock_watchdog.py"),
                "--apply",
                "--json",
            ],
        ),
        (
            ...
            [
                str(PY),
                str(PROJECT_ROOT / "scripts" / "ops" / "canary_auto_tuner.py"),
                "--json",
            ],
        ),
    ]
    if hot_storage_prefers_external:
        maintenance_jobs.insert(
            1, ("storage_failback_sync", _storage_failback_sync_cmd())
        )
    else:
        maintenance.append(
            {
    ...
    for name, cmd in maintenance_jobs:
        rc, out, err = _run(
            cmd, timeout_seconds=float(args.maintenance_timeout_seconds)
        )
        row: Dict[str, Any] = {
            "name": name,
            "ok": rc == 0,
    ...
    mount_transition_base = (
        ...
        else _evaluate_storage_mount_transition(
            storage_mount_prev, storage_mount_present
        )
    )
    if mount_transition_base:
        ...
        if not storage_mount_present:
            unavailable_reason = str(
                storage_mount_guard.get("external_unavailable_reason") or ""
            )
            if unavailable_reason == "volume_unmounted":
                alert_message = f"External BOT_LOGS volume is present but not mounted at {storage_mount_guard.get('mount_root')}."
            else:
    ...
    health_fast_refresh_age = max(
        int(os.getenv("OPS_WATCHDOG_HEALTH_FAST_MAX_AGE_SECONDS", "300")), 60
    )
    if args.refresh_reports:
        refresh_payload = _refresh_runtime_reports(
            max_age_seconds=max(int(args.refresh_max_age_seconds), 60),
            backpressure_max_age_seconds=max(
                int(os.getenv("OPS_WATCHDOG_BACKPRESSURE_MAX_AGE_SECONDS", "300")), 60
            ),
            divergence_max_age_seconds=max(
                int(os.getenv("OPS_WATCHDOG_DIVERGENCE_MAX_AGE_SECONDS", "600")), 60
            ),
    ...
    network_payload: Dict[str, Any] = {
        "enabled": bool(args.network_guard),
        "results": [],
    }
    ...
    if args.network_guard:
        ...
        results = [
            _probe_host(h, timeout_seconds=float(args.network_timeout_seconds))
            for h in hosts
        ]
        ...
        network_outage_active = network_fail_streak_now >= max(
            int(args.network_fail_threshold), 1
        ) and (not any_ok)
    ...
    if args.require_all_sleeves:
        targets.append(
            _build_all_sleeves_target(
                heartbeat_max_age_seconds=args.all_sleeves_heartbeat_stale_seconds
            )
        )
    ...
    if args.require_coinbase:
        coinbase_cmd: List[str] = [
            ...
            os.getenv(
                ...
                "BTC-USD,ETH-USD,SOL-USD,AVAX-USD,LTC-USD,LINK-USD,DOGE-USD",
            ),
            "--interval-seconds",
        ...
        targets.append(
            {
                ...
                "exclude_patterns": _live_data_excludes(
                    coinbase_simulate, ["--profile crypto_futures"]
                ),
                "cmd": coinbase_cmd,
                ...
                "heartbeat_glob": str(
                    ...
                    / "governance"
                    / "health"
                    / "shadow_loop_default_crypto_coinbase_*.json"
                ),
                "heartbeat_max_age_seconds": max(
                    int(args.coinbase_heartbeat_stale_seconds), 60
                ),
                "max_running": 1,
                "duplicate_cleanup_grace_seconds": max(
                    float(
                        os.getenv(
                            "OPS_WATCHDOG_COINBASE_DUPLICATE_CLEANUP_GRACE_SECONDS",
                            "1.0",
                        )
                        or 1.0
                    ),
    ...
    if args.require_coinbase_futures:
        ...
        coinbase_futures_cmd: List[str] = [
            ...
            "--context-symbols",
            os.getenv(
                "COINBASE_FUTURES_CONTEXT_SYMBOLS",
                "BTC-USD,ETH-USD,SOL-USD,AVAX-USD,LTC-USD,LINK-USD,DOGE-USD",
            ),
            "--interval-seconds",
        ...
        coinbase_futures_simulate = _env_flag(
            "OPS_WATCHDOG_COINBASE_FUTURES_SIMULATE", "0"
        )
        ...
        targets.append(
            {
                ...
                "heartbeat_glob": str(
                    ...
                    / "governance"
                    / "health"
                    / "shadow_loop_*crypto_futures*_crypto_coinbase_*.json"
                ),
                "heartbeat_max_age_seconds": max(
                    int(args.coinbase_heartbeat_stale_seconds), 60
                ),
    ...
    safety_pause_target_names = {
        str(t.get("name") or "")
        for t in targets
        if str(t.get("name") or "") != "sql_link_writer"
    }

    for t in targets:
        running = _proc_running(
            t["pattern"], exclude_patterns=t.get("exclude_patterns", [])
        )
        alt_running_values = [
        ...
        process_scan_unknown = running == PROCESS_SCAN_UNKNOWN or any(
            value == PROCESS_SCAN_UNKNOWN for value in alt_running_values
        )
        alt_running = sum(max(int(value), 0) for value in alt_running_values)
        duplicate_cleanup: Dict[str, Any] = {}
        max_running = _safe_int(t.get("max_running"), 0)
        matched_pid_count = 0
        if max_running > 0 and not process_scan_unknown:
            matched_pid_count = len(
                _matching_pids(
                    str(t["pattern"]), exclude_patterns=t.get("exclude_patterns", [])
                )
            )
        if max_running > 0 and matched_pid_count > max_running:
            duplicate_cleanup = _trim_duplicate_processes(
                ...
                grace_seconds=float(
                    t.get("duplicate_cleanup_grace_seconds", 1.0) or 1.0
                ),
            )
            running = _proc_running(
                t["pattern"], exclude_patterns=t.get("exclude_patterns", [])
        ...
        heartbeat_age = (
            _latest_heartbeat_age_seconds(heartbeat_glob) if heartbeat_required else 0.0
        )
        heartbeat_max_age = float(t.get("heartbeat_max_age_seconds", 0) or 0.0)
        heartbeat_fresh = (not heartbeat_required) or (
            heartbeat_age <= heartbeat_max_age
        )
        ...
        process_elapsed_seconds = (
            ...
            if running > 0 and not process_scan_unknown
            else None
        )
        parent_elapsed_seconds = (
            process_elapsed_seconds if parent_process_required else None
        )
        child_fanout = _child_fanout_health(
            t,
            running=max(int(running), 0),
        ...
        if t.get("name") == "all_sleeves":
            ...
            launcher_artifact_certified_fanout = bool(
                ...
                and ((child_process_live and child_fanout_ok) or process_scan_unknown)
                and launcher_artifact_health.get("ok", False)
            )
        process_live = (
            (running > 0)
            if parent_process_required
            else ((running > 0) or child_process_live)
        )
        if process_scan_unknown and heartbeat_fresh:
            process_live = True
        if launcher_artifact_certified_fanout:
            ...
            child_fanout = {
                **child_fanout,
                "ok": True,
                "reason": str(
                    launcher_artifact_health.get("reason")
                    or "launcher_artifact_certified_fanout"
                ),
                "certified_by_launcher_artifact": True,
            }
        heartbeat_startup_grace_seconds = max(
            float(t.get("heartbeat_startup_grace_seconds", 0.0) or 0.0), 0.0
        )
        ...
        heartbeat_ok = (
            heartbeat_fresh
            and (process_live or not heartbeat_required)
            and child_fanout_ok
        )
        ...
        if process_scan_unknown:
            row["process_scan_unavailable"] = True
            row["process_live_reason"] = (
                "process_scan_unavailable_heartbeat_fresh"
                if heartbeat_fresh
                else "process_scan_unavailable"
            )
            row["process_scan_policy"] = (
                "do_not_restart_or_cleanup_from_unknown_process_enumeration"
            )
        ...
        for key in (
            "restart_storm_impact",
            "restart_storm_quarantine_allowed",
            "live_execution_critical",
        ):
            if key in t:
                row[key] = t[key]
        ...
        if "restart_storm_min_healthy_seconds" in t:
            row["restart_storm_min_healthy_seconds"] = int(
                t["restart_storm_min_healthy_seconds"]
            )
        if alt_running > 0:
        ...
        if heartbeat_required:
            ...
            if heartbeat_startup_grace_active:
                ...
                row["heartbeat_startup_grace_seconds"] = round(
                    float(heartbeat_startup_grace_seconds), 2
                )
        ...
        if t["name"] == "sql_link_writer" and not process_live:
            ...
            if bool(writer_idle_health.get("ok", False)):
                ...
                row["process_live_reason"] = str(
                    writer_idle_health.get("reason")
                    or "sql_writer_on_demand_idle_complete"
                )
            else:
                ...
                if bool(writer_recovery_health.get("ok", False)):
                    ...
                    row["virtual_process_live"] = True
                    row["process_live_reason"] = str(
                        writer_recovery_health.get("reason")
                        or "sql_writer_active_progress_recovered"
                    )
        ...
        if safety_pause["active"] and t["name"] in safety_pause_target_names:
            ...
            row["safety_pause_reason"] = str(
                safety_pause.get("reason") or "safety_pause_active"
            )
            row["operator_stop_active"] = bool(
                safety_pause.get("operator_stop_active", False)
            )
            row["global_halt_active"] = bool(
                safety_pause.get("global_halt_active", False)
            )
        ...
        if _creative_pause_suppresses_target(str(t.get("name") or ""), creative_pause):
            ...
            row["creative_pause_reason"] = str(
                creative_pause.get("reason") or "creative_cotenant_pause_active"
            )
            row["creative_session_level"] = str(
                creative_pause.get("creative_session_level") or ""
            )
            row["creative_session_kind"] = str(
                creative_pause.get("creative_session_kind") or ""
            )
        ...
        if t["name"] == "execution_lane_paper":
            ...
            if bool(runtime_pause.get("paused", False)):
                ...
                row["runtime_pause_reason"] = str(
                    runtime_pause.get("reason") or "runtime_paper_execution_paused"
                )
        ...
        if process_scan_unknown:
            row["restart_skipped"] = "process_scan_unavailable"
            row["reason"] = (
                "process enumeration unavailable; refusing restart from uncertain process evidence"
            )
            status.append(row)
            continue
        ...
        if bool(launcher_priority.get("required", False)) and not bool(
            launcher_priority.get("compliant", False)
        ):
        ...
        all_sleeves_orphans_without_parent = (
            parent_process_required and running <= 0 and alt_running > 0
        )
        if t["name"] == "all_sleeves" and all_sleeves_orphans_without_parent:
            cleanup_patterns = (
                t.get("orphan_cleanup_patterns")
                if isinstance(t.get("orphan_cleanup_patterns"), list)
                else []
            )
        ...
        if t["name"] == "all_sleeves":
            ready, reason = _all_sleeves_start_ready(
                str(t.get("broker", "schwab")), bool(t.get("simulate", False))
            )
            if not ready:
        ...
        if not bool(restart_authority.get("ok", False)):
            ...
            row["reason"] = ",".join(
                str(item) for item in restart_authority.get("blockers", []) if str(item)
            )
            status.append(row)
            continue
        ...
        if not _within_budget(events, t["name"], args.max_restarts_per_hour):
            ...
            if bool(restart_budget_repair_probe.get("allowed", False)):
                ...
                status.append(row)
                alert_severity, alert_event = _restart_budget_alert_metadata(
                    str(t["name"]), row
                )
                alerts.append(
                    {
        ...
        if parent_process_required and running > 0 and not child_fanout_ok:
            restart_reason = str(
                child_fanout.get("reason") or "child_fanout_below_floor"
            )
        ...
        elif all_sleeves_orphans_without_parent:
        ...
        try:
            with component_action_guard(
                ...
                if not process_live and running_under_lease > 0:
                    ...
                    row["reason"] = (
                        "peer_supervisor_recovered_target_under_shared_restart_lease"
                    )
                    status.append(row)
                    continue
        ...
        evt = {
            ...
            "budget_repair_probe": bool(
                restart_budget_repair_probe.get("allowed", False)
            ),
        }
        events.append(evt)
        restarts.append(evt)
    ...
    for storm in restart_storms:
        storm_isolated = bool(storm.get("quarantinable", False)) and not bool(
            storm.get("blocks_execution_clear", True)
        )
        alert_severity = "warn" if storm_isolated else "critical"
        alert_key = (
            "watchdog_restart_storm_isolated"
            if storm_isolated
            else "watchdog_restart_storm"
        )
        alerts.append(
            {
                "name": storm["name"],
    ...
    events, restart_debt_forgiveness = _forgive_resolved_restart_debt(
        events, recent_restart_storms
    )
    ...
    state = {
        ...
        "storage_external_available": bool(
            storage_mount_guard.get("external_available", False)
        ),
