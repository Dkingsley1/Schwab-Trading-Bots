...
def _candidate(project_root: Path) -> dict[str, Any]:
    ...
    soak_started = max(
        (value for value in window_values if value is not None), default=None
    )
    identity_bound = bool(
        str(state.get("candidate_id") or "").strip() and soak_started is not None
    )
    ...
    return {
        ...
        "soak_started_utc": (
            soak_started.isoformat() if soak_started is not None else ""
        ),
...
def _runtime_collection_active(project_root: Path) -> tuple[bool, dict[str, Any]]:
    ...
    live_runtime = load_json(
        project_root / "governance" / "health" / "live_runtime_status_latest.json"
    )
    watchdog_status = (
        str(watchdog.get("overall_status") or watchdog.get("status") or "")
        .strip()
        .lower()
    )
    runtime_status = (
        str(live_runtime.get("overall_status") or live_runtime.get("status") or "")
        .strip()
        .lower()
    )
    process_count = max(
    ...
    active = bool(
        watchdog_status in {"ready", "ok", "active"}
        or runtime_status in {"ready", "ok", "active"}
        or process_count > 0
    )
    return active, {
...
def _metric(
    ...
    *,
    ...
    progress = (
        1.0
        if target <= 0.0 and current >= target
        else max(0.0, min(float(current) / float(target), 1.0)) if target > 0.0 else 0.0
    )
    return {
        "metric_id": metric_id,
        "label": label,
...
def _candidate_bound_artifact_state(
    ...
    *,
    ...
    candidate_started = (
        candidate.get("soak_started")
        if isinstance(candidate.get("soak_started"), datetime)
        else None
    )
    ...
    *,
    ...
    reason = (
        ...
        if payload and current
            ...
            f"{producer_id}_candidate_epoch_stale"
            if payload
            else f"{producer_id}_artifact_missing"
        )
    )
    return (
        _producer(
...
def _raw_metrics(
    project_root: Path, *, candidate: dict[str, Any], now: datetime
) -> list[dict[str, Any]]:
    ...
    replay_capture = load_json(
        project_root
        / "governance"
        / "health"
        / "market_replay_fill_capture_latest.json"
    )
    ...
    advancement = load_json(
        project_root
        / "governance"
        / "health"
        / "promotion_candidate_advancement_latest.json"
    )
    rollout = load_json(
        project_root / "governance" / "health" / "canary_rollout_latest.json"
    )
    rollout_thresholds = _as_dict(rollout.get("thresholds"))
    rollout_canary = _as_dict(rollout.get("canary_statistics"))
    profitability = load_json(
        project_root
        / "governance"
        / "health"
    ...
    minimum_cohort_samples = _safe_float(
        rollout_thresholds.get("minimum_samples_per_cohort"), 400.0
    )
    minimum_canary_days = _safe_float(
        rollout_thresholds.get("minimum_independent_days"), 3.0
    )
    minimum_canary_ess = _safe_float(
        rollout_thresholds.get("minimum_effective_samples"), 50.0
    )
    ...
    advancement_epoch = (
        "fresh" if advancement_current and promotion_current else "pre_candidate"
    )
    acquisition_status = (
        str(acquisition.get("overall_status") or acquisition.get("status") or "missing")
        .strip()
        .lower()
    )
    acquisition_ready = bool(
        ...
        and acquisition_status
        not in {"waiting_for_source", "missing", "blocked", "failed", "error"}
    )
    replay_status = (
        str(
            replay_capture.get("overall_status")
            or replay_capture.get("status")
            or "missing"
        )
        .strip()
        .lower()
    ...
    replay_ready = bool(
        ...
        and replay_current
        and replay_status
        in {"ready", "waiting_for_observations", "waiting_for_paper_orders"}
    )
    ...
    fill_producer = _producer(
        ...
        status=(
            ...
            if acquisition_ready
                ...
                f"market_replay_{replay_status}"
                if replay_ready
        ...
        reason=(
            ...
            if calibration and not calibration_current
                ...
                "independent_fill_and_market_replay_sources_unavailable"
    ...
    def cohort_producer(
        cohort: str, row: dict[str, Any], samples: float
    ) -> dict[str, Any]:
        source_ready = bool(
            rollout_current
            and (
                row.get("source_ready", False)
                or samples > 0
                or (not coverage and source_files_seen)
            )
        )
        return _producer(
            ...
            status=(
                ...
                if source_ready
                    ...
                    "stale_candidate_epoch" if not rollout_current else "source_missing"
                )
            ),
            reason=(
                f"canary_rollout_{cohort}_candidate_epoch_stale"
                if not rollout_current
                else f"{cohort}_cohort_source_missing" if not source_ready else ""
            ),
            cadence_hours=6.0,
    ...
    canary_samples = (
        _safe_float(rollout.get("canary_samples")) if rollout_current else 0.0
    )
    baseline_samples = (
        _safe_float(rollout.get("baseline_samples")) if rollout_current else 0.0
    )
    ...
    return [
        ...
        _metric(
            "independent_fills",
            "Independent paper/replay fills",
            (
                _safe_float(calibration.get("independent_samples"))
        ...
        _metric(
            "post_cost_samples",
            "Post-cost trade observations",
            _safe_float(post_cost.get("sample_count")),
            _safe_float(robust_thresholds.get("minimum_samples"), 30.0),
            unit="observations",
            kind="evidence",
            producer=performance_producer,
        ),
        _metric(
            "post_cost_days",
            "Independent post-cost days",
            _safe_float(robust.get("unique_day_count")),
            _safe_float(robust_thresholds.get("minimum_days"), 7.0),
            unit="days",
            kind="evidence",
            producer=daily_performance_producer,
        ),
        _metric(
            "post_cost_symbols",
            "Post-cost symbol breadth",
            _safe_float(robust.get("unique_symbol_count")),
            _safe_float(robust_thresholds.get("minimum_symbols"), 5.0),
            unit="symbols",
            kind="evidence",
            producer=performance_producer,
        ),
        _metric(
            "post_cost_effective_samples",
            "Cluster-effective post-cost samples",
            _safe_float(robust.get("effective_sample_size")),
            _safe_float(robust_thresholds.get("minimum_effective_samples"), 20.0),
        ...
        _metric(
            "considered_bots",
            "Independently considered promotion bots",
            (
                _safe_float(promotion_scope.get("considered_bots"))
                if promotion_scope_active
                else 0.0
            ),
            _safe_float(promotion_scope.get("min_considered_bots"), 4.0),
            unit="bots",
            kind="evidence",
        ...
        _metric(
            ...
            "Qualified promotion candidates",
            (
                float(len(promotion_details.get("promotion_candidate_ids") or []))
                if promotion_scope_active
                else 0.0
            ),
            _safe_float(promotion_scope.get("min_considered_bots"), 4.0),
        ...
        _metric(
            "canary_samples",
            "Candidate canary observations",
            canary_samples,
            minimum_cohort_samples,
            unit="observations",
            kind="evidence",
        ...
        _metric(
            "canary_independent_days",
            "Canary independent days",
            _safe_float(active_rollout_canary.get("unique_day_count")),
            minimum_canary_days,
            unit="days",
            kind="evidence",
            producer=canary_daily_producer,
        ),
        _metric(
            "canary_effective_samples",
            "Canary effective samples",
            _safe_float(active_rollout_canary.get("effective_sample_size")),
            minimum_canary_ess,
            unit="effective_samples",
            kind="evidence",
            producer=canary_daily_producer,
        ...
        _metric(
            "strict_post_cost_samples",
            "Strict post-cost observations",
            _safe_float(post_cost.get("sample_count")),
            _safe_float(strict_policy.get("minimum_post_cost_samples"), 200.0),
            unit="observations",
            kind="evidence",
            producer=cumulative_performance_producer,
        ),
        _metric(
            "strict_independent_days",
            "Strict independent post-cost days",
            _safe_float(robust.get("unique_day_count")),
            _safe_float(strict_policy.get("minimum_independent_days"), 30.0),
            unit="days",
            kind="evidence",
            producer=cumulative_performance_producer,
        ),
        _metric(
            "strict_symbol_breadth",
            "Strict post-cost symbol breadth",
            _safe_float(robust.get("unique_symbol_count")),
            _safe_float(strict_policy.get("minimum_symbols"), 10.0),
            unit="symbols",
            kind="evidence",
            producer=cumulative_performance_producer,
        ),
        _metric(
            "strict_effective_samples",
            "Strict cluster-effective samples",
            _safe_float(robust.get("effective_sample_size")),
        ...
        _metric(
            "strict_regime_breadth",
            "Strict independent regime breadth",
            _safe_float(robust.get("unique_regime_count")),
            _safe_float(strict_policy.get("minimum_regimes"), 3.0),
            unit="regimes",
            kind="evidence",
            producer=cumulative_performance_producer,
        ),
        _metric(
            "strict_profitable_sleeves",
            "Strict independently profitable sleeves",
            _safe_float(
                allocation.get(
                    "qualified_sleeve_count",
                    len(allocation.get("qualified_sleeves") or []),
                )
            ),
            _safe_float(strict_policy.get("minimum_profitable_sleeves"), 4.0),
            unit="sleeves",
            kind="evidence",
            producer=profitability_producer,
        ),
        _metric(
            "positive_conservative_lcb",
            "Positive conservative post-cost lower bound",
            float(
                bool(robust.get("positive_clustered_lower_confidence_bound_95", False))
            ),
            1.0,
            unit="gate",
            kind="evidence",
        ...
        _metric(
            "stressed_expectancy_gate",
            "Adversarial stressed expectancy",
            float(firewall_controls.get("06_stressed_post_cost_expectancy", False)),
            1.0,
            unit="gate",
            kind="evidence",
            producer=profitability_producer,
        ...
        _metric(
            ...
            "FDR and overfit evidence",
            float(
                bool(active_multiple_testing.get("statistical_evidence_ready", False))
            ),
            1.0,
            unit="gate",
            kind="evidence",
            producer=multiple_testing_producer,
        ),
        _metric(
            "holdout_vault_gate",
            "Sealed holdout evaluation evidence",
            float(bool(active_holdout.get("evidence_ready", False))),
            1.0,
            unit="gate",
            kind="evidence",
            producer=holdout_producer,
        ),
        _metric(
            "benchmark_common_days",
            "Cash and passive benchmark common days",
            _safe_float(active_benchmark.get("common_day_count")),
            _safe_float(benchmark_thresholds.get("minimum_common_days"), 30.0),
            unit="days",
            kind="evidence",
            producer=benchmark_producer,
        ),
        _metric(
            "benchmark_hurdle_gate",
            "Cash and passive benchmark hurdle",
            float(bool(active_benchmark.get("evidence_ready", False))),
            1.0,
            unit="gate",
            kind="evidence",
            producer=benchmark_gate_producer,
        ),
        _metric(
            "edge_decay_gate",
            "Edge-decay evidence and containment",
            float(
                bool(
                    edge_contract.get("evidence_ready", False)
                    and edge_contract.get("automatic_demotion_ready", False)
                )
            ),
            1.0,
            unit="gate",
            kind="evidence",
            producer=decay_producer,
        ),
        _metric(
            "risk_of_ruin_days",
            "Risk-of-ruin independent days",
            _safe_float(risk_of_ruin.get("day_count")),
            _safe_float(
                _as_dict(risk_of_ruin.get("thresholds")).get("minimum_days"), 30.0
            ),
            unit="days",
            kind="evidence",
            producer=cumulative_validator_producer,
        ),
        _metric(
            "risk_of_ruin_gate",
            "Risk-of-ruin and drawdown stress",
            float(
                bool(
                    risk_of_ruin.get("available", False)
                    and risk_of_ruin.get("passes", False)
                )
            ),
            1.0,
            unit="gate",
        ...
        _metric(
            ...
            "Tail concentration and conservative allocation",
            float(firewall_controls.get("h09_tail_concentration", False)),
            1.0,
            unit="gate",
            kind="evidence",
            producer=profitability_producer,
        ...
        _metric(
            "profitability_epoch_coherence",
            "Atomic profitability evidence epoch",
            float(bool(epoch_contract.get("ready", False))),
            1.0,
            unit="gate",
            kind="evidence",
            producer=profitability_producer,
        ...
        _metric(
            "raw_net_pnl",
            "Raw paper net PnL",
            _safe_float(target_current.get("net_pnl")),
            _safe_float(target_thresholds.get("min_net_pnl"), 50000.0),
            unit="usd",
            kind="outcome",
        ),
    ]
...
def _enrich_rates(
    ...
    *,
    ...
    prior_metrics = (
        prior_state.get("metrics")
        if isinstance(prior_state.get("metrics"), dict)
        else {}
    )
    same_candidate = bool(candidate_id and candidate_id == prior_candidate)
    elapsed_hours = (
        max((now - prior_timestamp).total_seconds() / 3600.0, 0.0)
        if same_candidate and prior_timestamp is not None
        else 0.0
    )
    ...
    for raw in metrics:
        ...
        prior = (
            prior_metrics.get(metric_id)
            if isinstance(prior_metrics.get(metric_id), dict)
            else {}
        )
        ...
        producer_binding_changed = bool(
            same_candidate
            and producer_binding_key
            and producer_binding_key != prior_binding_key
        )
        ...
        rate = (
            delta / elapsed_hours
            if comparable and elapsed_hours > 0.0 and delta > 0.0
            else None
        )
        ...
        producer_resumed = bool(
            comparable and producer_ready and not prior_producer_ready
        )
        schedule_resumed = bool(
            comparable and schedule_active and not prior_schedule_active
        )
        unchanged_since = (
            parse_iso_utc(prior.get("unchanged_since_utc"))
            if comparable
            and delta <= 0.0
            and not producer_resumed
            and not schedule_resumed
            else now
        )
        ...
        cadence_hours = max(
            _safe_float(producer.get("expected_cadence_hours"), stall_hours), 0.25
        )
        ...
        stalled = bool(
            collection_active
            and expected_to_move
            and not regressed
            and unchanged_hours >= stall_threshold
        )
        eta_hours = (
            float(row["remaining"]) / rate
            if rate is not None and rate > 0.0 and row["remaining"] > 0.0
            else 0.0 if row["complete"] else None
        )
        if row["complete"]:
...
def build_payload(
    ...
    *,
    ...
    waiting_ids = [
        str(row["metric_id"])
        for row in metrics
        if row.get("accrual_state") == "waiting_precondition"
    ]
    ...
    status = (
        ...
        if not candidate.get("credit_eligible", False)
            ...
            if regressed_ids
                ...
                "stalled"
                if stalled_ids
                else "complete" if not pending_ids else "accumulating"
            )
        )
    )
    payload = {
        ...
        "ok": bool(
            candidate.get("credit_eligible", False)
            and not stalled_ids
            and not regressed_ids
        ),
        "candidate_binding": {
            key: value for key, value in candidate.items() if key != "soak_started"
        },
        "collection_active": collection_active,
        ...
        "recommended_actions": ordered_unique(
            [
                (
                    "investigate the producer or route for stalled evidence metrics"
                    if stalled_ids
                    else ""
                ),
                (
                    "investigate evidence counter regression before continuing promotion"
                    if regressed_ids
                    else ""
                ),
                (
                    "satisfy explicit source prerequisites for waiting evidence metrics"
                    if waiting_ids
                    else ""
                ),
                (
                    "continue candidate-bound collection for pending evidence metrics"
                    if pending_ids
                    else ""
                ),
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Track candidate-bound readiness evidence accrual and stalls."
    )
